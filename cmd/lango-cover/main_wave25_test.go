...
func TestWave25RunRejectsInvalidArguments(t *testing.T) {
	t.Parallel()
...
func TestWave25RunDiscoversModuleFromGoMod(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	wave25WriteFile(t, root, "go.mod", "module example.com/wave25\n\ngo 1.23\n")
	wave25WriteFile(t, root, "pkg/covered.go", "package pkg\n")
	profile := wave25WriteCoverage(t, root, []string{
		"mode: set",
		"example.com/wave25/pkg/covered.go:1.1,2.1 7 1",
		"example.com/wave25/pkg/covered.go:3.1,4.1 3 0",
	})
...
func TestWave25RunReturnsErrorForMissingProfile(t *testing.T) {
	t.Parallel()
...
func TestWave25RunAppliesThresholdGateBranches(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	wave25WriteFile(t, root, "go.mod", "module example.com/wave25\n")
	wave25WriteFile(t, root, "pkg/threshold.go", "package pkg\n")
	profile := wave25WriteCoverage(t, root, []string{
		"mode: set",
		"example.com/wave25/pkg/threshold.go:1.1,2.1 90 1",
		"example.com/wave25/pkg/threshold.go:3.1,4.1 10 0",
	})
...
func TestWave25ReadModulePathHandlesMissingAndMalformedGoMod(t *testing.T) {
	t.Parallel()
	...
	wave25WriteFile(t, malformedRoot, "go.mod", "go 1.23\nrequire example.com/dep v1.0.0\n")
	if got := readModulePath(malformedRoot); got != "" {
		t.Fatalf("malformed go.mod module = %q, want empty", got)
	}
}

func wave25WriteFile(t *testing.T, root, rel, content string) {
	t.Helper()
...
func wave25WriteCoverage(t *testing.T, root string, lines []string) string {
	t.Helper()

	path := filepath.Join(root, "wave25.cover.out")
