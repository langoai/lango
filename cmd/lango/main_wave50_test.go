...
func TestWave50NewDeadLetterStatusLoaderWrapsBuildAppError(t *testing.T) {
	t.Parallel()
	...
	loader := newDeadLetterStatusLoader(func() (*bootstrap.Result, error) {
		return &bootstrap.Result{Config: cfg, ProfileName: "wave50"}, nil
	})
...
func TestWave50NewDeadLetterStatusLoaderReportsUnavailableToolsAfterAppBuild(t *testing.T) {
	t.Parallel()

	cfg := config.DefaultConfig()
	cfg.DataRoot = t.TempDir()
	cfg.Session.DatabasePath = filepath.Join(cfg.DataRoot, "lango.db")
	cfg.Agent.Provider = "wave50"
	cfg.Providers = map[string]config.ProviderConfig{
		"wave50": {Type: types.ProviderOpenAI, APIKey: "test-key"},
	}
	loader := newDeadLetterStatusLoader(func() (*bootstrap.Result, error) {
		return &bootstrap.Result{Config: cfg, ProfileName: "wave50"}, nil
	})
...
func TestWave50CockpitDeadLetterListOptionsCopiesAllFilters(t *testing.T) {
	t.Parallel()
...
func TestWave50NewDeadLetterStatusLoaderKeepsBootstrapErrorPath(t *testing.T) {
	t.Parallel()

	bootErr := errors.New("wave50 boot refused")
	loader := newDeadLetterStatusLoader(func() (*bootstrap.Result, error) {
		return nil, bootErr
	})
	...
	assert.Nil(t, bridge)
	assert.Nil(t, cleanup)
	assert.Contains(t, err.Error(), "bootstrap: wave50 boot refused")
}
