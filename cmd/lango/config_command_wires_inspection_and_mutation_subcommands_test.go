...
func TestConfigCommandWiresInspectionAndMutationSubcommands(t *testing.T) {
	t.Parallel()
...
func TestValidateInitialModeRejectsUnknownMode(t *testing.T) {
	t.Parallel()

	err := validateInitialMode(config.DefaultConfig(), "configCommandWiresInspectionAndMutationSubcommands3-missing")
	require.Error(t, err)
	assert.Contains(t, err.Error(), `unknown mode "configCommandWiresInspectionAndMutationSubcommands3-missing"`)
	assert.Contains(t, err.Error(), "valid modes can be listed via /mode")
}

func TestStartupSummaryRendersDisabledChannelAndMCPDetails(t *testing.T) {
	t.Parallel()
