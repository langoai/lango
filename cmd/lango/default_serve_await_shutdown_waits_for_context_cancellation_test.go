...
func TestDefaultServeAwaitShutdownWaitsForContextCancellation(t *testing.T) {
...
func TestWatchServeSignalsLogsStopErrorAndStillCancels(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	stopErr := errors.New("defaultServeAwaitShutdownWaitsForContextCancellation3 stop refused")
	app := &fakeServeApp{
...
func TestRootChatCommandRoutesModeThroughRunnerSeam(t *testing.T) {
	...
	runChatFn = func(mode string) error {
		gotMode = mode
		return errors.New("defaultServeAwaitShutdownWaitsForContextCancellation3 stop before TUI")
	}
	...
	assert.Contains(t, err.Error(), "defaultServeAwaitShutdownWaitsForContextCancellation3 stop before TUI")
}

func TestConfigKeysRunsThroughComposedConfigCommand(t *testing.T) {
	cmd := configCmd()
