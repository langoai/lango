...
type runChatUsesProgramSeamAndCleansUpSessionSessionStore struct {
...
func (s *runChatUsesProgramSeamAndCleansUpSessionSessionStore) Create(sess *session.Session) error {
	s.createdModes = append(s.createdModes, sess.Mode())
	return nil
}

func (s *runChatUsesProgramSeamAndCleansUpSessionSessionStore) End(key string) error {
	s.endKeys = append(s.endKeys, key)
	return nil
}

type runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore struct {
	saveCalls []runChatUsesProgramSeamAndCleansUpSessionConfigSaveCall
}

type runChatUsesProgramSeamAndCleansUpSessionConfigSaveCall struct {
	profile      string
	provider     string
	explicitKeys map[string]bool
}

func (s *runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore) Save(_ context.Context, name string, cfg *config.Config, explicitKeys map[string]bool) error {
	s.saveCalls = append(s.saveCalls, runChatUsesProgramSeamAndCleansUpSessionConfigSaveCall{
		profile:      name,
...
func (*runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore) Load(context.Context, string) (*config.Config, map[string]bool, error) {
	return config.DefaultConfig(), nil, nil
}

func (*runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore) LoadActive(context.Context) (string, *config.Config, map[string]bool, error) {
	return "runChatUsesProgramSeamAndCleansUpSession1", config.DefaultConfig(), nil, nil
}

func (*runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore) SetActive(context.Context, string) error {
	return nil
}

func (*runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore) List(context.Context) ([]configstore.ProfileInfo, error) {
	return nil, nil
}

func (*runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore) Delete(context.Context, string) error {
	return nil
}

func (*runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore) Exists(context.Context, string) (bool, error) {
	return true, nil
}

func TestRunChatUsesProgramSeamAndCleansUpSession(t *testing.T) {
	restorePrepareTuiStartupInitializesLoggingAndRedirectsStdlibLogTUIProfile(t)
	restore := replaceRunChatUsesProgramSeamAndCleansUpSessionTUIHooks(t)
	defer restore()

	cfg := config.DefaultConfig()
	cfg.DataRoot = t.TempDir()
	cfg.Modes = map[string]config.SessionMode{"focus": {Name: "focus"}}
	store := &runChatUsesProgramSeamAndCleansUpSessionSessionStore{}
	stopCalls := 0
	runCalls := 0
	chatBootLoaderFn = func() (*bootstrap.Result, error) {
		return &bootstrap.Result{Config: cfg, ProfileName: "runChatUsesProgramSeamAndCleansUpSession1-chat"}, nil
	}
...
func TestRunCockpitProgramErrorStopsAppAndReturnsTUIError(t *testing.T) {
	restorePrepareTuiStartupInitializesLoggingAndRedirectsStdlibLogTUIProfile(t)
	restore := replaceRunChatUsesProgramSeamAndCleansUpSessionTUIHooks(t)
	defer restore()
	...
	store := &runChatUsesProgramSeamAndCleansUpSessionSessionStore{}
	stopCalls := 0
	tuiErr := errors.New("runChatUsesProgramSeamAndCleansUpSession1 cockpit program stopped")
	cockpitBootLoaderFn = func() (*bootstrap.Result, error) {
		return &bootstrap.Result{
			Config:      cfg,
			ProfileName: "runChatUsesProgramSeamAndCleansUpSession1-cockpit",
			Storage:     storage.NewFacade(&runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore{}, nil),
		}, nil
	}
	...
	assert.Contains(t, err.Error(), "TUI: runChatUsesProgramSeamAndCleansUpSession1 cockpit program stopped")
	assert.Equal(t, 1, stopCalls)
...
func TestRunWorkbenchProgramErrorStopsAppAndReturnsTUIError(t *testing.T) {
	restorePrepareTuiStartupInitializesLoggingAndRedirectsStdlibLogTUIProfile(t)
	restore := replaceRunChatUsesProgramSeamAndCleansUpSessionTUIHooks(t)
	defer restore()
	...
	store := &runChatUsesProgramSeamAndCleansUpSessionSessionStore{}
	stopCalls := 0
	tuiErr := errors.New("runChatUsesProgramSeamAndCleansUpSession1 workbench program stopped")
	workbenchBootLoaderFn = func() (*bootstrap.Result, error) {
		return &bootstrap.Result{
			Config:      cfg,
			ProfileName: "runChatUsesProgramSeamAndCleansUpSession1-workbench",
			Storage:     storage.NewFacade(&runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore{}, nil),
		}, nil
	}
	...
	require.ErrorIs(t, err, tuiErr)
	assert.Contains(t, err.Error(), "TUI: runChatUsesProgramSeamAndCleansUpSession1 workbench program stopped")
	assert.Equal(t, 1, stopCalls)
...
func TestConfigGetAndSetUseCommandBootSeams(t *testing.T) {
	...
	cfg := config.DefaultConfig()
	cfg.Agent.Provider = "runChatUsesProgramSeamAndCleansUpSession1-provider"
	...
	assert.Equal(t, "runChatUsesProgramSeamAndCleansUpSession1-provider\n", getOut.String())

	profiles := &runChatUsesProgramSeamAndCleansUpSessionConfigProfileStore{}
	configBootResultFn = func() (*bootstrap.Result, error) {
		setCfg := config.DefaultConfig()
		return &bootstrap.Result{
			Config:      setCfg,
			ProfileName: "runChatUsesProgramSeamAndCleansUpSession1-profile",
			Storage:     storage.NewFacade(profiles, nil),
		}, nil
	}
	var setOut bytes.Buffer
	setCmd := configCmd()
	setCmd.SetArgs([]string{"set", "agent.provider", "runChatUsesProgramSeamAndCleansUpSession1-set"})
	setCmd.SetOut(&setOut)
	setCmd.SetErr(io.Discard)

	require.NoError(t, setCmd.Execute())
	assert.Equal(t, "Set agent.provider = runChatUsesProgramSeamAndCleansUpSession1-set\n", setOut.String())
	require.Len(t, profiles.saveCalls, 1)
	assert.Equal(t, "runChatUsesProgramSeamAndCleansUpSession1-profile", profiles.saveCalls[0].profile)
	assert.Equal(t, "runChatUsesProgramSeamAndCleansUpSession1-set", profiles.saveCalls[0].provider)
	assert.Nil(t, profiles.saveCalls[0].explicitKeys)
}

func replaceRunChatUsesProgramSeamAndCleansUpSessionTUIHooks(t *testing.T) func() {
	t.Helper()
