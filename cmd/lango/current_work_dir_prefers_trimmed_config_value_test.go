...
func TestCurrentWorkDirPrefersTrimmedConfigValue(t *testing.T) {
	t.Parallel()

	cfg := config.DefaultConfig()
	cfg.Tools.Exec.WorkDir = "  /tmp/lango-currentWorkDirPrefersTrimmedConfigValue0  "

	assert.Equal(t, "/tmp/lango-currentWorkDirPrefersTrimmedConfigValue0", currentWorkDir(cfg))
}

func TestCurrentWorkDirFallsBackToProcessDirectory(t *testing.T) {
	t.Parallel()
...
func TestTaskElapsedUsesDeterministicCompletedDurationAndRunningElapsed(t *testing.T) {
	t.Parallel()
