...
func TestWave26MainInvokesExitOnlyForNonZeroRunMain(t *testing.T) {
	...
	newRootCmdFn = func() *cobra.Command {
		cmd := &cobra.Command{RunE: func(*cobra.Command, []string) error {
			return errors.New("wave26 root failure")
		}}
		cmd.SetArgs([]string{})
...
func TestWave26RootCommandNonInteractiveShowsHelpWithoutWorkbench(t *testing.T) {
...
func TestWave26RootCommandInteractivePassesModeToWorkbench(t *testing.T) {
	origInteractive := isInteractiveFn
	origWorkbench := runWorkbenchFn
	t.Cleanup(func() {
		isInteractiveFn = origInteractive
...
func TestWave26ChatRuntimeFeaturesHandleNilAndMCPManager(t *testing.T) {
	t.Parallel()
