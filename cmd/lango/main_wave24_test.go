...
func TestWave24RunMainSandboxWorkerModeShortCircuitsOtherStartup(t *testing.T) {
...
func TestWave24RunMainFormatsExitErrors(t *testing.T) {
	origSandbox := isSandboxWorkerModeFn
	origBroker := isStorageBrokerModeFn
	origNewRoot := newRootCmdFn
	...
	newRootCmdFn = func() *cobra.Command {
		return &cobra.Command{RunE: func(*cobra.Command, []string) error {
			return cliexit.New(7, errors.New("wave24 routed exit"))
		}}
	}
	assert.Equal(t, 7, runMain())
	assert.Contains(t, stderr.String(), "wave24 routed exit")
...
func TestWave24ConfigCommandWiresProfileAndValueSubcommands(t *testing.T) {
	cmd := configCmd()
...
func TestWave24RunChatInvalidModeStopsBeforeLoggingAndAppCreation(t *testing.T) {
	...
	chatBootLoaderFn = func() (*bootstrap.Result, error) {
		return &bootstrap.Result{Config: cfg, ProfileName: "wave24"}, nil
	}
...
func TestWave24RunCockpitLoggingErrorStopsBeforeAppBuilder(t *testing.T) {
	restoreWave19TUIProfile(t)
...
func TestWave24RunWorkbenchBootstrapErrorStopsBeforeLogging(t *testing.T) {
	restoreWave19TUIProfile(t)
