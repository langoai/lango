...
type wave19RoundTripFunc func(*http.Request) (*http.Response, error)

func (f wave19RoundTripFunc) RoundTrip(req *http.Request) (*http.Response, error) {
	return f(req)
}

func restoreWave19TUIProfile(t *testing.T) {
	t.Helper()
	t.Cleanup(func() { tui.SetProfile("default") })
}

func wave19AppMode(t *testing.T, opt app.AppOption) app.AppMode {
	t.Helper()
...
func TestPrepareTUIStartupInitializesLoggingAndRedirectsStdlibLog(t *testing.T) {
	...
	log.Print("wave19 stdlib log")
	cleanup()

	logPath := filepath.Join(cfg.DataRoot, "wave19.log")
	...
	assert.Contains(t, notice.String(), "Initializing wave19...")

	logBytes, readErr := os.ReadFile(logPath)
	require.NoError(t, readErr)
	assert.Contains(t, string(logBytes), "wave19 stdlib log")
}
...
func TestHealthCmdUsesInjectedHTTPClientAndPort(t *testing.T) {
	...
	newHealthHTTPClientFn = func() *http.Client {
		return &http.Client{Transport: wave19RoundTripFunc(func(req *http.Request) (*http.Response, error) {
...
func TestServeCmdPropagatesSetupErrorsBeforeStartingApplication(t *testing.T) {
	...
	cfg := config.DefaultConfig()
	serveBootLoaderFn = func() (*bootstrap.Result, error) {
		return &bootstrap.Result{Config: cfg, ProfileName: "wave19"}, nil
	}
