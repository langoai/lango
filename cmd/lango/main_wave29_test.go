...
func TestWave29ConfigSetLocalValidationStopsBeforeBootstrap(t *testing.T) {
	for _, tt := range []struct {
		...
		},
		{
			...
			args:    []string{"set", "--from-env", "LANGO_WAVE29_VALUE"},
			wantErr: "accepts 1 arg with --from-env: <dot.path>",
		},
		{
			name:    "from-env fails before bootstrap when env is missing",
			args:    []string{"set", "agent.provider", "--from-env", "LANGO_WAVE29_MISSING"},
			wantErr: `environment variable "LANGO_WAVE29_MISSING" is not set`,
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("LANGO_WAVE29_MISSING", "")
			require.NoError(t, os.Unsetenv("LANGO_WAVE29_MISSING"))
...
func TestWave29ConfigKeysCommandWritesStaticCatalogWithoutBootstrap(t *testing.T) {
	cmd := configCmd()
