...
| `cli/cockpit/` | `lango` / `lango cockpit` multi-panel TUI surface. Owns Mission Control, shared pending approval ownership, activity buffers, compatibility learning-buffer fallback, transient proposal rendering, durable-first mission projection, deterministic agenda/loop projection, and the sidebar/detail-page shell around the shared chat model. In the current loop slice, dead-letter and cron loops are projected as operator-global rows, while mission, inquiry, and follow-up loops remain session-scoped |
