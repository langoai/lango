...
| `app/` | Application bootstrap and wiring. `app.go` defines `New()` (component initialization), `Start()`, and `Stop()`. Wiring is split across domain-specific files (`wiring_*.go`) and module files (`modules_*.go`) that create subsystems such as knowledge, memory, graph, MCP, P2P, payment, smart account, economy, observability, automation, durable missions, and transient proposals. `types.go` defines the `App` struct with all component fields, including durable mission store/service handles plus transient proposal registry/service handles. The app layer also owns mission-aware adapters that connect approval, background, and RunLedger execution flows to the mission lifecycle without pushing mission imports downward |
...
| `cli/cockpit/` | `lango` / `lango cockpit` multi-panel TUI surface. Owns Mission Control, shared pending approval ownership, activity buffers, compatibility learning-buffer fallback, transient proposal rendering, durable-first mission projection, and the sidebar/detail-page shell around the shared chat model |
