...
16. `dead-letter browsing / status observation` slice now includes local and transaction-global family grouping, compact per-submission breakdown, a thin raw background-task bridge on the detail view, a cockpit read surface, a thin cockpit filter bar, and a first replay/write control with confirm/refresh UX; the follow-on work is richer cockpit filters, richer loading/failure recovery feedback, and higher-level CLI surfaces
