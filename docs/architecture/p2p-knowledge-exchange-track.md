...
16. `dead-letter browsing / status observation` slice now includes local and transaction-global family grouping, compact per-submission breakdown, a thin raw background-task bridge on the detail view, and a cockpit read surface; the follow-on work is replay/write controls, richer cockpit filtering, and higher-level CLI surfaces
