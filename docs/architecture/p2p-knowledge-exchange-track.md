...
16. `dead-letter browsing / status observation` slice now includes local and transaction-global family grouping, compact per-submission breakdown, and a thin raw background-task bridge on the detail view; the follow-on work is higher-level cockpit or CLI surfaces
