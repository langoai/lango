...
16. `dead-letter browsing / status observation` slice now includes local and transaction-global family grouping, compact per-submission breakdown, a thin raw background-task bridge on the detail view, a cockpit read surface, a thin cockpit filter bar with subtype, latest-family, any-match-family, and actor/time filtering, and a first replay/write control with confirm/refresh plus running/failure feedback UX; the follow-on work is richer cockpit filters beyond latest/any-match family and higher-level CLI surfaces
