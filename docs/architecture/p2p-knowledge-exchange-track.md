...
16. `dead-letter browsing / status observation` slice now includes local and transaction-global family grouping, compact per-submission breakdown, a thin raw background-task bridge on the detail view, a cockpit read surface, and a thin cockpit filter bar; the follow-on work is replay/write controls, richer cockpit filters, and higher-level CLI surfaces
