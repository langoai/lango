...
func TestWave22NewImporterDefaults(t *testing.T) {
	t.Parallel()
...
func TestWave22ImportFromRepoGitFallbackUsesHTTPWithoutLiveNetwork(t *testing.T) {
	const skillBody = `---
name: wave22-http-fallback
...
Fallback content.`
	...
	assert.Equal(t, []string{"wave22-http-fallback"}, result.Imported)
	assert.Empty(t, result.Skipped)
	assert.Empty(t, result.Errors)
	assert.Equal(t, []string{"wave22-http-fallback"}, store.savedNames())
}

func TestWave22ImportViaGitCopiesFixtureAndHandlesBranches(t *testing.T) {
	fixture := t.TempDir()
	writeSkillFixture(t, filepath.Join(fixture, "packs", "valid"), "wave22-git-valid", "Valid git import.")
	writeSkillFixture(t, filepath.Join(fixture, "packs", "existing"), "wave22-existing", "Existing git import.")
...
Invalid.`)
	...
	store := newRecordingSkillStore()
	store.existing["wave22-existing"] = &SkillEntry{Name: "wave22-existing"}
	ref := &GitHubRef{Owner: "owner", Repo: "repo", Branch: "wave22-branch", Path: "packs"}

	result, err := im.importViaGit(context.Background(), ref, store, ImportConfig{})
	require.NoError(t, err)
	assertGitCloneArgs(t, logPath, "--depth=1 --branch wave22-branch https://github.com/owner/repo.git")

	assert.Equal(t, []string{"wave22-git-valid"}, result.Imported)
	assert.Equal(t, []string{"wave22-existing"}, result.Skipped)
	require.Len(t, result.Errors, 1)
	assert.Contains(t, result.Errors[0], "invalid: parse:")
	assert.Equal(t, []string{"wave22-git-valid"}, store.savedNames())
	assert.Equal(t, map[string]string{
		"wave22-git-valid/scripts/setup.sh": "echo setup",
	}, store.savedResources())
}

func TestWave22ImportViaGitReadClonedDirFailure(t *testing.T) {
	fixture := t.TempDir()
...
func TestWave22ImportSingleWithResourcesHTTPBranches(t *testing.T) {
	const skillBody = `---
name: wave22-single-http
...
Single content.`
	...
	assert.Equal(t, "wave22-single-http", entry.Name)
	assert.Equal(t, "https://github.com/owner/repo", entry.Source)
	assert.Equal(t, []string{"wave22-single-http"}, store.savedNames())
	assert.Equal(t, map[string]string{
		"wave22-single-http/scripts/run.sh": "echo run",
	}, store.savedResources())
}

func TestWave22ImportSingleViaGitSuccessAndFallbackFailure(t *testing.T) {
	t.Run("success from cloned fixture", func(t *testing.T) {
		fixture := t.TempDir()
		writeSkillFixture(t, filepath.Join(fixture, "skills", "single"), "wave22-single-git", "Single git import.")
		...
		im := NewImporterWithClient(http.DefaultClient, zap.NewNop().Sugar())
		store := newRecordingSkillStore()
		ref := &GitHubRef{Owner: "owner", Repo: "repo", Branch: "wave22-single-branch", Path: "skills"}

		entry, err := im.ImportSingleWithResources(context.Background(), ref, "single", store)
		require.NoError(t, err)
		assertGitCloneArgs(t, logPath, "--depth=1 --branch wave22-single-branch https://github.com/owner/repo.git")

		assert.Equal(t, "wave22-single-git", entry.Name)
		assert.Equal(t, []string{"wave22-single-git"}, store.savedNames())
		assert.Equal(t, map[string]string{
			"wave22-single-git/assets/note.txt": "asset",
		}, store.savedResources())
	})
...
func TestWave22CloneRepoGitFailureIncludesOutput(t *testing.T) {
	installFakeGit(t, "fail", "")
	...
	assert.Contains(t, err.Error(), "wave22 fake git failure")
}
...
printf '%%s\n' "$*" >> "$log"
if [ "$mode" = "fail" ]; then
  echo "wave22 fake git failure" >&2
  exit 42
fi
dest="${@: -1}"
mkdir -p "$dest"
