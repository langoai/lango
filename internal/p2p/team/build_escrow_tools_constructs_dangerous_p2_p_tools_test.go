...
type buildEscrowToolsConstructsDangerousP2PToolsSettler struct {
	lockErr    error
	releaseErr error
	locked     []*big.Int
	released   []*big.Int
...
func (s *buildEscrowToolsConstructsDangerousP2PToolsSettler) Lock(_ context.Context, _ string, amount *big.Int) error {
	if s.lockErr != nil {
		return s.lockErr
	}
...
func (s *buildEscrowToolsConstructsDangerousP2PToolsSettler) Release(_ context.Context, _ string, amount *big.Int) error {
	if s.releaseErr != nil {
		return s.releaseErr
	}
...
func (s *buildEscrowToolsConstructsDangerousP2PToolsSettler) Refund(context.Context, string, *big.Int) error {
	return nil
}

func TestBuildEscrowToolsConstructsDangerousP2PTools(t *testing.T) {
	t.Parallel()

	coord, _ := setupCoordinator(t)
	tools := BuildEscrowTools(coord, buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(&buildEscrowToolsConstructsDangerousP2PToolsSettler{}), nil)
...
func TestTeamFormWithBudgetCreatesEscrowAndBudget(t *testing.T) {
	t.Parallel()

	coord, _ := setupCoordinator(t)
	settler := &buildEscrowToolsConstructsDangerousP2PToolsSettler{}
	escrowEngine := buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(settler)
...
func TestTeamFormWithBudgetAutoSplitsAcrossWorkers(t *testing.T) {
	t.Parallel()

	coord, _ := setupCoordinator(t)
	escrowEngine := buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(&buildEscrowToolsConstructsDangerousP2PToolsSettler{})
	budgetEngine, err := budget.NewEngine(budget.NewStore(), config.BudgetConfig{})
	require.NoError(t, err)
	tool := findToolByName(BuildEscrowTools(coord, escrowEngine, budgetEngine), "team_form_with_budget")
	require.NotNil(t, tool)
...
func TestTeamFormWithBudgetWrapsTeamAndEscrowErrors(t *testing.T) {
	t.Parallel()

	coord, _ := setupCoordinator(t)
	tool := findToolByName(BuildEscrowTools(coord, buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(&buildEscrowToolsConstructsDangerousP2PToolsSettler{}), nil), "team_form_with_budget")
	require.NotNil(t, tool)
...
func TestTeamCompleteMilestoneReleasesWhenAllMilestonesComplete(t *testing.T) {
	t.Parallel()

	settler := &buildEscrowToolsConstructsDangerousP2PToolsSettler{}
	escrowEngine := buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(settler)
	entry := buildEscrowToolsConstructsDangerousP2PToolsActiveEscrow(t, escrowEngine)
	coord, _ := setupCoordinator(t)
...
func TestTeamCompleteMilestoneReportsReleaseError(t *testing.T) {
	t.Parallel()

	settler := &buildEscrowToolsConstructsDangerousP2PToolsSettler{releaseErr: errors.New("release unavailable")}
	escrowEngine := buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(settler)
	entry := buildEscrowToolsConstructsDangerousP2PToolsActiveEscrow(t, escrowEngine)
	coord, _ := setupCoordinator(t)
	tool := findToolByName(BuildEscrowTools(coord, escrowEngine, nil), "team_complete_milestone")
...
func TestTeamCompleteMilestoneWrapsEngineError(t *testing.T) {
	t.Parallel()

	coord, _ := setupCoordinator(t)
	tool := findToolByName(BuildEscrowTools(coord, buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(&buildEscrowToolsConstructsDangerousP2PToolsSettler{}), nil), "team_complete_milestone")
	require.NotNil(t, tool)
...
func buildEscrowToolsConstructsDangerousP2PToolsEscrowEngine(settler *buildEscrowToolsConstructsDangerousP2PToolsSettler) *escrow.Engine {
	cfg := escrow.DefaultEngineConfig()
	cfg.AutoRelease = false
	return escrow.NewEngine(escrow.NewMemoryStore(), settler, cfg)
}

func buildEscrowToolsConstructsDangerousP2PToolsActiveEscrow(t *testing.T, engine *escrow.Engine) *escrow.EscrowEntry {
	t.Helper()
	...
	entry, err := engine.Create(ctx, escrow.CreateRequest{
		...
		Reason:    "initSecurityDeterministicDisabledAndErrorBranches4",
		TaskID:    "task-initSecurityDeterministicDisabledAndErrorBranches4",
