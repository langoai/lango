...
func (g *wave10PayGate) Check(peerDID, toolName string, payload map[string]interface{}) (PayGateResult, error) {
	g.calls++
	g.seen = append(g.seen, peerDID+":"+toolName)
	return g.result, g.err
}

type wave10SecurityEvents struct {
	successes []string
	failures  []string
}

func (e *wave10SecurityEvents) RecordToolFailure(peerDID string) {
	e.failures = append(e.failures, peerDID)
}

func (e *wave10SecurityEvents) RecordToolSuccess(peerDID string) {
	e.successes = append(e.successes, peerDID)
}

type wave10OntologyHandler struct {
...
func (h *wave10OntologyHandler) HandleSchemaQuery(_ context.Context, peerDID string, req SchemaQueryRequest) (*SchemaQueryResponse, error) {
	h.peerSeen = peerDID
	h.querySeen = req
	return h.queryResp, h.err
}

func (h *wave10OntologyHandler) HandleSchemaPropose(_ context.Context, peerDID string, req SchemaProposeRequest) (*SchemaProposeResponse, error) {
	h.peerSeen = peerDID
	h.propSeen = req
	return h.proposeResp, h.err
}

type wave10ProtocolStream struct {
	rw     io.ReadWriter
	closed bool
}

func (s *wave10ProtocolStream) Read(p []byte) (int, error)  { return s.rw.Read(p) }
func (s *wave10ProtocolStream) Write(p []byte) (int, error) { return s.rw.Write(p) }
func (s *wave10ProtocolStream) Close() error                { s.closed = true; return nil }
func (s *wave10ProtocolStream) CloseWrite() error           { return nil }
func (s *wave10ProtocolStream) CloseRead() error            { return nil }
func (s *wave10ProtocolStream) Reset() error                { s.closed = true; return nil }
func (s *wave10ProtocolStream) ResetWithError(network.StreamErrorCode) error {
	s.closed = true
	return nil
}
func (s *wave10ProtocolStream) SetDeadline(time.Time) error      { return nil }
func (s *wave10ProtocolStream) SetReadDeadline(time.Time) error  { return nil }
func (s *wave10ProtocolStream) SetWriteDeadline(time.Time) error { return nil }
func (s *wave10ProtocolStream) ID() string                       { return "wave10-protocol-stream" }
func (s *wave10ProtocolStream) Protocol() protocol.ID            { return ProtocolID }
func (s *wave10ProtocolStream) SetProtocol(protocol.ID) error    { return nil }
func (s *wave10ProtocolStream) Stat() network.Stats              { return network.Stats{} }
func (s *wave10ProtocolStream) Conn() network.Conn               { return &wave10ProtocolConn{} }
func (s *wave10ProtocolStream) Scope() network.StreamScope       { return &network.NullScope{} }

type wave10ProtocolConn struct{}

func (c *wave10ProtocolConn) Close() error                               { return nil }
func (c *wave10ProtocolConn) CloseWithError(network.ConnErrorCode) error { return nil }
func (c *wave10ProtocolConn) ID() string                                 { return "wave10-protocol-conn" }
func (c *wave10ProtocolConn) NewStream(context.Context) (network.Stream, error) {
	return nil, errors.New("not implemented")
}
func (c *wave10ProtocolConn) GetStreams() []network.Stream       { return nil }
func (c *wave10ProtocolConn) IsClosed() bool                     { return false }
func (c *wave10ProtocolConn) As(any) bool                        { return false }
func (c *wave10ProtocolConn) LocalPeer() peer.ID                 { return peer.ID("wave10-local") }
func (c *wave10ProtocolConn) RemotePeer() peer.ID                { return peer.ID("wave10-remote") }
func (c *wave10ProtocolConn) RemotePublicKey() crypto.PubKey     { return nil }
func (c *wave10ProtocolConn) ConnState() network.ConnectionState { return network.ConnectionState{} }
func (c *wave10ProtocolConn) LocalMultiaddr() ma.Multiaddr {
	return ma.StringCast("/ip4/127.0.0.1/tcp/11001")
}
func (c *wave10ProtocolConn) RemoteMultiaddr() ma.Multiaddr {
	return ma.StringCast("/ip4/127.0.0.1/tcp/11002")
}
func (c *wave10ProtocolConn) Stat() network.ConnStats  { return network.ConnStats{} }
func (c *wave10ProtocolConn) Scope() network.ConnScope { return &network.NullScope{} }

type wave10SplitReadWriter struct {
	reader io.Reader
	writer io.Writer
}

func (rw *wave10SplitReadWriter) Read(p []byte) (int, error) {
	return rw.reader.Read(p)
}

func (rw *wave10SplitReadWriter) Write(p []byte) (int, error) {
	return rw.writer.Write(p)
}

func wave10Handler(t *testing.T, peerDID string) (*Handler, *handshake.SessionStore, string) {
	t.Helper()
...
func TestWave10SettersAndBasicDispatchBranches(t *testing.T) {
	t.Parallel()

	peerDID := "did:key:wave10-setters"
	h, _, token := wave10Handler(t, peerDID)
	...
	events := &wave10SecurityEvents{}
	bus := eventbus.New()
	payGate := &wave10PayGate{result: PayGateResult{Status: payGateStatusFree}}
	...
	ontology := &wave10OntologyHandler{}
	...
	h.cardFn = func() map[string]interface{} {
		return map[string]interface{}{"name": "wave10-agent", "capabilities": []interface{}{"search"}}
	}
	...
	assert.Equal(t, "wave10-agent", card.Result["name"])

	unknown := h.handleRequest(context.Background(), nil, &Request{Type: RequestType("unknown_wave10"), SessionToken: token, RequestID: "unknown"})
	assert.Equal(t, ResponseStatusError, unknown.Status)
	assert.Contains(t, unknown.Error, "unknown request type")
}

func TestWave10PriceQueryBranches(t *testing.T) {
	t.Parallel()

	peerDID := "did:key:wave10-price"
	h, _, token := wave10Handler(t, peerDID)
	...
	h.SetPayGate(&wave10PayGate{err: errors.New("meter down")})
	...
	h.SetPayGate(&wave10PayGate{result: PayGateResult{Status: payGateStatusFree}})
	...
	h.SetPayGate(&wave10PayGate{result: PayGateResult{Status: payGateStatusPaymentRequired, PriceQuote: quote}})
...
func TestWave10SafetyGateBlocksBeforeApprovalPaymentAndExecution(t *testing.T) {
	t.Parallel()

	peerDID := "did:key:wave10-safety"
	h, _, token := wave10Handler(t, peerDID)
	payGate := &wave10PayGate{result: PayGateResult{Status: payGateStatusVerified, Auth: "auth"}}
	approvalCalls := 0
	execCalls := 0
...
func TestWave10PaidInvokePaymentBranchesAndEvents(t *testing.T) {
	t.Parallel()

	peerDID := "did:key:wave10-paid"
	h, _, token := wave10Handler(t, peerDID)
	...
	h.SetPayGate(&wave10PayGate{result: PayGateResult{Status: payGateStatusInvalid}})
	...
	h.firewall.SetZKAttestFunc(func(responseHash, agentDIDHash []byte) (*firewall.AttestationResult, error) {
		...
		return &firewall.AttestationResult{
			...
			CircuitID:    "wave10-circuit",
			Scheme:       "test",
		}, nil
	})

	h.SetPayGate(&wave10PayGate{result: PayGateResult{Status: payGateStatusVerified, Auth: "verified-auth"}})
	...
	h.SetPayGate(&wave10PayGate{result: PayGateResult{Status: payGateStatusPostPayApproved, SettlementID: "settlement-1"}})
...
func TestWave10SchemaAndTeamBranches(t *testing.T) {
	t.Parallel()

	peerDID := "did:key:wave10-schema-team"
	h, _, token := wave10Handler(t, peerDID)
	...
	h.SetOntologyHandler(&wave10OntologyHandler{err: errors.New("schema backend unavailable")})
	...
	h.SetTeamHandler(func(_ context.Context, gotPeer string, gotType RequestType, payload map[string]interface{}) (map[string]interface{}, error) {
		...
		assert.Equal(t, "wave10-team", payload["teamId"])
	...
	teamOK := h.handleRequest(context.Background(), nil, &Request{
		...
		SessionToken: token,
		RequestID:    "team-ok",
		Payload:      map[string]interface{}{"teamId": "wave10-team"},
	})
...
func TestWave10StreamHandlerDecodeErrorAndSendRequest(t *testing.T) {
	t.Parallel()

	h, _, _ := wave10Handler(t, "did:key:wave10-stream")
	var invalidIO bytes.Buffer
	invalidIO.WriteString("{not-json")
	stream := &wave10ProtocolStream{rw: &invalidIO}
	...
	sendResp, err := SendRequest(context.Background(), &wave10ProtocolStream{
		rw: &wave10SplitReadWriter{reader: &responseBytes, writer: &sentBytes},
