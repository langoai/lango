...
type wave45CompileErrorCircuit struct{}

func (*wave45CompileErrorCircuit) Define(frontend.API) error {
	return errors.New("define failed")
}

type wave45BadWitnessCircuit struct {
	Public frontend.Variable `gnark:",public"`
}

func (*wave45BadWitnessCircuit) Define(frontend.API) error {
	return nil
}

func newWave45Service(scheme ProofScheme) *ProverService {
...
func TestWave45ProofSchemeAndSRSModeValidation(t *testing.T) {
...
func TestWave45NewProverServiceDefaultsAndCacheDirErrors(t *testing.T) {
...
func TestWave45CompileRejectsUnsupportedSchemeAndWrapsCircuitErrors(t *testing.T) {
	circuit, _ := validOwnershipAssignment()

	unsupported := newWave45Service(ProofScheme("snark"))
	...
	broken := newWave45Service(SchemeGroth16)
	err = broken.Compile("broken", &wave45CompileErrorCircuit{})
	require.Error(t, err)
...
func TestWave45ProveRejectsInvalidStateBeforeBackendProvers(t *testing.T) {
	_, assignment := validOwnershipAssignment()

	t.Run("witness error", func(t *testing.T) {
		svc := newWave45Service(SchemePlonk)
		svc.compiled["ownership"] = &CompiledCircuit{}

		proof, err := svc.Prove(context.Background(), "ownership", &wave45BadWitnessCircuit{
			Public: struct{}{},
		})
		require.Error(t, err)
		assert.Nil(t, proof)
	...
	t.Run("invalid groth16 proving key", func(t *testing.T) {
		svc := newWave45Service(SchemeGroth16)
	...
	t.Run("unsupported scheme", func(t *testing.T) {
		svc := newWave45Service(ProofScheme("snark"))
		svc.compiled["ownership"] = &CompiledCircuit{}
...
func TestWave45VerifyRejectsInvalidStateBeforeBackendVerification(t *testing.T) {
	_, assignment := validOwnershipAssignment()
...
func TestWave45VerifyReportsMalformedProofDeserialization(t *testing.T) {
	_, assignment := validOwnershipAssignment()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			svc := newWave45Service(tt.scheme)
...
func TestWave45ExportGroth16VerifierWrapsCompileErrors(t *testing.T) {
	svc := newWave45Service(SchemePlonk)
	err := svc.ExportGroth16Verifier("broken", &wave45CompileErrorCircuit{}, os.Stdout)
	require.Error(t, err)
	assert.Contains(t, err.Error(), `compile circuit "broken" for groth16`)
	assert.Contains(t, err.Error(), "define failed")
}

func TestWave45LoadSRSFromFileReportsMissingFile(t *testing.T) {
