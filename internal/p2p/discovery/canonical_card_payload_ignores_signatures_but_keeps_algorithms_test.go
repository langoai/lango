...
type canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsCardSigner struct {
	alg     string
	sig     []byte
	err     error
	payload []byte
...
func (s *canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsCardSigner) Sign(_ context.Context, payload []byte) ([]byte, error) {
...
func (s *canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsCardSigner) Algorithm() string {
	return s.alg
}

type canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsPQSigner struct {
	alg     string
	pub     []byte
	sig     []byte
	err     error
...
func (s *canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsPQSigner) SignPQ(_ context.Context, payload []byte) ([]byte, error) {
	s.payload = append([]byte(nil), payload...)
	if s.err != nil {
		return nil, s.err
	}
...
func (s *canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsPQSigner) PQAlgorithm() string {
	return s.alg
}

func (s *canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsPQSigner) PQPublicKey() []byte {
	return append([]byte(nil), s.pub...)
}

func TestCanonicalCardPayloadIgnoresSignaturesButKeepsAlgorithms(t *testing.T) {
	t.Parallel()
...
func TestVerifyCardSignatureUsesCanonicalPayloadAndBundleKey(t *testing.T) {
	t.Parallel()

	bundle := canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsIdentityBundle()
...
func TestVerifyCardSignatureRejectsInvalidSignedCards(t *testing.T) {
	t.Parallel()
...
func TestVerifyCardSignatureAllowsUnsignedLegacyCard(t *testing.T) {
	t.Parallel()
...
func TestGossipServiceHandleMessageFiltersInvalidInputs(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "missing DID",
			data: canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t, GossipCard{Name: "missing-did"}),
		},
		{
			name: "revoked DID",
			...
			data: canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t, GossipCard{
		...
		},
		{
			...
			data: canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsSignedCardJSON(t),
		},
	}
...
func TestGossipServiceHandleMessageCredentialAndTimestampBranches(t *testing.T) {
	t.Parallel()
	...
	gs.handleMessage(&pubsub.Message{Message: &pb.Message{Data: canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t, staleButAccepted)}})
	...
	gs.handleMessage(&pubsub.Message{Message: &pb.Message{Data: canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t, older)}})
	assert.Equal(t, "stale", gs.FindByDID("did:lango:cred").Name)

	newer := staleButAccepted
	newer.Name = "newer"
	newer.Timestamp = now.Add(time.Minute)
	gs.handleMessage(&pubsub.Message{Message: &pb.Message{Data: canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t, newer)}})
	...
	gs.handleMessage(&pubsub.Message{Message: &pb.Message{Data: canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t, invalid)}})
	assert.Nil(t, gs.FindByDID("did:lango:invalid-cred"))
	assert.Equal(t, 1, verifierCalls)
}

func TestGossipServiceSignCardBranches(t *testing.T) {
	t.Parallel()
	...
	t.Run("classical and PQ signatures use the same canonical payload", func(t *testing.T) {
		...
		signer := &canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsCardSigner{alg: "ed25519", sig: []byte("classical-signature")}
		pqSigner := &canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsPQSigner{alg: "mldsa65", pub: []byte("pq-public"), sig: []byte("pq-signature")}
		gs := newTestGossipServiceFields()
	...
	t.Run("classical signing error skips signatures", func(t *testing.T) {
		...
		gs := newTestGossipServiceFields()
		gs.cardSigner = &canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsCardSigner{alg: "ed25519", err: errors.New("sign failed")}
...
func canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsIdentityBundle() *identity.IdentityBundle {
...
func canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsSignedCardJSON(t *testing.T) []byte {
	t.Helper()
	...
	bundleJSON, err := json.Marshal(bundle)
	require.NoError(t, err)
	return canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t, GossipCard{
		DID:       did,
		Bundle:    bundleJSON,
		Signature: []byte("signature"),
		Timestamp: time.Unix(1700000000, 0).UTC(),
...
func canonicalCardPayloadIgnoresSignaturesButKeepsAlgorithmsMarshalCard(t *testing.T, card GossipCard) []byte {
	t.Helper()
