...
func TestPeerIDFromStringParsesValidAndRejectsInvalidInput(t *testing.T) {
	t.Parallel()
...
func TestGossipServiceHandleMessageStoresUnsignedNewerCardsOnly(t *testing.T) {
	t.Parallel()

	gs := newTestGossipServiceFields()
	older := GossipCard{
		DID:          "did:lango:networkModuleMetadataAndEnablementBranches5",
		Name:         "older",
	...
	gs.handleMessage(peerIdFromStringParsesValidAndRejectsInvalidInputPubSubMessage(t, newer))
	require.Equal(t, "newer", gs.FindByDID("did:lango:networkModuleMetadataAndEnablementBranches5").Name)

	gs.handleMessage(peerIdFromStringParsesValidAndRejectsInvalidInputPubSubMessage(t, older))
	require.Equal(t, "newer", gs.FindByDID("did:lango:networkModuleMetadataAndEnablementBranches5").Name)
	require.Len(t, gs.FindByCapability("search"), 1)
	require.Len(t, gs.KnownPeers(), 1)
}

func TestGossipServiceHandleMessageValidationRejectsVerifierErrorsAndMalformedSignedBundles(t *testing.T) {
	t.Parallel()
	...
	t.Run("credential verifier error discards card", func(t *testing.T) {
		...
		gs.handleMessage(peerIdFromStringParsesValidAndRejectsInvalidInputPubSubMessage(t, GossipCard{
...
func TestGossipServicePublishLoopCanceledContextReturnsWithoutHostOrTopic(t *testing.T) {
	t.Parallel()
...
func peerIdFromStringParsesValidAndRejectsInvalidInputPubSubMessage(t *testing.T, card GossipCard) *pubsub.Message {
	t.Helper()
