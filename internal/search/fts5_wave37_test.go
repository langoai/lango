...
func openWave37SearchDB(t *testing.T) *sql.DB {
	t.Helper()
...
func requireWave37FTS5(t *testing.T, db *sql.DB) {
	t.Helper()
...
func TestFTS5IndexWave37_TableLifecycleAndDBAccessor(t *testing.T) {
	db := openWave37SearchDB(t)
	requireWave37FTS5(t, db)

	idx := NewFTS5Index(db, "wave37_lifecycle_fts", []string{"title", "body"})
...
func TestFTS5IndexWave37_DeterministicFallbackBranches(t *testing.T) {
	db := openWave37SearchDB(t)
	idx := NewFTS5Index(db, "wave37_no_extension_fts", []string{"title", "body"})
...
func TestFTS5IndexWave37_ExecerBranchesWithoutFTS5Extension(t *testing.T) {
	ctx := context.Background()
	idx := NewFTS5Index(nil, "wave37_exec_fts", []string{"title", "body"})
	ex := &wave37ExecRecorder{}

	require.NoError(t, idx.InsertWithExec(ctx, ex, "doc-1", []string{"title", "body"}))
	require.Len(t, ex.calls, 1)
	assert.Equal(t, `INSERT INTO wave37_exec_fts(source_id, title, body) VALUES(?, ?, ?)`, ex.calls[0].query)
	...
	assert.Equal(t, `DELETE FROM wave37_exec_fts WHERE source_id = ?`, ex.calls[1].query)
	...
	assert.Equal(t, `DELETE FROM wave37_exec_fts WHERE source_id = ?`, ex.calls[2].query)
	assert.Equal(t, `INSERT INTO wave37_exec_fts(source_id, title, body) VALUES(?, ?, ?)`, ex.calls[3].query)

	deleteErr := errors.New("delete failed")
	ex = &wave37ExecRecorder{err: deleteErr}
	err := idx.UpdateWithExec(ctx, ex, "doc-3", []string{"ignored", "body"})
	require.ErrorIs(t, err, deleteErr)
	assert.ErrorContains(t, err, "FTS5 delete from wave37_exec_fts")
	require.Len(t, ex.calls, 1)

	insertErr := errors.New("insert failed")
	ex = &wave37ExecRecorder{errOnCall: 2, err: insertErr}
	err = idx.UpdateWithExec(ctx, ex, "doc-4", []string{"new", "body"})
	require.ErrorIs(t, err, insertErr)
	assert.ErrorContains(t, err, "FTS5 insert into wave37_exec_fts")
	require.Len(t, ex.calls, 2)
}

func TestFTS5IndexWave37_DirectSQLBranchesWithoutFTS5Extension(t *testing.T) {
	db := openWave37SearchDB(t)
	ctx := context.Background()
	_, err := db.Exec(`CREATE TABLE wave37_sql_fts(source_id TEXT PRIMARY KEY, title TEXT, body TEXT)`)
	require.NoError(t, err)
	idx := NewFTS5Index(db, "wave37_sql_fts", []string{"title", "body"})

	require.NoError(t, idx.Insert(ctx, "doc-1", []string{"alpha", "first body"}))
	var title string
	err = db.QueryRow(`SELECT title FROM wave37_sql_fts WHERE source_id = ?`, "doc-1").Scan(&title)
	require.NoError(t, err)
	...
	var count int
	err = db.QueryRow(`SELECT count(*) FROM wave37_sql_fts`).Scan(&count)
	require.NoError(t, err)
	...
	require.Error(t, err)
	assert.ErrorContains(t, err, "FTS5 search wave37_sql_fts")
}

func TestFTS5IndexWave37_InsertUpdateDeleteWithTransactionExec(t *testing.T) {
	db := openWave37SearchDB(t)
	requireWave37FTS5(t, db)

	ctx := context.Background()
	idx := NewFTS5Index(db, "wave37_tx_fts", []string{"title", "body"})
	require.NoError(t, idx.EnsureTable())
...
func TestFTS5IndexWave37_DirectInsertUpdateDeleteAndSearchLimits(t *testing.T) {
	db := openWave37SearchDB(t)
	requireWave37FTS5(t, db)

	ctx := context.Background()
	idx := NewFTS5Index(db, "wave37_direct_fts", []string{"title", "body"})
	require.NoError(t, idx.EnsureTable())
...
func TestFTS5IndexWave37_BulkInsertSuccessAndRollbackOnRowError(t *testing.T) {
	db := openWave37SearchDB(t)
	requireWave37FTS5(t, db)

	ctx := context.Background()
	idx := NewFTS5Index(db, "wave37_bulk_fts", []string{"title", "body"})
	require.NoError(t, idx.EnsureTable())
...
func TestFTS5IndexWave37_ErrorBranchesWrapOperationContext(t *testing.T) {
	db := openWave37SearchDB(t)
	requireWave37FTS5(t, db)

	ctx := context.Background()
	idx := NewFTS5Index(db, "wave37_errors_fts", []string{"title", "body"})

	_, err := idx.Search(ctx, "anything", 10)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "FTS5 search wave37_errors_fts")
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "FTS5 insert into wave37_errors_fts")
}

func TestFTS5IndexWave37_SearchEscapesProblematicQueries(t *testing.T) {
	db := openWave37SearchDB(t)
	requireWave37FTS5(t, db)

	ctx := context.Background()
	idx := NewFTS5Index(db, "wave37_escape_fts", []string{"title", "body"})
	require.NoError(t, idx.EnsureTable())
	require.NoError(t, idx.BulkInsert(ctx, []Record{
...
func TestSanitizeFTS5QueryWave37_Branches(t *testing.T) {
	tests := []struct {
		name string
		give string
		want string
...
type wave37ExecCall struct {
	query string
	args  []any
}

type wave37ExecRecorder struct {
	calls     []wave37ExecCall
	errOnCall int
	err       error
}

func (e *wave37ExecRecorder) ExecContext(_ context.Context, query string, args ...any) (sql.Result, error) {
	e.calls = append(e.calls, wave37ExecCall{
		query: query,
	...
	return wave37SQLResult(1), nil
}

type wave37SQLResult int64

func (r wave37SQLResult) LastInsertId() (int64, error) { return int64(r), nil }
func (r wave37SQLResult) RowsAffected() (int64, error) { return int64(r), nil }

func TestFTS5IndexWave37_UsesTempFileSQLite(t *testing.T) {
	db := openWave37SearchDB(t)
	requireWave37FTS5(t, db)
