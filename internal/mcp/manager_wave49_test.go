...
func TestServerManagerWave49_ConnectAllRegistersEnabledServersAndReturnsErrors(t *testing.T) {
	t.Parallel()
...
func TestServerManagerWave49_SettersPropagateToExistingAndFutureConnections(t *testing.T) {
	t.Parallel()
	...
	iso := &recordingIsolator{name: "wave49-sandbox"}
	bus := eventbus.New()
	workspace := t.TempDir()
	dataRoot := t.TempDir()
...
func TestServerManagerWave49_AggregatesCapabilitiesAndStatusFromRegisteredConnections(t *testing.T) {
	t.Parallel()
	...
	require.ElementsMatch(t, []string{"alpha/tool-a", "beta/tool-b"}, wave49ToolNames(mgr.AllTools()))
	require.ElementsMatch(t, []string{"alpha/resource-a", "beta/resource-b"},
		wave49ResourceNames(mgr.AllResources()))
	require.ElementsMatch(t, []string{"alpha/prompt-a", "beta/prompt-b"},
		wave49PromptNames(mgr.AllPrompts()))
...
func TestServerManagerWave49_AggregateResultsAreSliceCopies(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, []string{"copy-source/tool"}, wave49ToolNames(mgr.AllTools()))
	...
	require.Equal(t, []string{"copy-source/resource"}, wave49ResourceNames(mgr.AllResources()))

	prompts := mgr.AllPrompts()
	require.Len(t, prompts, 1)
	prompts[0] = DiscoveredPrompt{ServerName: "mutated", Prompt: &sdkmcp.Prompt{Name: "mutated"}}
	require.Equal(t, []string{"copy-source/prompt"}, wave49PromptNames(mgr.AllPrompts()))
}

func TestServerManagerWave49_DisconnectAllStopsRegisteredConnections(t *testing.T) {
	t.Parallel()

	clientSession, serverSession := wave49InMemorySession(t)
...
func wave49InMemorySession(t *testing.T) (*sdkmcp.ClientSession, *sdkmcp.ServerSession) {
	t.Helper()
	...
	server := sdkmcp.NewServer(
		&sdkmcp.Implementation{Name: "wave49-server", Version: "test"},
		nil,
	)
	client := sdkmcp.NewClient(
		&sdkmcp.Implementation{Name: "wave49-client", Version: "test"},
		nil,
	)
...
func wave49ToolNames(tools []DiscoveredTool) []string {
...
func wave49ResourceNames(resources []DiscoveredResource) []string {
...
func wave49PromptNames(prompts []DiscoveredPrompt) []string {
