...
func TestServerConnectionWave42_ToolsReturnsSliceCopy(t *testing.T) {
	t.Parallel()
...
func TestServerConnectionWave42_StdioTransportPublishesFailOpenSandboxDecision(t *testing.T) {
	t.Parallel()
	...
	conn := NewServerConnection("unsandboxed",
		config.MCPServerConfig{
			...
			Command:   "echo",
			Args:      []string{"hello"},
			Env:       map[string]string{"MCP_TEST_ENV": "wave42"},
		},
		config.MCPConfig{},
	)
	...
	require.Contains(t, commandTransport.Command.Env, "MCP_TEST_ENV=wave42")
...
func TestServerConnectionWave42_SandboxApplyErrorPublishesDecisionByMode(t *testing.T) {
	t.Parallel()
...
func TestServerConnectionWave42_TimeoutPrefersServerThenGlobalThenDefault(t *testing.T) {
	t.Parallel()
...
func TestServerConnectionWave42_ConnectTransportStartErrorSetsFailedState(t *testing.T) {
	t.Parallel()

	missingCommand := filepath.Join(t.TempDir(), "missing-mcp-command-wave42")
...
func TestServerConnectionWave42_DisconnectClosesInMemorySession(t *testing.T) {
	t.Parallel()

	clientSession, _ := newWave42InMemorySession(t)
...
func TestServerConnectionWave42_HealthCheckUsesInMemorySessionState(t *testing.T) {
	t.Parallel()
...
func newWave42InMemorySession(t *testing.T) (*sdkmcp.ClientSession, *sdkmcp.ServerSession) {
	t.Helper()
	...
	server := sdkmcp.NewServer(
		&sdkmcp.Implementation{Name: "wave42-server", Version: "test"},
		nil,
	)
	client := sdkmcp.NewClient(
		&sdkmcp.Implementation{Name: "wave42-client", Version: "test"},
		nil,
	)
