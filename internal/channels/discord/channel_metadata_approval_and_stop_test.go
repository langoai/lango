...
type channelMetadataApprovalAndStopSession struct {
...
func newChannelMetadataApprovalAndStopSession() *channelMetadataApprovalAndStopSession {
	state := &discordgo.State{}
	state.User = &discordgo.User{
		ID:       "bot-18",
		Username: "DiscordApprovalBot",
	}

	return &channelMetadataApprovalAndStopSession{state: state}
}

func (s *channelMetadataApprovalAndStopSession) Open() error {
	return nil
}

func (s *channelMetadataApprovalAndStopSession) Close() error {
	s.closeCalls++
	return nil
}

func (s *channelMetadataApprovalAndStopSession) AddHandler(handler interface{}) func() {
	s.registeredFns = append(s.registeredFns, handler)
	return func() {}
}

func (s *channelMetadataApprovalAndStopSession) ChannelMessageSend(
	channelID string,
	content string,
	...
	return &discordgo.Message{ID: "channelMetadataApprovalAndStop8-msg", Content: content}, nil
}

func (s *channelMetadataApprovalAndStopSession) ChannelMessageSendComplex(
	channelID string,
	...
	return &discordgo.Message{ID: "channelMetadataApprovalAndStop8-complex-msg", Content: data.Content}, nil
}

func (s *channelMetadataApprovalAndStopSession) ChannelMessageEditComplex(
...
func (s *channelMetadataApprovalAndStopSession) ChannelTyping(
	channelID string,
	options ...discordgo.RequestOption,
) error {
...
func (s *channelMetadataApprovalAndStopSession) InteractionRespond(
...
func (s *channelMetadataApprovalAndStopSession) ApplicationCommandCreate(
	appID string,
	guildID string,
...
func (s *channelMetadataApprovalAndStopSession) GetState() *discordgo.State {
	return s.state
}

type channelMetadataApprovalAndStopUserMessageError struct {
	internal string
	user     string
}

func (e channelMetadataApprovalAndStopUserMessageError) Error() string {
	return e.internal
}

func (e channelMetadataApprovalAndStopUserMessageError) UserMessage() string {
	return e.user
}

func TestChannelMetadataApprovalAndStop(t *testing.T) {
	t.Parallel()

	session := newChannelMetadataApprovalAndStopSession()
...
func TestGuildMentionAndContentHelpers(t *testing.T) {
	t.Parallel()
...
func TestSendTextSplitsAndStopsOnError(t *testing.T) {
	t.Parallel()
	...
	errorSession := newChannelMetadataApprovalAndStopSession()
...
func TestSendEmbedMapsFieldsAndReturnsComplexError(t *testing.T) {
	t.Parallel()
...
func TestFormatChannelErrorAndSendError(t *testing.T) {
	t.Parallel()

	assert.Equal(
		t,
		"Please retry with a smaller request.",
		formatChannelError(channelMetadataApprovalAndStopUserMessageError{
	...
	channel.sendError("chan-18", channelMetadataApprovalAndStopUserMessageError{
...
func TestTypingStartStopIsDeterministic(t *testing.T) {
	t.Parallel()
	...
	privateSession := newChannelMetadataApprovalAndStopSession()
...
func TestStartRegistersCommandsWhenApplicationIDIsConfigured(t *testing.T) {
	t.Parallel()
...
func TestInteractionCreateRoutesOnlyMessageComponents(t *testing.T) {
	t.Parallel()
