...
func TestFindContentRootPrefersRoleMainDiv(t *testing.T) {
	t.Parallel()

	doc := parseFindContentRootPrefersRoleMainDivHTML(t, `<html><body>
...
func TestFindLargestDivChoosesMostTextWhenSemanticRootsAreAbsent(t *testing.T) {
	t.Parallel()
...
func TestFindLargestDivReturnsNilWithoutDivs(t *testing.T) {
	t.Parallel()

	doc := parseFindContentRootPrefersRoleMainDivHTML(t, `<html><body><section>content</section></body></html>`)

	assert.Nil(t, findLargestDiv(doc))
	assert.Nil(t, findByAttr(doc, atom.Div, "role", "main"))
}

func TestExtractMarkdownCoversInlineAndBlockBranches(t *testing.T) {
	t.Parallel()
...
func parseFindContentRootPrefersRoleMainDivHTML(t *testing.T, input string) *html.Node {
	t.Helper()
