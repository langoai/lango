...
func TestWave50BuildTools_MetadataAndReadStatHandlers(t *testing.T) {
	t.Parallel()
	...
	byName := wave50FilesystemToolsByName(tools)
	...
	assert.ElementsMatch(t, []string{
		...
		"fs_delete",
		"fs_stat",
	}, wave50FilesystemToolNames(tools))
...
func TestWave50BuildTools_MutationHandlersRequireInputsAndApplySideEffects(t *testing.T) {
	t.Parallel()

	tmpDir := t.TempDir()
	tools := wave50FilesystemToolsByName(BuildTools(New(Config{AllowedPaths: []string{tmpDir}})))
...
func wave50FilesystemToolsByName(tools []*agent.Tool) map[string]*agent.Tool {
...
func wave50FilesystemToolNames(tools []*agent.Tool) []string {
