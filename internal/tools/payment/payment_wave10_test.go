...
func TestPaymentLimitsTool_ReturnsGenericAndEntLimiterFields(t *testing.T) {
	...
	genericLimiter := &wave10Limiter{
		spent:     wave10MustParseUSDC(t, "2.50"),
		remaining: wave10MustParseUSDC(t, "17.50"),
	}
	tools := BuildTools(&wave10PaymentService{chainID: 84532}, genericLimiter, nil, 84532, nil, nil, &fakeExecutionAuditor{})
	...
	entLimiter, err := wallet.NewStoreSpendingLimiter(
		wave10UsageStore{amounts: []string{"1.00", "bad-amount", "0.75"}},
		"5.00",
		"20.00",
		"1.00",
	)
	require.NoError(t, err)
	tools = BuildTools(&wave10PaymentService{chainID: 84532}, entLimiter, nil, 84532, nil, nil, &fakeExecutionAuditor{})
	limitsTool = findTool(tools, "payment_limits")
	require.NotNil(t, limitsTool)
...
func TestPaymentLimitsTool_PropagatesLimiterErrors(t *testing.T) {
	...
	limitsTool := buildLimitsTool(&wave10Limiter{spentErr: errors.New("spent failed")})
	...
	limitsTool = buildLimitsTool(&wave10Limiter{spent: big.NewInt(0), remainingErr: errors.New("remaining failed")})
	got, err = limitsTool.Handler(context.Background(), map[string]interface{}{})
	require.Error(t, err)
	assert.Nil(t, got)
...
type wave10PaymentService struct {
	balance          string
	balanceErr       error
	history          []corepayment.TransactionInfo
	historyErr       error
...
func (s *wave10PaymentService) Send(context.Context, corepayment.PaymentRequest) (*corepayment.PaymentReceipt, error) {
	return &corepayment.PaymentReceipt{}, nil
}

func (s *wave10PaymentService) Balance(context.Context) (string, error) {
	if s.balanceErr != nil {
		return "", s.balanceErr
	}
...
func (s *wave10PaymentService) History(_ context.Context, limit int) ([]corepayment.TransactionInfo, error) {
...
func (s *wave10PaymentService) WalletAddress(context.Context) (string, error) {
	if s.walletErr != nil {
		return "", s.walletErr
	}
...
func (s *wave10PaymentService) ChainID() int64 {
	return s.chainID
}

func (s *wave10PaymentService) RecordX402Payment(context.Context, corepayment.X402PaymentRecord) error {
	return nil
}

type wave10Limiter struct {
	spent        *big.Int
	spentErr     error
	remaining    *big.Int
	remainingErr error
...
func (l *wave10Limiter) Check(context.Context, *big.Int) error {
	return nil
}

func (l *wave10Limiter) Record(context.Context, *big.Int) error {
	return nil
}

func (l *wave10Limiter) DailySpent(context.Context) (*big.Int, error) {
	if l.spentErr != nil {
...
func (l *wave10Limiter) DailyRemaining(context.Context) (*big.Int, error) {
...
func (l *wave10Limiter) IsAutoApprovable(context.Context, *big.Int) (bool, error) {
	return false, nil
}

type wave10UsageStore struct {
	amounts []string
}

func (s wave10UsageStore) DailySpendSince(context.Context, time.Time) ([]string, error) {
	return s.amounts, nil
}

func wave10MustParseUSDC(t *testing.T, amount string) *big.Int {
	t.Helper()
