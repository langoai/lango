...
func TestDenyAllPaymentExecutionGate_DeniesMissingReceipt(t *testing.T) {
	t.Parallel()
...
func TestCreateWalletTool_CreatesAndReportsExistingWallet(t *testing.T) {
	secrets := newDenyAllPaymentExecutionGateDeniesMissingReceiptSecretsStore(t)
	...
	require.NoError(t, err)
	created := requireDenyAllPaymentExecutionGateDeniesMissingReceiptMap(t, result)
	...
	result, err = tool.Handler(context.Background(), map[string]interface{}{})
	require.NoError(t, err)
	existing := requireDenyAllPaymentExecutionGateDeniesMissingReceiptMap(t, result)
...
func TestX402FetchTool_FetchesTruncatesAndRecordsPaymentResponse(t *testing.T) {
	...
	interceptor := x402.NewInterceptor(
		denyAllPaymentExecutionGateDeniesMissingReceiptSignerProvider{signer: denyAllPaymentExecutionGateDeniesMissingReceiptEvmSigner{address: signerAddress}},
		nil,
		x402.Config{Enabled: true, ChainID: 84532},
		zap.NewNop().Sugar(),
	)
	svc := &denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder{}
	...
	payload := requireDenyAllPaymentExecutionGateDeniesMissingReceiptMap(t, result)
...
func TestX402FetchTool_PropagatesSignerProviderError(t *testing.T) {
	t.Parallel()

	interceptor := x402.NewInterceptor(
		denyAllPaymentExecutionGateDeniesMissingReceiptSignerProvider{err: errors.New("signer unavailable")},
		nil,
		x402.Config{Enabled: true, ChainID: 84532},
		zap.NewNop().Sugar(),
	)
...
func newDenyAllPaymentExecutionGateDeniesMissingReceiptSecretsStore(t *testing.T) *security.SecretsStore {
	t.Helper()
...
func requireDenyAllPaymentExecutionGateDeniesMissingReceiptMap(t *testing.T, result interface{}) map[string]interface{} {
	t.Helper()
...
type denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder struct {
	records []corepayment.X402PaymentRecord
}

func (s *denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder) Send(context.Context, corepayment.PaymentRequest) (*corepayment.PaymentReceipt, error) {
	return nil, errors.New("unexpected send")
}

func (s *denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder) Balance(context.Context) (string, error) {
	return "", errors.New("unexpected balance")
}

func (s *denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder) History(context.Context, int) ([]corepayment.TransactionInfo, error) {
	return nil, errors.New("unexpected history")
}

func (s *denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder) WalletAddress(context.Context) (string, error) {
	return "", errors.New("unexpected wallet address")
}

func (s *denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder) ChainID() int64 {
	return 84532
}

func (s *denyAllPaymentExecutionGateDeniesMissingReceiptPaymentRecorder) RecordX402Payment(_ context.Context, record corepayment.X402PaymentRecord) error {
	s.records = append(s.records, record)
	return nil
}

type denyAllPaymentExecutionGateDeniesMissingReceiptSignerProvider struct {
	signer evm.ClientEvmSigner
	err    error
}

func (p denyAllPaymentExecutionGateDeniesMissingReceiptSignerProvider) EvmSigner(context.Context) (evm.ClientEvmSigner, error) {
	if p.err != nil {
		return nil, p.err
	}
	if p.signer != nil {
	...
	return denyAllPaymentExecutionGateDeniesMissingReceiptPrivateKeySigner{key: key}, nil
}

type denyAllPaymentExecutionGateDeniesMissingReceiptEvmSigner struct {
	address string
}

func (s denyAllPaymentExecutionGateDeniesMissingReceiptEvmSigner) Address() string { return s.address }

func (s denyAllPaymentExecutionGateDeniesMissingReceiptEvmSigner) SignTypedData(context.Context, evm.TypedDataDomain, map[string][]evm.TypedDataField, string, map[string]interface{}) ([]byte, error) {
	return []byte("signature"), nil
}

type denyAllPaymentExecutionGateDeniesMissingReceiptPrivateKeySigner struct {
	key *ecdsa.PrivateKey
}

func (s denyAllPaymentExecutionGateDeniesMissingReceiptPrivateKeySigner) Address() string {
	return crypto.PubkeyToAddress(s.key.PublicKey).Hex()
}

func (s denyAllPaymentExecutionGateDeniesMissingReceiptPrivateKeySigner) SignTypedData(context.Context, evm.TypedDataDomain, map[string][]evm.TypedDataField, string, map[string]interface{}) ([]byte, error) {
