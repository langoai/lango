...
func TestWave50BuildTools_MetadataAndHandlerUsesSearchEndpoint(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "coverage wave", r.URL.Query().Get("q"))
	...
	require.True(t, ok)
	assert.Equal(t, "coverage wave", payload["query"])
