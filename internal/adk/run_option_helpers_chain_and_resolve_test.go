...
func TestRunOptionHelpersChainAndResolve(t *testing.T) {
	t.Parallel()
...
func TestCollectionHelpersCoverNilEmptyAndMultipleParts(t *testing.T) {
	t.Parallel()
...
func TestRecordDiagnosticsCountsDelegationsRootToolsAndResults(t *testing.T) {
	t.Parallel()
...
func TestIsolatedAgentSetAndCollectionPolicy(t *testing.T) {
	t.Parallel()
...
func TestDiscardReasonClassifiesRecoverableCleanupReasons(t *testing.T) {
	t.Parallel()
...
func TestTruncatePreservesShortTextAndRuneBoundaries(t *testing.T) {
	t.Parallel()
...
func TestRunStreamingDetailedErrorReturnsPartialDiagnosticsAndFinish(t *testing.T) {
	t.Parallel()

	root, err := adkagent.New(adkagent.Config{
		Name:        "runOptionHelpersChainAndResolve0-error-root",
		Description: "runOptionHelpersChainAndResolve0 error root",
		Run: func(adkagent.InvocationContext) iter.Seq2[*session.Event, error] {
			return func(yield func(*session.Event, error) bool) {
				if !yield(&session.Event{
					Author: "runOptionHelpersChainAndResolve0-error-root",
					LLMResponse: model.LLMResponse{
	...
	report, err := agent.RunStreamingDetailed(
		context.Background(),
		"runOptionHelpersChainAndResolve0-stream-error",
		"fail after partial",
