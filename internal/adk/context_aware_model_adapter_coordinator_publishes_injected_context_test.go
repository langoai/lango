...
func TestContextAwareModelAdapterCoordinatorPublishesInjectedContext(t *testing.T) {
	t.Parallel()

	p := &mockProvider{
		id: "newDeadLetterStatusLoaderWrapsBuildAppError0-provider",
		events: []provider.StreamEvent{
			{Type: provider.StreamEventPlainText, Text: "ok"},
			{Type: provider.StreamEventDone},
		},
	}
	inner := NewModelAdapter(p, "newDeadLetterStatusLoaderWrapsBuildAppError0-model")
	retriever := knowledge.NewContextRetriever(nil, 5, zap.NewNop().Sugar())
	coordinatorAgent := &contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent{
	...
	adapter := NewContextAwareModelAdapter(inner, retriever, prompt.DefaultBuilder(), zap.NewNop().Sugar()).
		...
		WithEventBus(bus)
	assert.Equal(t, "newDeadLetterStatusLoaderWrapsBuildAppError0-model", adapter.Name())

	ctx := session.WithSessionKey(context.Background(), "newDeadLetterStatusLoaderWrapsBuildAppError0:session")
	ctx = session.WithTurnID(ctx, "turn-newDeadLetterStatusLoaderWrapsBuildAppError0")
	req := &adkmodel.LLMRequest{
		Model: "newDeadLetterStatusLoaderWrapsBuildAppError0-model",
	...
	assert.Equal(t, "turn-newDeadLetterStatusLoaderWrapsBuildAppError0", gotEvent.TurnID)
	assert.Equal(t, "newDeadLetterStatusLoaderWrapsBuildAppError0:session", gotEvent.SessionKey)
...
func TestAdaptToolWithTimeoutReportsDeadline(t *testing.T) {
	t.Parallel()
...
func TestAdaptToolForAgentInjectsOwnerWithoutTimeout(t *testing.T) {
	t.Parallel()
	...
	got, err := runnable.Run(&contextHelpersRoundTripAgentAndChildSessionToolContext{Context: context.Background()}, map[string]any{})
	require.NoError(t, err)
	assert.Equal(t, map[string]any{"result": "planner"}, got)
}

func TestBuildInputSchemaConvertsInterfaceRequiredBranches(t *testing.T) {
	t.Parallel()
...
type contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent struct {
	mu       sync.Mutex
	findings []retrieval.Finding
	gotQuery string
	gotLimit int
...
func (a *contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent) Name() string {
	return "newDeadLetterStatusLoaderWrapsBuildAppError0-retrieval"
}

func (a *contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent) Layers() []knowledge.ContextLayer {
	return []knowledge.ContextLayer{knowledge.LayerUserKnowledge}
}

func (a *contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent) Search(_ context.Context, query string, limit int) ([]retrieval.Finding, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.gotQuery = query
	a.gotLimit = limit
...
func (a *contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent) query() string {
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.gotQuery
}

func (a *contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent) limit() int {
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.gotLimit
}
...
var (
	_ retrieval.RetrievalAgent = (*contextAwareModelAdapterCoordinatorPublishesInjectedContextRetrievalAgent)(nil)
)
