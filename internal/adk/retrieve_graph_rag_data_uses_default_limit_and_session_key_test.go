...
type retrieveGraphRagDataUsesDefaultLimitAndSessionKeyContentRetriever struct {
	query   string
...
func (r *retrieveGraphRagDataUsesDefaultLimitAndSessionKeyContentRetriever) Retrieve(_ context.Context, query string, opts graph.ContentRetrieveOptions) ([]graph.ContentResult, error) {
	r.query = query
	r.opts = opts
	return r.results, r.err
}

func TestRetrieveGraphRAGDataUsesDefaultLimitAndSessionKey(t *testing.T) {
	t.Parallel()

	retriever := &retrieveGraphRagDataUsesDefaultLimitAndSessionKeyContentRetriever{
		results: []graph.ContentResult{{
			Collection: "session",
			SourceID:   "turn-1",
...
func TestFormatGraphRAGSectionTruncatesContentBeforeGraphResults(t *testing.T) {
	t.Parallel()
...
func TestFormatRecallSectionAppliesBudgetAndSkipsEmptySummaries(t *testing.T) {
	t.Parallel()
...
func TestDiscardActiveChildWithoutReasonRollsBackOverlayWithoutParentNote(t *testing.T) {
	t.Parallel()
...
func TestCleanupFailedTurnWithNilStoreIsNoop(t *testing.T) {
	t.Parallel()
...
func TestDanglingToolCallsNormalizesEmptyIDsAndMatchesResponsesByName(t *testing.T) {
	t.Parallel()
...
func TestRetrieveGraphRAGDataTreatsRetrieverErrorsAsEmptyResults(t *testing.T) {
	t.Parallel()

	retriever := &retrieveGraphRagDataUsesDefaultLimitAndSessionKeyContentRetriever{err: errors.New("fts unavailable")}
	graphRAG := graph.NewGraphRAGService(retriever, nil, 1, 1, zap.NewNop().Sugar())
