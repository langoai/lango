...
type runAndCollectUsesLearnedFixRetryResponseFixProvider struct {
	calls int
	errs  []string
}

func (p *runAndCollectUsesLearnedFixRetryResponseFixProvider) GetFixForError(_ context.Context, toolName string, err error) (string, bool) {
	p.calls++
	p.errs = append(p.errs, toolName+":"+err.Error())
	return "retry with deterministic fix", true
}

func TestRunAndCollectUsesLearnedFixRetryResponse(t *testing.T) {
	t.Parallel()

	var inputs []string
	root, err := adkagent.New(adkagent.Config{
		Name:        "runAndCollectUsesLearnedFixRetryResponse4-root",
		Description: "runAndCollectUsesLearnedFixRetryResponse4 root",
		Run: func(ctx adkagent.InvocationContext) iter.Seq2[*session.Event, error] {
			...
			return func(yield func(*session.Event, error) bool) {
				...
				yield(&session.Event{
					Author: "runAndCollectUsesLearnedFixRetryResponse4-root",
	...
	provider := &runAndCollectUsesLearnedFixRetryResponseFixProvider{}
	agent, err := NewAgentFromADK(root, newMockStore(), WithAgentErrorFixProvider(provider))
	require.NoError(t, err)

	got, err := agent.RunAndCollect(context.Background(), "runAndCollectUsesLearnedFixRetryResponse4-learned-fix", "original input")
	require.NoError(t, err)
...
func TestNewAgentFromADKAppliesOptionsAndFiltersIsolatedNames(t *testing.T) {
	t.Parallel()

	var rootObserved string
	root := newStaticADKAgent(t, "runAndCollectUsesLearnedFixRetryResponse4-options-root", nil)
