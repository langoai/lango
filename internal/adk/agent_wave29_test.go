...
func TestWave29PureTransferCallClassification(t *testing.T) {
	t.Parallel()
...
func TestWave29RunAllowsPureTransferButRejectsRootToolCalls(t *testing.T) {
	t.Parallel()

	child := newStaticADKAgent(t, "operator", nil)

	transferRoot := newWave29SingleEventAgent(t, "transfer-root", []adkagent.Agent{child}, &session.Event{
	...
	for event, err := range transferAgent.Run(context.Background(), "wave29-transfer", "delegate") {
		require.NoError(t, err)
	...
	toolRoot := newWave29SingleEventAgent(t, "tool-root", []adkagent.Agent{child}, &session.Event{
		Author: "tool-root",
		LLMResponse: model.LLMResponse{
			Content: &genai.Content{Parts: []*genai.Part{
				{FunctionCall: &genai.FunctionCall{Name: "read_file"}},
	...
	for event, err := range toolAgent.Run(context.Background(), "wave29-root-tool", "call tool") {
		require.Error(t, err)
		assert.Nil(t, event)
...
func TestWave29MissingAgentRetryHelpers(t *testing.T) {
	t.Parallel()

	assert.Equal(t, "planner", extractMissingAgent(wave29ErrorString("failed to find agent: planner")))
	assert.Empty(t, extractMissingAgent(wave29ErrorString("provider failed")))
...
func TestWave29ToolSignatureFallsBackWhenArgsCannotMarshal(t *testing.T) {
	t.Parallel()
...
func newWave29SingleEventAgent(
	t *testing.T,
	name string,
	...
	agent, err := adkagent.New(adkagent.Config{
		Name:        name,
		Description: name + " wave29 test agent",
		SubAgents:   subAgents,
...
type wave29ErrorString string

func (e wave29ErrorString) Error() string { return string(e) }
