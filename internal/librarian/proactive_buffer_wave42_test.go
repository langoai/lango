...
type wave42TextGenerator struct {
	responses []string
	err       error
	calls     int
}

func (g *wave42TextGenerator) GenerateText(_ context.Context, _, _ string) (string, error) {
	g.calls++
	if g.err != nil {
		return "", g.err
	}
...
func TestWave42ProactiveBufferDefaultConfig(t *testing.T) {
...
func TestWave42ProactiveBufferProcessProviderErrors(t *testing.T) {
	...
	t.Run("observation provider error stops before analysis", func(t *testing.T) {
		generator := &wave42TextGenerator{responses: []string{`{"extractions":[],"gaps":[]}`}}
		buffer := NewProactiveBuffer(
...
func TestWave42ProactiveBufferProcessSkipsBelowObservationThreshold(t *testing.T) {
	generator := &wave42TextGenerator{responses: []string{`{"extractions":[],"gaps":[]}`}}
	buffer := NewProactiveBuffer(
		NewObservationAnalyzer(generator, testutil.NopLogger()),
		nil,
...
func TestWave42ProactiveBufferProcessCooldownAndMaxPendingInquiries(t *testing.T) {
	...
	generator := &wave42TextGenerator{responses: []string{
		wave42AnalysisWithGapsJSON(),
		wave42AnalysisWithGapsJSON(),
	}}
	buffer := NewProactiveBuffer(
		NewObservationAnalyzer(generator, testutil.NopLogger()),
		NewInquiryProcessor(&wave42TextGenerator{}, inquiryStore, knowledgeStore, testutil.NopLogger()),
		inquiryStore,
		knowledgeStore,
		func(string) ([]session.Message, error) {
			return nil, nil
...
func TestWave42ProactiveBufferProcessAutoSaveUnknownTypeAndEvents(t *testing.T) {
	ctx := context.Background()
	client := testutil.TestEntClient(t)
	inquiryStore := NewInquiryStore(client, testutil.NopLogger())
	knowledgeStore := knowledge.NewStore(client, testutil.NopLogger())
...
func TestWave42ProactiveBufferProcessResolvesInquiryAnswers(t *testing.T) {
	ctx := context.Background()
	client := testutil.TestEntClient(t)
	inquiryStore := NewInquiryStore(client, testutil.NopLogger())
	knowledgeStore := knowledge.NewStore(client, testutil.NopLogger())
...
func wave42AnalysisWithGapsJSON() string {
	return `{
		"extractions": [],
		"gaps": [
			{
