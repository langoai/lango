...
func TestWave56DockerRuntimeRunAssemblesContainerAndExecutionRequest(t *testing.T) {
	api := newWave56DockerAPIFake(t)
...
func TestWave56DockerRuntimeRunParsesDockerFramedToolError(t *testing.T) {
	api := newWave56DockerAPIFake(t)
	api.attachResponse = wave56DockerFrame([]byte(`{"output":{"partial":true},"error":"denied"}`))
...
func TestWave56DockerRuntimeRunReturnsOOMAndRemovesContainer(t *testing.T) {
	api := newWave56DockerAPIFake(t)
...
func TestWave56DockerRuntimeRunReportsCreateError(t *testing.T) {
	api := newWave56DockerAPIFake(t)
...
func TestWave56DockerRuntimeRunReportsMalformedResultAndRemovesContainer(t *testing.T) {
	api := newWave56DockerAPIFake(t)
...
type wave56DockerAPIFake struct {
	t      *testing.T
...
type wave56DockerAPISnapshot struct {
	createRequest container.CreateRequest
	attachQuery   url.Values
	attachRequest ExecutionRequest
	startCalls    int
...
func newWave56DockerAPIFake(t *testing.T) *wave56DockerAPIFake {
	t.Helper()

	api := &wave56DockerAPIFake{
		t:              t,
		errs:           make(chan error, 16),
		containerID:    "wave56-container",
		waitStatusCode: 0,
	}
...
func (api *wave56DockerAPIFake) close() {
	if api.server != nil {
		api.server.Close()
	}
}

func (api *wave56DockerAPIFake) recordAsyncError(err error) {
	if err == nil {
		return
	}
	select {
...
func (api *wave56DockerAPIFake) assertNoAsyncErrors(t *testing.T) {
	t.Helper()
...
func (api *wave56DockerAPIFake) snapshot() wave56DockerAPISnapshot {
	api.mu.Lock()
	defer api.mu.Unlock()

	return wave56DockerAPISnapshot{
...
func (api *wave56DockerAPIFake) handle(w http.ResponseWriter, r *http.Request) {
	path := wave56DockerAPIPath(r.URL.Path)
	switch {
...
func (api *wave56DockerAPIFake) handleCreate(w http.ResponseWriter, r *http.Request) {
...
func (api *wave56DockerAPIFake) handleAttach(w http.ResponseWriter, r *http.Request) {
...
func (api *wave56DockerAPIFake) serveHijackedAttach(conn net.Conn, rw *bufio.ReadWriter) {
	defer conn.Close()
...
func (api *wave56DockerAPIFake) handleStart(w http.ResponseWriter, _ *http.Request) {
...
func (api *wave56DockerAPIFake) handleWait(w http.ResponseWriter, _ *http.Request) {
	if api.waitStatus != 0 {
...
func (api *wave56DockerAPIFake) handleRemove(w http.ResponseWriter, _ *http.Request) {
	api.mu.Lock()
	api.removeCalls++
	api.mu.Unlock()
	w.WriteHeader(http.StatusNoContent)
...
func wave56DockerAPIPath(path string) string {
...
func wave56DockerFrame(payload []byte) []byte {
