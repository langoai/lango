...
func TestEditorInitWindowSizeAndWelcomeTransition(t *testing.T) {
	e := NewEditor()
...
func TestEditorMenuCheckersReflectCurrentState(t *testing.T) {
...
func TestCategoryIsEnabledCoversDeterministicBranches(t *testing.T) {
	trueValue := true
	tests := []struct {
		name      string
		category  string
...
func TestEditorEscPersistsNewProviderFormAndReturnsToList(t *testing.T) {
	e := NewEditorWithConfig(&config.Config{})
	e.step = StepForm
	e.activeForm = NewProviderForm("", config.ProviderConfig{})
	e.activeForm.Focus = true
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "id", "local-openai")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "type", "openai")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "apikey", "${OPENAI_API_KEY}")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "baseurl", "https://api.example.test/v1")
...
func TestEditorEscPersistsNewAuthProviderFormAndReturnsToList(t *testing.T) {
	e := NewEditorWithConfig(&config.Config{})
	e.step = StepForm
	e.activeForm = NewOIDCProviderForm("", config.OIDCProviderConfig{})
	e.activeForm.Focus = true
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "oidc_id", "corp")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "oidc_issuer", "https://issuer.example.test")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "oidc_client_id", "client-id")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "oidc_redirect", "http://localhost/callback")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "oidc_scopes", "openid,email")
...
func TestEditorEscPersistsNewMCPServerFormAndReturnsToList(t *testing.T) {
	e := NewEditorWithConfig(&config.Config{})
	e.step = StepForm
	e.activeForm = NewMCPServerForm("", config.MCPServerConfig{})
	e.activeForm.Focus = true
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "mcp_srv_name", "filesystem")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "mcp_srv_transport", "stdio")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "mcp_srv_command", "npx")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "mcp_srv_args", "-y,@modelcontextprotocol/server-filesystem")
	setEditorInitWindowSizeAndWelcomeTransitionFieldValue(e.activeForm, "mcp_srv_timeout", "15s")
	setEditorInitWindowSizeAndWelcomeTransitionFieldChecked(e.activeForm, "mcp_srv_enabled", true)
...
func TestEditorEscClosesOpenDropdownWithoutLeavingForm(t *testing.T) {
...
func TestEditorHelperBranchesAreNoOpsWhenStateIsMissing(t *testing.T) {
	e := NewEditorWithConfig(&config.Config{})
...
func TestEditorSetupFlowKeyBranchesCompleteAndSkip(t *testing.T) {
	tests := []struct {
		name string
		key  tea.KeyType
	}{
...
func TestEditorSetupFlowEscCancelsAndReturnsToMenuEditorInitWindowSizeAndWelcomeTransition(t *testing.T) {
	e := NewEditorWithConfig(&config.Config{})
	e.step = StepSetupFlow
	e.setupFlow = NewSetupFlow("p2p", []DepResult{
		{
...
func TestEditorCancelSelectionSetsErrorAndQuitCommand(t *testing.T) {
	e := NewEditorWithConfig(&config.Config{})
...
func setEditorInitWindowSizeAndWelcomeTransitionFieldValue(form *tuicore.FormModel, key string, value string) {
...
func setEditorInitWindowSizeAndWelcomeTransitionFieldChecked(form *tuicore.FormModel, key string, checked bool) {
	for _, field := range form.Fields {
		if field.Key != key {
			continue
		}
