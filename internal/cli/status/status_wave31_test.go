...
func TestWave31ToolCatalogDeadLetterBridgeDetailReadsTypedAndJSONPayloads(t *testing.T) {
	tests := []struct {
		name    string
		raw     interface{}
		wantTx  string
...
func TestWave31ToolCatalogDeadLetterBridgeDetailReportsUnavailableAndInvalidPayloads(t *testing.T) {
	tests := []struct {
		name    string
...
func TestWave31ToolCatalogDeadLetterBridgeRetryTrimsIDAndPreservesPrincipal(t *testing.T) {
	catalog := toolcatalog.New()
	var gotParams map[string]interface{}
	...
	bridge := NewToolCatalogDeadLetterBridge(catalog)
	ctx := ctxkeys.WithPrincipal(context.Background(), "operator:wave31")
	err := bridge.Retry(ctx, "  tx-retry  ")

	require.NoError(t, err)
	assert.Equal(t, "tx-retry", gotParams["transaction_receipt_id"])
	assert.Equal(t, "operator:wave31", gotPrincipal)
}

func TestWave31ToolCatalogDeadLetterBridgeRetryReportsUnavailableAndHandlerErrors(t *testing.T) {
...
func TestWave31OptionalIntAcceptsNumericToolPayloadTypes(t *testing.T) {
...
func TestWave31DeadLetterRetryFollowUpChangedDetectsStatusAndTaskTransitions(t *testing.T) {
...
func TestWave31CollectDeadLetterRetryFollowUpHandlesErrorCancelAndTimeout(t *testing.T) {
	baseline := postadjudicationstatus.TransactionStatus{
		IsDeadLettered: true,
		CanRetry:       true,
		RetryDeadLetterSummary: postadjudicationstatus.RetryDeadLetterSummary{
	...
	t.Run("canceled wait returns last unchanged follow-up", func(t *testing.T) {
		...
		bridge := &wave31CancelAfterFirstDetailBridge{
			detail: baseline,
			cancel: cancel,
		}
...
type wave31CancelAfterFirstDetailBridge struct {
	detail postadjudicationstatus.TransactionStatus
	cancel context.CancelFunc
	calls  int
}

func (b *wave31CancelAfterFirstDetailBridge) List(context.Context, DeadLetterListOptions) (DeadLetterListPage, error) {
	return DeadLetterListPage{}, nil
}

func (b *wave31CancelAfterFirstDetailBridge) Detail(ctx context.Context, transactionReceiptID string) (postadjudicationstatus.TransactionStatus, error) {
	b.calls++
...
func (b *wave31CancelAfterFirstDetailBridge) Retry(context.Context, string) error {
	return nil
}

func TestWave31DeadLetterRetryCmdJSONIncludesSanitizedFollowUpError(t *testing.T) {
...
func TestWave31SanitizeDeadLetterBacklogEntryCoversNestedBreakdown(t *testing.T) {
...
func TestWave31SanitizeTransactionReceiptCoversEscrowInputAndEnums(t *testing.T) {
...
func TestWave31JSONAndCLIErrorHelpersHandleNilAndBlankMessages(t *testing.T) {
