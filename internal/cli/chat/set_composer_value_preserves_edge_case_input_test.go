...
func TestSetComposerValuePreservesEdgeCaseInput(t *testing.T) {
...
func TestPendingApprovalKeyNilSharedAndFullscreenBranches(t *testing.T) {
	m := newTestModel()
	...
	fullscreen.approval.pending = &ApprovalRequestMsg{
		Request: approval.ApprovalRequest{
			ID:       "apr-setComposerValuePreservesEdgeCaseInput0",
			ToolName: "fs_write",
...
func TestUpdateResizeTickAndNonKeyBranchesWithoutProgram(t *testing.T) {
	m := newTestModel()
...
func TestSubmitCurrentInputNoopAndSetupGuardBranches(t *testing.T) {
	m := newTestModel()
	m.SetComposerValue(" \n\t ")
...
func TestSubmitCmdNilParentReturnsDoneAndTracksTurnState(t *testing.T) {
	...
	assert.Equal(t, "setComposerValuePreservesEdgeCaseInput0-session", executor.sessionID)
	assert.Equal(t, "direct submit", executor.input)
	assert.NotNil(t, executor.ctx)
}

func TestViewFooterAndSetupBranches(t *testing.T) {
	m := newTestModel()
	m.quitting = true
