...
func TestWave23HandleKeyCtrlDQuitsAndCancelsWithoutSubmitting(t *testing.T) {
	m := newTestModel()
	cancelled := false
...
func TestWave23HandleKeyCancellingStateIgnoresNonQuitKeys(t *testing.T) {
	m := newTestModel()
...
func TestWave23StreamingCtrlCTransitionsToCancellingAndPreservesDraft(t *testing.T) {
	m := newTestModel()
...
func TestWave23ApprovingExpiredConfirmIsClearedBeforeKeyDispatch(t *testing.T) {
	m := newTestModel()
...
func TestWave23ApprovingCriticalSessionGrantSecondPressRespondsAlwaysAllow(t *testing.T) {
...
func TestWave23ResetApprovalOwnerSharedPendingClearsLocalDialogState(t *testing.T) {
...
func TestWave23SubmitCurrentInputSlashResetsDraftBeforeSetupGuard(t *testing.T) {
	m := newTestModel()
	m.cfg = incompleteSetupConfig()
	m.SetComposerValue(" /unknown-wave23 ")
	...
	require.True(t, ok)
	assert.Contains(t, sys.Text, "Unknown command: /unknown-wave23")
	assert.Empty(t, m.chatView.entries)
}

func TestWave23SubmitCurrentInputSkipsSetupGuardWhenGuardDisabled(t *testing.T) {
...
func TestWave23AgentSetupReadyAndSetupVisibilityBranches(t *testing.T) {
...
func TestWave23ReplayKeysAppliesInputKeysAndKeepsProcessingAfterCommandKeys(t *testing.T) {
	m := newTestModel()
...
func TestWave23DoneRedirectUsesSubmissionHookAndDoesNotRenderFailure(t *testing.T) {
	var submissions []string
	m := New(Deps{
	...
	assert.Empty(t, m.pendingRedirectInput)
	assert.Equal(t, stateStreaming, m.state)
	assert.Equal(t, []string{"wave23-session:replacement prompt"}, submissions)
	assert.True(t, m.pending.IsActive())
