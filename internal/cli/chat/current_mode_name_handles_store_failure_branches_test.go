...
type currentModeNameHandlesStoreFailureBranchesSessionStore struct {
...
func (s *currentModeNameHandlesStoreFailureBranchesSessionStore) Get(string) (*session.Session, error) {
	return s.getSession, s.getErr
}

func (s *currentModeNameHandlesStoreFailureBranchesSessionStore) Create(sess *session.Session) error {
	if s.createErr != nil {
		return s.createErr
	}
...
func (s *currentModeNameHandlesStoreFailureBranchesSessionStore) Update(sess *session.Session) error {
	if s.updateErr != nil {
		return s.updateErr
	}
...
func TestCurrentModeNameHandlesStoreFailureBranches(t *testing.T) {
	m := newTestModel()

	m.sessionStore = &currentModeNameHandlesStoreFailureBranchesSessionStore{getErr: errors.New("store unavailable")}
	if got := m.currentModeName(); got != "" {
		t.Fatalf("expected empty mode when store returns error, got %q", got)
	}

	m.sessionStore = &currentModeNameHandlesStoreFailureBranchesSessionStore{}
	if got := m.currentModeName(); got != "" {
		t.Fatalf("expected empty mode when session is missing, got %q", got)
	}
}

func TestSetSessionModePropagatesCreateAndUpdateFailures(t *testing.T) {
	createErr := errors.New("create denied")
	createStore := &currentModeNameHandlesStoreFailureBranchesSessionStore{createErr: createErr}
	m := newTestModel()
	...
	updateStore := &currentModeNameHandlesStoreFailureBranchesSessionStore{
...
func TestHandleStreamingEnterWithBlankDraftDoesNotCancel(t *testing.T) {
	m := newTestModel()
...
func TestSubmitComposerWithParentUsesProvidedContext(t *testing.T) {
	...
	m := New(Deps{
		...
		SessionKey:   "initP2PEarlyExitBranchesAvoidNetworkState2-session",
		SessionStore: submitTestSessionStore{},
	})
	parent := context.WithValue(context.Background(), currentModeNameHandlesStoreFailureBranchesContextKey{}, "parent")
	...
	if got := executor.ctx.Value(currentModeNameHandlesStoreFailureBranchesContextKey{}); got != "parent" {
		t.Fatalf("expected runner context to inherit parent value, got %v", got)
	}
}

type currentModeNameHandlesStoreFailureBranchesContextKey struct{}
