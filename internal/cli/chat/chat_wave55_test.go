...
func TestWave55SharedApprovalResolveFailureKeepsPendingAndShowsStatus(t *testing.T) {
	...
	assert.Equal(t, "apr-wave55", shared.lastResolvedID)
...
func TestWave55SharedApprovalResolveAdvancesToNextPending(t *testing.T) {
...
func TestWave55DoneMsgFlushesTurnTokenUsageCommandAndResetsCounters(t *testing.T) {
	m := newTestModel()
	m.turnActive = true
...
func TestWave55BudgetWarningRendersDeterministicStatus(t *testing.T) {
	m := newTestModel()
