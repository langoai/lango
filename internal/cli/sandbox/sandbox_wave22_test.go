...
type wave22FakeIsolator struct {
	applyErr error
	rewrite  func(*exec.Cmd)
}

func (f wave22FakeIsolator) Apply(_ context.Context, cmd *exec.Cmd, _ sandboxos.Policy) error {
	if f.applyErr != nil {
		return f.applyErr
	}
	if f.rewrite != nil {
...
func (f wave22FakeIsolator) Available() bool { return true }
func (f wave22FakeIsolator) Name() string    { return "wave22-fake" }
func (f wave22FakeIsolator) Reason() string  { return "" }

func wave22RewriteShellExit(t *testing.T, code int) func(*exec.Cmd) {
	t.Helper()
...
func TestWave22CollectRecentDecisionsConvertsRecordsAndSwallowsErrors(t *testing.T) {
	t.Parallel()
...
func TestWave22BuildStatusSnapshotCoversOptOutFailModesAndWarnings(t *testing.T) {
	t.Parallel()
...
func TestWave22RenderStatusTableIncludesUnavailableAndWarningBranches(t *testing.T) {
	t.Parallel()
...
func TestWave22StatusCommandValidationAndLoaderErrors(t *testing.T) {
	t.Parallel()
...
func TestWave22NewTestCmdNativeBackendUnavailableSkipsSmokeTests(t *testing.T) {
	t.Parallel()
...
func TestWave22ProbeNetCommandConnectsToLoopbackAndReportsDialFailure(t *testing.T) {
	t.Parallel()
...
func TestWave22RunSmokeTestHelpersUseApplyAndCommandOutcome(t *testing.T) {
	t.Parallel()

	applyErr := errors.New("apply failed")
	assert.False(t, runReadTest(wave22FakeIsolator{applyErr: applyErr}))

	assert.True(t, runReadTest(wave22FakeIsolator{rewrite: wave22RewriteShellExit(t, 0)}))
	assert.False(t, runReadTest(wave22FakeIsolator{rewrite: wave22RewriteShellExit(t, 1)}))

	assert.True(t, runWriteTest(wave22FakeIsolator{rewrite: wave22RewriteShellExit(t, 1)}))
	assert.False(t, runWriteTest(wave22FakeIsolator{rewrite: wave22RewriteShellExit(t, 0)}))

	assert.True(t, runNetworkDenyTest(wave22FakeIsolator{rewrite: wave22RewriteShellExit(t, 1)}))
	assert.False(t, runNetworkDenyTest(wave22FakeIsolator{rewrite: wave22RewriteShellExit(t, 0)}))
	assert.False(t, runNetworkDenyTest(wave22FakeIsolator{applyErr: applyErr}))
}

func TestWave22RunWorkspaceWriteTestCreatesAllowedFile(t *testing.T) {
	t.Parallel()

	assert.True(t, runWorkspaceWriteTest(wave22FakeIsolator{}))
	assert.False(t, runWorkspaceWriteTest(wave22FakeIsolator{applyErr: errors.New("apply failed")}))
}

func TestWave22FindTouchFallsBackWhenPathLookupMisses(t *testing.T) {
	var want string
...
func runtimeGOOSForWave22() string {
	return runtime.GOOS
}
