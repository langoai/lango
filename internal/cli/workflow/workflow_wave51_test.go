...
func TestWave51WorkflowReadCommandsUseStateStoreWhenRunLedgerIsNotAuthoritative(t *testing.T) {
	t.Run("list prints started column from workflow state store", func(t *testing.T) {
		bootLoader, runID := wave51WorkflowBootLoader(t)
		...
		assert.Contains(t, out, "wave51-state")
	...
	t.Run("status prints state-store step errors with truncation", func(t *testing.T) {
		...
		assert.Contains(t, out, "Workflow:  wave51-state")
	...
	t.Run("history prints compact table from workflow state store", func(t *testing.T) {
		bootLoader, _ := wave51WorkflowBootLoader(t)
...
func TestWave51WorkflowReadCommandsWrapStateStoreErrors(t *testing.T) {
	client := enttest.Open(t, "sqlite3", wave51SQLiteDSN(t, "closed"))
...
func TestWave51InitEngineBranches(t *testing.T) {
	...
	client := enttest.Open(t, "sqlite3", wave51SQLiteDSN(t, "engine"))
...
func wave51WorkflowBootLoader(t *testing.T) (func() (*bootstrap.Result, error), string) {
	t.Helper()
	client := enttest.Open(t, "sqlite3", wave51SQLiteDSN(t, "state"))
	t.Cleanup(func() { client.Close() })
...
func wave51SQLiteDSN(t *testing.T, suffix string) string {
	t.Helper()
