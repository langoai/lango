...
func TestConfigSetFromEnvArgValidationBranches(t *testing.T) {
	tests := []struct {
		name     string
		args     []string
		contains string
		...
		{
			name:     "missing path",
			args:     []string{"--from-env", "LANGO_PROVIDER_VALUE"},
...
func TestConfigPathHelpersCoverEmptyAndFallbackBranches(t *testing.T) {
...
func TestSensitiveSegmentVariantsRemainRedacted(t *testing.T) {
	sensitive := []string{
		"webhookURL",
		"credential",
		"credentials",
...
func TestRedactConfigGetValueNilAndJSONMarshalFallback(t *testing.T) {
...
func TestRedactReflectValuePreservesJSONMarshalers(t *testing.T) {
	value := configSetFromEnvArgValidationBranchesJSONMarshaler{Value: "raw-secret"}
	got := redactConfigGetReflectValue([]string{"custom"}, reflect.ValueOf(value))

	if _, ok := got.(configSetFromEnvArgValidationBranchesJSONMarshaler); !ok {
...
type configSetFromEnvArgValidationBranchesJSONMarshaler struct {
	Value string
}

func (m configSetFromEnvArgValidationBranchesJSONMarshaler) MarshalJSON() ([]byte, error) {
