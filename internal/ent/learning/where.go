// Code generated by ent, DO NOT EDIT.

package learning

import (
	"time"

	"entgo.io/ent/dialect/sql"
	"github.com/google/uuid"
	"github.com/langoai/lango/internal/ent/predicate"
)

// ID filters vertices based on their ID field.
func ID(id uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldID, id))
}

// IDEQ applies the EQ predicate on the ID field.
func IDEQ(id uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldID, id))
}

// IDNEQ applies the NEQ predicate on the ID field.
func IDNEQ(id uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldID, id))
}

// IDIn applies the In predicate on the ID field.
func IDIn(ids ...uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldID, ids...))
}

// IDNotIn applies the NotIn predicate on the ID field.
func IDNotIn(ids ...uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldID, ids...))
}

// IDGT applies the GT predicate on the ID field.
func IDGT(id uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldID, id))
}

// IDGTE applies the GTE predicate on the ID field.
func IDGTE(id uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldID, id))
}

// IDLT applies the LT predicate on the ID field.
func IDLT(id uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldID, id))
}

// IDLTE applies the LTE predicate on the ID field.
func IDLTE(id uuid.UUID) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldID, id))
}

// Trigger applies equality check predicate on the "trigger" field. It's identical to TriggerEQ.
func Trigger(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldTrigger, v))
}

// ErrorPattern applies equality check predicate on the "error_pattern" field. It's identical to ErrorPatternEQ.
func ErrorPattern(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldErrorPattern, v))
}

// Diagnosis applies equality check predicate on the "diagnosis" field. It's identical to DiagnosisEQ.
func Diagnosis(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldDiagnosis, v))
}

// Fix applies equality check predicate on the "fix" field. It's identical to FixEQ.
func Fix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldFix, v))
}

// PayloadCiphertext applies equality check predicate on the "payload_ciphertext" field. It's identical to PayloadCiphertextEQ.
func PayloadCiphertext(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldPayloadCiphertext, v))
}

// PayloadNonce applies equality check predicate on the "payload_nonce" field. It's identical to PayloadNonceEQ.
func PayloadNonce(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldPayloadNonce, v))
}

// PayloadKeyVersion applies equality check predicate on the "payload_key_version" field. It's identical to PayloadKeyVersionEQ.
func PayloadKeyVersion(v int) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldPayloadKeyVersion, v))
}

// OccurrenceCount applies equality check predicate on the "occurrence_count" field. It's identical to OccurrenceCountEQ.
func OccurrenceCount(v int) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldOccurrenceCount, v))
}

// SuccessCount applies equality check predicate on the "success_count" field. It's identical to SuccessCountEQ.
func SuccessCount(v int) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldSuccessCount, v))
}

// Confidence applies equality check predicate on the "confidence" field. It's identical to ConfidenceEQ.
func Confidence(v float64) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldConfidence, v))
}

// CreatedAt applies equality check predicate on the "created_at" field. It's identical to CreatedAtEQ.
func CreatedAt(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldCreatedAt, v))
}

// UpdatedAt applies equality check predicate on the "updated_at" field. It's identical to UpdatedAtEQ.
func UpdatedAt(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldUpdatedAt, v))
}

// TriggerEQ applies the EQ predicate on the "trigger" field.
func TriggerEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldTrigger, v))
}

// TriggerNEQ applies the NEQ predicate on the "trigger" field.
func TriggerNEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldTrigger, v))
}

// TriggerIn applies the In predicate on the "trigger" field.
func TriggerIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldTrigger, vs...))
}

// TriggerNotIn applies the NotIn predicate on the "trigger" field.
func TriggerNotIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldTrigger, vs...))
}

// TriggerGT applies the GT predicate on the "trigger" field.
func TriggerGT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldTrigger, v))
}

// TriggerGTE applies the GTE predicate on the "trigger" field.
func TriggerGTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldTrigger, v))
}

// TriggerLT applies the LT predicate on the "trigger" field.
func TriggerLT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldTrigger, v))
}

// TriggerLTE applies the LTE predicate on the "trigger" field.
func TriggerLTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldTrigger, v))
}

// TriggerContains applies the Contains predicate on the "trigger" field.
func TriggerContains(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContains(FieldTrigger, v))
}

// TriggerHasPrefix applies the HasPrefix predicate on the "trigger" field.
func TriggerHasPrefix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasPrefix(FieldTrigger, v))
}

// TriggerHasSuffix applies the HasSuffix predicate on the "trigger" field.
func TriggerHasSuffix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasSuffix(FieldTrigger, v))
}

// TriggerEqualFold applies the EqualFold predicate on the "trigger" field.
func TriggerEqualFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEqualFold(FieldTrigger, v))
}

// TriggerContainsFold applies the ContainsFold predicate on the "trigger" field.
func TriggerContainsFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContainsFold(FieldTrigger, v))
}

// ErrorPatternEQ applies the EQ predicate on the "error_pattern" field.
func ErrorPatternEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldErrorPattern, v))
}

// ErrorPatternNEQ applies the NEQ predicate on the "error_pattern" field.
func ErrorPatternNEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldErrorPattern, v))
}

// ErrorPatternIn applies the In predicate on the "error_pattern" field.
func ErrorPatternIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldErrorPattern, vs...))
}

// ErrorPatternNotIn applies the NotIn predicate on the "error_pattern" field.
func ErrorPatternNotIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldErrorPattern, vs...))
}

// ErrorPatternGT applies the GT predicate on the "error_pattern" field.
func ErrorPatternGT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldErrorPattern, v))
}

// ErrorPatternGTE applies the GTE predicate on the "error_pattern" field.
func ErrorPatternGTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldErrorPattern, v))
}

// ErrorPatternLT applies the LT predicate on the "error_pattern" field.
func ErrorPatternLT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldErrorPattern, v))
}

// ErrorPatternLTE applies the LTE predicate on the "error_pattern" field.
func ErrorPatternLTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldErrorPattern, v))
}

// ErrorPatternContains applies the Contains predicate on the "error_pattern" field.
func ErrorPatternContains(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContains(FieldErrorPattern, v))
}

// ErrorPatternHasPrefix applies the HasPrefix predicate on the "error_pattern" field.
func ErrorPatternHasPrefix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasPrefix(FieldErrorPattern, v))
}

// ErrorPatternHasSuffix applies the HasSuffix predicate on the "error_pattern" field.
func ErrorPatternHasSuffix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasSuffix(FieldErrorPattern, v))
}

// ErrorPatternIsNil applies the IsNil predicate on the "error_pattern" field.
func ErrorPatternIsNil() predicate.Learning {
	return predicate.Learning(sql.FieldIsNull(FieldErrorPattern))
}

// ErrorPatternNotNil applies the NotNil predicate on the "error_pattern" field.
func ErrorPatternNotNil() predicate.Learning {
	return predicate.Learning(sql.FieldNotNull(FieldErrorPattern))
}

// ErrorPatternEqualFold applies the EqualFold predicate on the "error_pattern" field.
func ErrorPatternEqualFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEqualFold(FieldErrorPattern, v))
}

// ErrorPatternContainsFold applies the ContainsFold predicate on the "error_pattern" field.
func ErrorPatternContainsFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContainsFold(FieldErrorPattern, v))
}

// DiagnosisEQ applies the EQ predicate on the "diagnosis" field.
func DiagnosisEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldDiagnosis, v))
}

// DiagnosisNEQ applies the NEQ predicate on the "diagnosis" field.
func DiagnosisNEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldDiagnosis, v))
}

// DiagnosisIn applies the In predicate on the "diagnosis" field.
func DiagnosisIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldDiagnosis, vs...))
}

// DiagnosisNotIn applies the NotIn predicate on the "diagnosis" field.
func DiagnosisNotIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldDiagnosis, vs...))
}

// DiagnosisGT applies the GT predicate on the "diagnosis" field.
func DiagnosisGT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldDiagnosis, v))
}

// DiagnosisGTE applies the GTE predicate on the "diagnosis" field.
func DiagnosisGTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldDiagnosis, v))
}

// DiagnosisLT applies the LT predicate on the "diagnosis" field.
func DiagnosisLT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldDiagnosis, v))
}

// DiagnosisLTE applies the LTE predicate on the "diagnosis" field.
func DiagnosisLTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldDiagnosis, v))
}

// DiagnosisContains applies the Contains predicate on the "diagnosis" field.
func DiagnosisContains(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContains(FieldDiagnosis, v))
}

// DiagnosisHasPrefix applies the HasPrefix predicate on the "diagnosis" field.
func DiagnosisHasPrefix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasPrefix(FieldDiagnosis, v))
}

// DiagnosisHasSuffix applies the HasSuffix predicate on the "diagnosis" field.
func DiagnosisHasSuffix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasSuffix(FieldDiagnosis, v))
}

// DiagnosisIsNil applies the IsNil predicate on the "diagnosis" field.
func DiagnosisIsNil() predicate.Learning {
	return predicate.Learning(sql.FieldIsNull(FieldDiagnosis))
}

// DiagnosisNotNil applies the NotNil predicate on the "diagnosis" field.
func DiagnosisNotNil() predicate.Learning {
	return predicate.Learning(sql.FieldNotNull(FieldDiagnosis))
}

// DiagnosisEqualFold applies the EqualFold predicate on the "diagnosis" field.
func DiagnosisEqualFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEqualFold(FieldDiagnosis, v))
}

// DiagnosisContainsFold applies the ContainsFold predicate on the "diagnosis" field.
func DiagnosisContainsFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContainsFold(FieldDiagnosis, v))
}

// FixEQ applies the EQ predicate on the "fix" field.
func FixEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldFix, v))
}

// FixNEQ applies the NEQ predicate on the "fix" field.
func FixNEQ(v string) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldFix, v))
}

// FixIn applies the In predicate on the "fix" field.
func FixIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldFix, vs...))
}

// FixNotIn applies the NotIn predicate on the "fix" field.
func FixNotIn(vs ...string) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldFix, vs...))
}

// FixGT applies the GT predicate on the "fix" field.
func FixGT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldFix, v))
}

// FixGTE applies the GTE predicate on the "fix" field.
func FixGTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldFix, v))
}

// FixLT applies the LT predicate on the "fix" field.
func FixLT(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldFix, v))
}

// FixLTE applies the LTE predicate on the "fix" field.
func FixLTE(v string) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldFix, v))
}

// FixContains applies the Contains predicate on the "fix" field.
func FixContains(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContains(FieldFix, v))
}

// FixHasPrefix applies the HasPrefix predicate on the "fix" field.
func FixHasPrefix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasPrefix(FieldFix, v))
}

// FixHasSuffix applies the HasSuffix predicate on the "fix" field.
func FixHasSuffix(v string) predicate.Learning {
	return predicate.Learning(sql.FieldHasSuffix(FieldFix, v))
}

// FixIsNil applies the IsNil predicate on the "fix" field.
func FixIsNil() predicate.Learning {
	return predicate.Learning(sql.FieldIsNull(FieldFix))
}

// FixNotNil applies the NotNil predicate on the "fix" field.
func FixNotNil() predicate.Learning {
	return predicate.Learning(sql.FieldNotNull(FieldFix))
}

// FixEqualFold applies the EqualFold predicate on the "fix" field.
func FixEqualFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldEqualFold(FieldFix, v))
}

// FixContainsFold applies the ContainsFold predicate on the "fix" field.
func FixContainsFold(v string) predicate.Learning {
	return predicate.Learning(sql.FieldContainsFold(FieldFix, v))
}

// PayloadCiphertextEQ applies the EQ predicate on the "payload_ciphertext" field.
func PayloadCiphertextEQ(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldPayloadCiphertext, v))
}

// PayloadCiphertextNEQ applies the NEQ predicate on the "payload_ciphertext" field.
func PayloadCiphertextNEQ(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldPayloadCiphertext, v))
}

// PayloadCiphertextIn applies the In predicate on the "payload_ciphertext" field.
func PayloadCiphertextIn(vs ...[]byte) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldPayloadCiphertext, vs...))
}

// PayloadCiphertextNotIn applies the NotIn predicate on the "payload_ciphertext" field.
func PayloadCiphertextNotIn(vs ...[]byte) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldPayloadCiphertext, vs...))
}

// PayloadCiphertextGT applies the GT predicate on the "payload_ciphertext" field.
func PayloadCiphertextGT(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldPayloadCiphertext, v))
}

// PayloadCiphertextGTE applies the GTE predicate on the "payload_ciphertext" field.
func PayloadCiphertextGTE(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldPayloadCiphertext, v))
}

// PayloadCiphertextLT applies the LT predicate on the "payload_ciphertext" field.
func PayloadCiphertextLT(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldPayloadCiphertext, v))
}

// PayloadCiphertextLTE applies the LTE predicate on the "payload_ciphertext" field.
func PayloadCiphertextLTE(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldPayloadCiphertext, v))
}

// PayloadCiphertextIsNil applies the IsNil predicate on the "payload_ciphertext" field.
func PayloadCiphertextIsNil() predicate.Learning {
	return predicate.Learning(sql.FieldIsNull(FieldPayloadCiphertext))
}

// PayloadCiphertextNotNil applies the NotNil predicate on the "payload_ciphertext" field.
func PayloadCiphertextNotNil() predicate.Learning {
	return predicate.Learning(sql.FieldNotNull(FieldPayloadCiphertext))
}

// PayloadNonceEQ applies the EQ predicate on the "payload_nonce" field.
func PayloadNonceEQ(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldPayloadNonce, v))
}

// PayloadNonceNEQ applies the NEQ predicate on the "payload_nonce" field.
func PayloadNonceNEQ(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldPayloadNonce, v))
}

// PayloadNonceIn applies the In predicate on the "payload_nonce" field.
func PayloadNonceIn(vs ...[]byte) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldPayloadNonce, vs...))
}

// PayloadNonceNotIn applies the NotIn predicate on the "payload_nonce" field.
func PayloadNonceNotIn(vs ...[]byte) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldPayloadNonce, vs...))
}

// PayloadNonceGT applies the GT predicate on the "payload_nonce" field.
func PayloadNonceGT(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldPayloadNonce, v))
}

// PayloadNonceGTE applies the GTE predicate on the "payload_nonce" field.
func PayloadNonceGTE(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldPayloadNonce, v))
}

// PayloadNonceLT applies the LT predicate on the "payload_nonce" field.
func PayloadNonceLT(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldPayloadNonce, v))
}

// PayloadNonceLTE applies the LTE predicate on the "payload_nonce" field.
func PayloadNonceLTE(v []byte) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldPayloadNonce, v))
}

// PayloadNonceIsNil applies the IsNil predicate on the "payload_nonce" field.
func PayloadNonceIsNil() predicate.Learning {
	return predicate.Learning(sql.FieldIsNull(FieldPayloadNonce))
}

// PayloadNonceNotNil applies the NotNil predicate on the "payload_nonce" field.
func PayloadNonceNotNil() predicate.Learning {
	return predicate.Learning(sql.FieldNotNull(FieldPayloadNonce))
}

// PayloadKeyVersionEQ applies the EQ predicate on the "payload_key_version" field.
func PayloadKeyVersionEQ(v int) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldPayloadKeyVersion, v))
}

// PayloadKeyVersionNEQ applies the NEQ predicate on the "payload_key_version" field.
func PayloadKeyVersionNEQ(v int) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldPayloadKeyVersion, v))
}

// PayloadKeyVersionIn applies the In predicate on the "payload_key_version" field.
func PayloadKeyVersionIn(vs ...int) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldPayloadKeyVersion, vs...))
}

// PayloadKeyVersionNotIn applies the NotIn predicate on the "payload_key_version" field.
func PayloadKeyVersionNotIn(vs ...int) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldPayloadKeyVersion, vs...))
}

// PayloadKeyVersionGT applies the GT predicate on the "payload_key_version" field.
func PayloadKeyVersionGT(v int) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldPayloadKeyVersion, v))
}

// PayloadKeyVersionGTE applies the GTE predicate on the "payload_key_version" field.
func PayloadKeyVersionGTE(v int) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldPayloadKeyVersion, v))
}

// PayloadKeyVersionLT applies the LT predicate on the "payload_key_version" field.
func PayloadKeyVersionLT(v int) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldPayloadKeyVersion, v))
}

// PayloadKeyVersionLTE applies the LTE predicate on the "payload_key_version" field.
func PayloadKeyVersionLTE(v int) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldPayloadKeyVersion, v))
}

// PayloadKeyVersionIsNil applies the IsNil predicate on the "payload_key_version" field.
func PayloadKeyVersionIsNil() predicate.Learning {
	return predicate.Learning(sql.FieldIsNull(FieldPayloadKeyVersion))
}

// PayloadKeyVersionNotNil applies the NotNil predicate on the "payload_key_version" field.
func PayloadKeyVersionNotNil() predicate.Learning {
	return predicate.Learning(sql.FieldNotNull(FieldPayloadKeyVersion))
}

// CategoryEQ applies the EQ predicate on the "category" field.
func CategoryEQ(v Category) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldCategory, v))
}

// CategoryNEQ applies the NEQ predicate on the "category" field.
func CategoryNEQ(v Category) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldCategory, v))
}

// CategoryIn applies the In predicate on the "category" field.
func CategoryIn(vs ...Category) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldCategory, vs...))
}

// CategoryNotIn applies the NotIn predicate on the "category" field.
func CategoryNotIn(vs ...Category) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldCategory, vs...))
}

// TagsIsNil applies the IsNil predicate on the "tags" field.
func TagsIsNil() predicate.Learning {
	return predicate.Learning(sql.FieldIsNull(FieldTags))
}

// TagsNotNil applies the NotNil predicate on the "tags" field.
func TagsNotNil() predicate.Learning {
	return predicate.Learning(sql.FieldNotNull(FieldTags))
}

// OccurrenceCountEQ applies the EQ predicate on the "occurrence_count" field.
func OccurrenceCountEQ(v int) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldOccurrenceCount, v))
}

// OccurrenceCountNEQ applies the NEQ predicate on the "occurrence_count" field.
func OccurrenceCountNEQ(v int) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldOccurrenceCount, v))
}

// OccurrenceCountIn applies the In predicate on the "occurrence_count" field.
func OccurrenceCountIn(vs ...int) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldOccurrenceCount, vs...))
}

// OccurrenceCountNotIn applies the NotIn predicate on the "occurrence_count" field.
func OccurrenceCountNotIn(vs ...int) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldOccurrenceCount, vs...))
}

// OccurrenceCountGT applies the GT predicate on the "occurrence_count" field.
func OccurrenceCountGT(v int) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldOccurrenceCount, v))
}

// OccurrenceCountGTE applies the GTE predicate on the "occurrence_count" field.
func OccurrenceCountGTE(v int) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldOccurrenceCount, v))
}

// OccurrenceCountLT applies the LT predicate on the "occurrence_count" field.
func OccurrenceCountLT(v int) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldOccurrenceCount, v))
}

// OccurrenceCountLTE applies the LTE predicate on the "occurrence_count" field.
func OccurrenceCountLTE(v int) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldOccurrenceCount, v))
}

// SuccessCountEQ applies the EQ predicate on the "success_count" field.
func SuccessCountEQ(v int) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldSuccessCount, v))
}

// SuccessCountNEQ applies the NEQ predicate on the "success_count" field.
func SuccessCountNEQ(v int) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldSuccessCount, v))
}

// SuccessCountIn applies the In predicate on the "success_count" field.
func SuccessCountIn(vs ...int) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldSuccessCount, vs...))
}

// SuccessCountNotIn applies the NotIn predicate on the "success_count" field.
func SuccessCountNotIn(vs ...int) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldSuccessCount, vs...))
}

// SuccessCountGT applies the GT predicate on the "success_count" field.
func SuccessCountGT(v int) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldSuccessCount, v))
}

// SuccessCountGTE applies the GTE predicate on the "success_count" field.
func SuccessCountGTE(v int) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldSuccessCount, v))
}

// SuccessCountLT applies the LT predicate on the "success_count" field.
func SuccessCountLT(v int) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldSuccessCount, v))
}

// SuccessCountLTE applies the LTE predicate on the "success_count" field.
func SuccessCountLTE(v int) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldSuccessCount, v))
}

// ConfidenceEQ applies the EQ predicate on the "confidence" field.
func ConfidenceEQ(v float64) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldConfidence, v))
}

// ConfidenceNEQ applies the NEQ predicate on the "confidence" field.
func ConfidenceNEQ(v float64) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldConfidence, v))
}

// ConfidenceIn applies the In predicate on the "confidence" field.
func ConfidenceIn(vs ...float64) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldConfidence, vs...))
}

// ConfidenceNotIn applies the NotIn predicate on the "confidence" field.
func ConfidenceNotIn(vs ...float64) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldConfidence, vs...))
}

// ConfidenceGT applies the GT predicate on the "confidence" field.
func ConfidenceGT(v float64) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldConfidence, v))
}

// ConfidenceGTE applies the GTE predicate on the "confidence" field.
func ConfidenceGTE(v float64) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldConfidence, v))
}

// ConfidenceLT applies the LT predicate on the "confidence" field.
func ConfidenceLT(v float64) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldConfidence, v))
}

// ConfidenceLTE applies the LTE predicate on the "confidence" field.
func ConfidenceLTE(v float64) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldConfidence, v))
}

// CreatedAtEQ applies the EQ predicate on the "created_at" field.
func CreatedAtEQ(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldCreatedAt, v))
}

// CreatedAtNEQ applies the NEQ predicate on the "created_at" field.
func CreatedAtNEQ(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldCreatedAt, v))
}

// CreatedAtIn applies the In predicate on the "created_at" field.
func CreatedAtIn(vs ...time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldCreatedAt, vs...))
}

// CreatedAtNotIn applies the NotIn predicate on the "created_at" field.
func CreatedAtNotIn(vs ...time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldCreatedAt, vs...))
}

// CreatedAtGT applies the GT predicate on the "created_at" field.
func CreatedAtGT(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldCreatedAt, v))
}

// CreatedAtGTE applies the GTE predicate on the "created_at" field.
func CreatedAtGTE(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldCreatedAt, v))
}

// CreatedAtLT applies the LT predicate on the "created_at" field.
func CreatedAtLT(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldCreatedAt, v))
}

// CreatedAtLTE applies the LTE predicate on the "created_at" field.
func CreatedAtLTE(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldCreatedAt, v))
}

// UpdatedAtEQ applies the EQ predicate on the "updated_at" field.
func UpdatedAtEQ(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldEQ(FieldUpdatedAt, v))
}

// UpdatedAtNEQ applies the NEQ predicate on the "updated_at" field.
func UpdatedAtNEQ(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldNEQ(FieldUpdatedAt, v))
}

// UpdatedAtIn applies the In predicate on the "updated_at" field.
func UpdatedAtIn(vs ...time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldIn(FieldUpdatedAt, vs...))
}

// UpdatedAtNotIn applies the NotIn predicate on the "updated_at" field.
func UpdatedAtNotIn(vs ...time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldNotIn(FieldUpdatedAt, vs...))
}

// UpdatedAtGT applies the GT predicate on the "updated_at" field.
func UpdatedAtGT(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldGT(FieldUpdatedAt, v))
}

// UpdatedAtGTE applies the GTE predicate on the "updated_at" field.
func UpdatedAtGTE(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldGTE(FieldUpdatedAt, v))
}

// UpdatedAtLT applies the LT predicate on the "updated_at" field.
func UpdatedAtLT(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldLT(FieldUpdatedAt, v))
}

// UpdatedAtLTE applies the LTE predicate on the "updated_at" field.
func UpdatedAtLTE(v time.Time) predicate.Learning {
	return predicate.Learning(sql.FieldLTE(FieldUpdatedAt, v))
}

// And groups predicates with the AND operator between them.
func And(predicates ...predicate.Learning) predicate.Learning {
	return predicate.Learning(sql.AndPredicates(predicates...))
}

// Or groups predicates with the OR operator between them.
func Or(predicates ...predicate.Learning) predicate.Learning {
	return predicate.Learning(sql.OrPredicates(predicates...))
}

// Not applies the not operator on the given predicate.
func Not(p predicate.Learning) predicate.Learning {
	return predicate.Learning(sql.NotPredicates(p))
}
