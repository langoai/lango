...
type payloadProtectorErrorsSurfaceFromKnowledgeAndLearningWritesFailingPayloadProtector struct{}

func (payloadProtectorErrorsSurfaceFromKnowledgeAndLearningWritesFailingPayloadProtector) EncryptPayload([]byte) ([]byte, []byte, int, error) {
	return nil, nil, 0, errors.New("runAndCollectUsesLearnedFixRetryResponse4 encrypt failed")
}

func (payloadProtectorErrorsSurfaceFromKnowledgeAndLearningWritesFailingPayloadProtector) DecryptPayload([]byte, []byte, int) ([]byte, error) {
	return nil, errors.New("runAndCollectUsesLearnedFixRetryResponse4 decrypt failed")
}

func TestPayloadProtectorErrorsSurfaceFromKnowledgeAndLearningWrites(t *testing.T) {
	store := newTestStore(t)
	store.SetPayloadProtector(payloadProtectorErrorsSurfaceFromKnowledgeAndLearningWritesFailingPayloadProtector{})
	ctx := context.Background()

	err := store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "runAndCollectUsesLearnedFixRetryResponse4-knowledge-encrypt-error",
	...
	require.Contains(t, err.Error(), "runAndCollectUsesLearnedFixRetryResponse4 encrypt failed")

	err = store.SaveLearning(ctx, "session-1", LearningEntry{
		Trigger:      "runAndCollectUsesLearnedFixRetryResponse4 learning encrypt error",
...
func TestKnowledgeDecryptFailureFallsBackToStoredProjection(t *testing.T) {
	store := newTestStore(t)
	store.SetPayloadProtector(stubPayloadProtector{})
	ctx := context.Background()

	const key = "runAndCollectUsesLearnedFixRetryResponse4-knowledge-decrypt-fallback"
...
func TestLearningDecryptFailureIsReturnedByReadsAndSearch(t *testing.T) {
	store := newTestStore(t)
	store.SetPayloadProtector(stubPayloadProtector{})
	ctx := context.Background()

	require.NoError(t, store.SaveLearning(ctx, "session-1", LearningEntry{
		Trigger:      "runAndCollectUsesLearnedFixRetryResponse4 corrupt learning",
...
func TestFTS5SyncFailuresDoNotBlockEntKnowledgeWrites(t *testing.T) {
	store, _ := newSaveKnowledgeAtomicRollsBackWhenFts5InsertFailsFTS5TestStore(t)
	ctx := context.Background()

	require.NoError(t, store.fts5Index.DropTable())
	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "runAndCollectUsesLearnedFixRetryResponse4-fts-sync-failure",
		Category: entknowledge.CategoryFact,
		Content:  "knowledge survives fts sync failure",
	}))

	got, err := store.GetKnowledge(ctx, "runAndCollectUsesLearnedFixRetryResponse4-fts-sync-failure")
	require.NoError(t, err)
	...
	require.Equal(t, "runAndCollectUsesLearnedFixRetryResponse4-fts-sync-failure", results[0].Key)

	scored, err := store.SearchKnowledgeScored(ctx, "survives", "", 10)
	require.NoError(t, err)
	require.Len(t, scored, 1)
	require.Equal(t, "runAndCollectUsesLearnedFixRetryResponse4-fts-sync-failure", scored[0].Entry.Key)
	require.Equal(t, "like", scored[0].SearchSource)
}

func TestLearningFTS5SearchFailureFallsBackToLIKE(t *testing.T) {
	store, _ := newSaveKnowledgeAtomicRollsBackWhenFts5InsertFailsFTS5TestStore(t)
	ctx := context.Background()

	require.NoError(t, store.SaveLearning(ctx, "session-1", LearningEntry{
		Trigger:      "runAndCollectUsesLearnedFixRetryResponse4 learning fallback",
	...
	require.NoError(t, err)
	require.Len(t, got, 1)
	require.Equal(t, "runAndCollectUsesLearnedFixRetryResponse4 learning fallback", got[0].Trigger)
}
