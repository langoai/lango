...
func TestSetFTS5IndexSettersStoreAndClearIndexes(t *testing.T) {
	store := newTestStore(t)

	knowledgeIdx := search.NewFTS5Index(nil, "knowledge_test_fts", []string{"key", "content"})
...
func TestSearchKnowledgeScoredFallsBackWhenFTS5SearchErrors(t *testing.T) {
	client := enttest.Open(t, "sqlite3", "file:initP2PInitializesLocalComponentsWithDefaultDurations7-scored-fallback?mode=memory&_fk=1")
	t.Cleanup(func() { client.Close() })

	rawDB, err := sql.Open("sqlite3", "file:initP2PInitializesLocalComponentsWithDefaultDurations7-scored-fallback-fts?mode=memory&_fk=1")
...
func TestSearchLearningsFallsBackWhenFTS5SearchErrors(t *testing.T) {
	client := enttest.Open(t, "sqlite3", "file:initP2PInitializesLocalComponentsWithDefaultDurations7-learning-fallback?mode=memory&_fk=1")
	t.Cleanup(func() { client.Close() })

	rawDB, err := sql.Open("sqlite3", "file:initP2PInitializesLocalComponentsWithDefaultDurations7-learning-fallback-fts?mode=memory&_fk=1")
	require.NoError(t, err)
	t.Cleanup(func() { rawDB.Close() })
	if !search.ProbeFTS5(rawDB) {
		t.Skip("FTS5 not available in current SQLite runtime")
	...
	require.NoError(t, store.SaveLearning(ctx, "session-1", LearningEntry{
		...
		ErrorPattern: "initP2PInitializesLocalComponentsWithDefaultDurations7 fallback needle",
	...
	require.Equal(t, "initP2PInitializesLocalComponentsWithDefaultDurations7 fallback needle", got[0].ErrorPattern)
}

func TestSaveKnowledgeAtomicProtectedUpdateAndDedupSyncsLatestFTS5(t *testing.T) {
	store, rawDB := newFTS5TestStore(t)
	store.SetPayloadProtector(stubPayloadProtector{})
	ctx := context.Background()

	entry := KnowledgeEntry{
		Key:         "initP2PInitializesLocalComponentsWithDefaultDurations7-atomic-knowledge",
		Category:    entknowledge.CategoryFact,
		Content:     "first protected content with alice@example.com",
		Source:      "tool:save_knowledge",
		SourceClass: "private-confidential",
		AssetLabel:  "knowledge/initP2PInitializesLocalComponentsWithDefaultDurations7-atomic",
	}
...
func TestSaveKnowledgeFTS5SyncFailuresDoNotBlockEntWrites(t *testing.T) {
	store, rawDB := newFTS5TestStore(t)
	ctx := context.Background()
	require.NoError(t, store.fts5Index.DropTable())

	entry := KnowledgeEntry{
		Key:      "initP2PInitializesLocalComponentsWithDefaultDurations7-sync-failure",
		Category: entknowledge.CategoryFact,
...
func TestSaveLearningSyncAndDeleteFailuresDoNotBlockEntWrites(t *testing.T) {
	store, rawDB := newFTS5TestStore(t)
	ctx := context.Background()
	require.NoError(t, store.learningFTS5Idx.DropTable())

	require.NoError(t, store.SaveLearning(ctx, "session-1", LearningEntry{
		Trigger:      "initP2PInitializesLocalComponentsWithDefaultDurations7 learning sync failure",
		ErrorPattern: "learning FTS insert failure",
		Fix:          "Ent row still persists",
		Category:     entlearning.CategoryToolError,
	}))
	id := learningIDsByTrigger(t, store, ctx)["initP2PInitializesLocalComponentsWithDefaultDurations7 learning sync failure"]
...
func TestKnowledgeVersionEquivalentReportsSourceClassErrors(t *testing.T) {
