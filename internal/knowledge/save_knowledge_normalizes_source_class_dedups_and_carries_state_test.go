...
type failingSaveKnowledgeNormalizesSourceClassDedupsAndCarriesStatePayloadProtector struct{}

func (failingSaveKnowledgeNormalizesSourceClassDedupsAndCarriesStatePayloadProtector) EncryptPayload([]byte) ([]byte, []byte, int, error) {
	return nil, nil, 0, errors.New("encrypt failed")
}

func (failingSaveKnowledgeNormalizesSourceClassDedupsAndCarriesStatePayloadProtector) DecryptPayload([]byte, []byte, int) ([]byte, error) {
	return nil, errors.New("decrypt failed")
}

func TestSaveKnowledgeNormalizesSourceClassDedupsAndCarriesState(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()

	entry := KnowledgeEntry{
		Key:         "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-version-state",
		Category:    entknowledge.CategoryFact,
		Content:     "versioned content",
		Tags:        []string{"prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9", "state"},
		Source:      "tool:first",
		SourceClass: "  public  ",
		AssetLabel:  "knowledge/prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9",
	}
	...
	require.Equal(t, []string{"prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9", "state"}, got.Tags)
...
func TestKnowledgeSearchAndKeywordHelpersHandleWhitespaceAndLatestOnly(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()

	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-alpha",
		Category: entknowledge.CategoryFact,
	...
	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-beta-key",
		Category: entknowledge.CategoryRule,
		Content:  "rule content",
	}))
	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-gamma",
		Category: entknowledge.CategoryFact,
	...
	require.ElementsMatch(t, []string{"prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-alpha", "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-beta-key"}, keys)

	facts, err := store.SearchKnowledge(ctx, "alpha beta", string(entknowledge.CategoryFact), 10)
	require.NoError(t, err)
	require.Len(t, facts, 1)
	require.Equal(t, "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-alpha", facts[0].Key)
}

func TestLearningStatsUseExactAggregatesAndOrderingBounds(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()
	base := time.Date(2026, 5, 19, 12, 0, 0, 0, time.UTC)

	_, err := store.client.Learning.Create().
		SetTrigger("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-old-timeout").
	...
	require.NoError(t, err)
	_, err = store.client.Learning.Create().
		SetTrigger("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-new-timeout").
		SetCategory(entlearning.CategoryTimeout).
		SetConfidence(0.8).
	...
	require.NoError(t, err)
	_, err = store.client.Learning.Create().
		SetTrigger("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-permission").
	...
	require.Equal(t, "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-permission", listed[0].Trigger)
	require.Equal(t, "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-old-timeout", listed[1].Trigger)
}

func TestLearningAndExternalRefHelpersFilterPrecisely(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()
	cutoff := time.Date(2026, 5, 19, 12, 0, 0, 0, time.UTC)

	_, err := store.client.Learning.Create().
		SetTrigger("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-delete-timeout-low-old").
	...
	require.NoError(t, err)
	_, err = store.client.Learning.Create().
		SetTrigger("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-keep-timeout-high-old").
	...
	require.NoError(t, err)
	_, err = store.client.Learning.Create().
		SetTrigger("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-keep-permission-low-old").
	...
	remaining, err := store.SearchLearningEntities(ctx, "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9", 10)
	require.NoError(t, err)
	...
	require.ElementsMatch(t, []string{
		"prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-keep-timeout-high-old",
		"prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-keep-permission-low-old",
	}, triggers)
	...
	require.NoError(t, store.SaveExternalRef(
		ctx,
		"prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-reference",
		"url",
	...
	require.NoError(t, store.SaveExternalRef(
		...
		"prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-reference",
		"file",
		"/tmp/prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-reference.md",
		"updated summary",
	))
	...
	require.Equal(t, "/tmp/prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-reference.md", refs[0].Location)
	require.Equal(t, "updated summary", refs[0].Summary)
}

func TestPayloadProtectionFailurePathsAreObservable(t *testing.T) {
	t.Run("knowledge write encryption error is wrapped", func(t *testing.T) {
		store := newTestStore(t)
		store.SetPayloadProtector(failingSaveKnowledgeNormalizesSourceClassDedupsAndCarriesStatePayloadProtector{})
		ctx := context.Background()

		err := store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
			Key:      "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-encrypt-fail",
			Category: entknowledge.CategoryFact,
	...
	t.Run("knowledge read falls back to stored projection on decrypt error", func(t *testing.T) {
		...
		_, err := store.client.Knowledge.Create().
			SetKey("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-decrypt-fallback").
		...
		got, err := store.GetKnowledge(ctx, "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-decrypt-fallback")
		require.NoError(t, err)
	...
	t.Run("learning write and read decryption errors are wrapped", func(t *testing.T) {
		...
		err := store.SaveLearning(ctx, "session-1", LearningEntry{
			Trigger:      "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9 learning encrypt",
		...
		_, err = store.client.Learning.Create().
			SetID(id).
			SetTrigger("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9 learning decrypt").
...
func TestClosedBackendErrorsCoverStoreOperations(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()
	...
	err = store.SaveAuditLog(ctx, AuditEntry{
		...
		Actor:  "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9",
	})
	require.Error(t, err)
...
func TestSaveAuditLogPersistsOptionalFields(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()

	err := store.SaveAuditLog(ctx, AuditEntry{
		SessionKey: "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-session",
		Action:     string(auditlog.ActionToolCall),
		Actor:      "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-agent",
	...
	row, err := store.client.AuditLog.Query().
		Where(auditlog.SessionKey("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-session")).
		Only(ctx)
	require.NoError(t, err)
	require.Equal(t, auditlog.ActionToolCall, row.Action)
	require.Equal(t, "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9-agent", row.Actor)
...
func TestSaveToolResultFormatsNonStringResult(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()
