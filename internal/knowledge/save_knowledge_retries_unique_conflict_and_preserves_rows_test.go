...
func TestSaveKnowledgeRetriesUniqueConflictAndPreservesRows(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()

	_, err := store.client.Knowledge.Create().
		SetKey("runChatUsesProgramSeamAndCleansUpSession1-conflict").
	...
	err = store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "runChatUsesProgramSeamAndCleansUpSession1-conflict",
	...
	rows, err := store.client.Knowledge.Query().
		Where(entknowledge.Key("runChatUsesProgramSeamAndCleansUpSession1-conflict")).
		All(ctx)
	require.NoError(t, err)
...
func TestSaveKnowledgeAtomicPersistsProtectedUpdateAndEvents(t *testing.T) {
	...
	entry := KnowledgeEntry{
		Key:         "runChatUsesProgramSeamAndCleansUpSession1-atomic-knowledge",
		Category:    entknowledge.CategoryFact,
		Content:     "runChatUsesProgramSeamAndCleansUpSession1 first protected content for alice@example.com",
		Tags:        []string{"runChatUsesProgramSeamAndCleansUpSession1", "atomic"},
		Source:      "tool:runChatUsesProgramSeamAndCleansUpSession1",
		SourceClass: "private-confidential",
		AssetLabel:  "knowledge/runChatUsesProgramSeamAndCleansUpSession1",
	}
	...
	entry.Content = "runChatUsesProgramSeamAndCleansUpSession1 second protected content with token SECRETSECRETSECRETSECRETSECRETSECRET"
	require.NoError(t, store.SaveKnowledge(ctx, "session-1", entry))
	require.NoError(t, store.SaveKnowledge(ctx, "session-1", entry))
	...
	require.Contains(t, events[0].Content, "first protected content")
...
func TestRelevanceUpdatesAffectLatestRowsAndSurfaceBackendErrors(t *testing.T) {
	store := newTestStore(t)
	ctx := context.Background()

	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "runChatUsesProgramSeamAndCleansUpSession1-relevance-versioned",
		Category: entknowledge.CategoryFact,
		Content:  "old content",
	}))
	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
	...
	rows, err := store.client.Knowledge.Query().
		Where(entknowledge.Key("runChatUsesProgramSeamAndCleansUpSession1-relevance-versioned")).
		Order(entknowledge.ByVersion()).
		All(ctx)
	require.NoError(t, err)
	require.Len(t, rows, 2)
	...
	require.NoError(t, store.BoostRelevanceScore(ctx, "runChatUsesProgramSeamAndCleansUpSession1-relevance-versioned", 0.5, 5.0))
	...
	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "runChatUsesProgramSeamAndCleansUpSession1-relevance-low",
		Category: entknowledge.CategoryFact,
		Content:  "low score",
	}))
	_, err = store.client.Knowledge.Update().
		Where(entknowledge.Key("runChatUsesProgramSeamAndCleansUpSession1-relevance-low"), entknowledge.IsLatest(true)).
	...
	latestVersioned, err := store.client.Knowledge.Query().
		Where(entknowledge.Key("runChatUsesProgramSeamAndCleansUpSession1-relevance-versioned"), entknowledge.IsLatest(true)).
		Only(ctx)
	require.NoError(t, err)
...
func TestFTSDeleteCleanupFailuresDoNotBlockEntityDeletes(t *testing.T) {
	store, rawDB := newSaveKnowledgeAtomicRollsBackWhenFts5InsertFailsFTS5TestStore(t)
	ctx := context.Background()

	require.NoError(t, store.SaveKnowledge(ctx, "session-1", KnowledgeEntry{
		Key:      "runChatUsesProgramSeamAndCleansUpSession1-delete-knowledge-fts",
		Category: entknowledge.CategoryFact,
		Content:  "runChatUsesProgramSeamAndCleansUpSession1 delete knowledge fts cleanup",
	}))
	require.NoError(t, store.fts5Index.DropTable())
	require.NoError(t, store.DeleteKnowledge(ctx, "runChatUsesProgramSeamAndCleansUpSession1-delete-knowledge-fts"))
	_, err := store.GetKnowledge(ctx, "runChatUsesProgramSeamAndCleansUpSession1-delete-knowledge-fts")
	require.Error(t, err)
	require.True(t, errors.Is(err, ErrKnowledgeNotFound))
	assertMissingSQLiteTable(t, rawDB, "knowledge_fts")

	require.NoError(t, recreateSaveKnowledgeRetriesUniqueConflictAndPreservesRowsLearningIndex(store))
	require.NoError(t, store.SaveLearning(ctx, "session-1", LearningEntry{
		Trigger:      "runChatUsesProgramSeamAndCleansUpSession1 delete learning fts",
		ErrorPattern: "runChatUsesProgramSeamAndCleansUpSession1 learning cleanup",
		Fix:          "delete still succeeds",
		Category:     entlearning.CategoryToolError,
	}))
	ids := learningIDsByTrigger(t, store, ctx)
	id := ids["runChatUsesProgramSeamAndCleansUpSession1 delete learning fts"]
...
func TestSaveLearningAtomicPersistsProtectedPayloadFTSAndEvent(t *testing.T) {
	store, rawDB := newSaveKnowledgeAtomicRollsBackWhenFts5InsertFailsFTS5TestStore(t)
	store.SetPayloadProtector(stubPayloadProtector{})
	ctx := context.Background()
	...
	err := store.SaveLearning(ctx, "session-1", LearningEntry{
		Trigger:      "  runChatUsesProgramSeamAndCleansUpSession1 protected learning  ",
		ErrorPattern: "runChatUsesProgramSeamAndCleansUpSession1 failure for bob@example.com",
		Diagnosis:    "diagnose protected learning",
		Fix:          "rotate token SECRETSECRETSECRETSECRETSECRETSECRET",
		Category:     entlearning.CategoryToolError,
		Tags:         []string{"runChatUsesProgramSeamAndCleansUpSession1", "learning"},
	})
	require.NoError(t, err)

	ids := learningIDsByTrigger(t, store, ctx)
	id := ids["runChatUsesProgramSeamAndCleansUpSession1 protected learning"]
	got, err := store.GetLearning(ctx, id)
	require.NoError(t, err)
	require.Equal(t, "runChatUsesProgramSeamAndCleansUpSession1 protected learning", got.Trigger)
	require.Equal(t, "runChatUsesProgramSeamAndCleansUpSession1 failure for bob@example.com", got.ErrorPattern)
	...
	require.Equal(t, []string{"runChatUsesProgramSeamAndCleansUpSession1", "learning"}, got.Tags)
	...
	require.NoError(t, err)
	require.Equal(t, "runChatUsesProgramSeamAndCleansUpSession1 protected learning", trigger)
	...
	require.Contains(t, events[0].Content, "runChatUsesProgramSeamAndCleansUpSession1 protected learning")
	require.NotContains(t, events[0].Content, "SECRETSECRETSECRETSECRETSECRETSECRET")

	err = store.SaveLearning(ctx, "session-1", LearningEntry{
		Trigger:      "runChatUsesProgramSeamAndCleansUpSession1 invalid category",
		ErrorPattern: "invalid category should fail",
		Category:     entlearning.Category("runChatUsesProgramSeamAndCleansUpSession1-invalid"),
	})
	require.Error(t, err)
	require.Contains(t, err.Error(), "create learning")
}

func recreateSaveKnowledgeRetriesUniqueConflictAndPreservesRowsLearningIndex(store *Store) error {
...
func TestResolveLearningEntryNilIsEmpty(t *testing.T) {
	store := newTestStore(t)
...
func TestKeywordPredicatesIgnoreWhitespaceOnlyTerms(t *testing.T) {
