...
func newWave50StateStore(t *testing.T) (*StateStore, *ent.Client) {
	t.Helper()
...
func TestStateStoreWave50_CreateRunWithIDPersistsCanonicalRunAndRejectsBadID(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	store, client := newWave50StateStore(t)
	workflow := &Workflow{
		Name:        "wave50",
	...
	assert.Equal(t, "wave50", run.WorkflowName)
...
func TestStateStoreWave50_StatusAndResultsReflectPersistedStepTransitions(t *testing.T) {
	t.Parallel()
...
func TestStateStoreWave50_ListRunsOrdersByNewestAndHonorsLimit(t *testing.T) {
	t.Parallel()
...
func TestStateStoreWave50_QueryMethodsRejectMalformedRunIDs(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	store, _ := newWave50StateStore(t)
