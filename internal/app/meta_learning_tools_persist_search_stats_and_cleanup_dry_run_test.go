...
func TestMetaLearningToolsPersistSearchStatsAndCleanupDryRun(t *testing.T) {
	ctx := context.Background()
	store := metaLearningToolsPersistSearchStatsAndCleanupDryRunKnowledgeStore(t)
	...
	got, err := saveTool.Handler(ctx, map[string]interface{}{
		"trigger":       "defaultServeAwaitShutdownWaitsForContextCancellation3-tool",
		"error_pattern": "defaultServeAwaitShutdownWaitsForContextCancellation3 timeout",
	...
	got, err = searchTool.Handler(ctx, map[string]interface{}{
		"query":    "defaultServeAwaitShutdownWaitsForContextCancellation3",
		"category": string(entlearning.CategoryTimeout),
	})
	require.NoError(t, err)
...
func TestMetaSkillToolsCreateFilterAndBlockTraversal(t *testing.T) {
	ctx := context.Background()
	store := metaLearningToolsPersistSearchStatsAndCleanupDryRunKnowledgeStore(t)
	skillsDir := t.TempDir()
	registry := metaLearningToolsPersistSearchStatsAndCleanupDryRunSkillRegistry(skillsDir)
	cfg := config.DefaultConfig()
	cfg.Modes = map[string]config.SessionMode{
		"defaultServeAwaitShutdownWaitsForContextCancellation3-mode": {
			Name:   "defaultServeAwaitShutdownWaitsForContextCancellation3-mode",
			Skills: []string{"defaultServeAwaitShutdownWaitsForContextCancellation3-skill"},
		},
	}
	...
	for _, name := range []string{"defaultServeAwaitShutdownWaitsForContextCancellation3-skill", "defaultServeAwaitShutdownWaitsForContextCancellation3-hidden"} {
	...
	got, err := listTool.Handler(session.WithModeName(ctx, "defaultServeAwaitShutdownWaitsForContextCancellation3-mode"), map[string]interface{}{
		"summary": true,
	})
	require.NoError(t, err)
	...
	assert.Equal(t, "defaultServeAwaitShutdownWaitsForContextCancellation3-skill", summaries[0]["name"])
	assert.Equal(t, "Deterministic skill", summaries[0]["description"])

	viewTool := findTool(tools, "view_skill")
	require.NotNil(t, viewTool)
	got, err = viewTool.Handler(ctx, map[string]interface{}{
		"name": "defaultServeAwaitShutdownWaitsForContextCancellation3-skill",
	})
	require.NoError(t, err)
	view := got.(map[string]interface{})
	assert.Equal(t, "defaultServeAwaitShutdownWaitsForContextCancellation3-skill", view["name"])
	assert.Contains(t, view["content"], "Deterministic skill")
	wantSkillPath, err := filepath.EvalSymlinks(filepath.Join(skillsDir, "defaultServeAwaitShutdownWaitsForContextCancellation3-skill", "SKILL.md"))
	require.NoError(t, err)
	assert.Equal(t, wantSkillPath, view["path"])

	got, err = viewTool.Handler(ctx, map[string]interface{}{
		"name": "defaultServeAwaitShutdownWaitsForContextCancellation3-skill",
		"path": "../defaultServeAwaitShutdownWaitsForContextCancellation3-hidden/SKILL.md",
	})
	require.Error(t, err)
	assert.Nil(t, got)
...
func TestAutomationBackgroundComponentLifecycleWithoutWorkers(t *testing.T) {
	t.Parallel()
...
func TestInitP2PDisabledOrMissingWalletKeepsFilesystemUntouched(t *testing.T) {
	t.Parallel()
...
func metaLearningToolsPersistSearchStatsAndCleanupDryRunKnowledgeStore(t *testing.T) *knowledge.Store {
	t.Helper()

	dbPath := filepath.Join(t.TempDir(), "defaultServeAwaitShutdownWaitsForContextCancellation3-knowledge.db")
	client := enttest.Open(t, "sqlite3", "file:"+dbPath+"?_fk=1")
	t.Cleanup(func() { client.Close() })
	return knowledge.NewStore(client, zap.NewNop().Sugar())
}

func metaLearningToolsPersistSearchStatsAndCleanupDryRunSkillRegistry(skillsDir string) *skill.Registry {
	store := skill.NewFileSkillStore(skillsDir, zap.NewNop().Sugar())
	return skill.NewRegistry(store, nil, zap.NewNop().Sugar())
}

func metaLearningToolsPersistSearchStatsAndCleanupDryRunModuleConfig(t *testing.T) *config.Config {
	t.Helper()
