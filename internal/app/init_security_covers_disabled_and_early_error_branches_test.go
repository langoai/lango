...
func TestInitSecurityCoversDisabledAndEarlyErrorBranches(t *testing.T) {
	t.Parallel()
...
func TestInitAuthReturnsNilForMissingOrInvalidProviders(t *testing.T) {
	t.Parallel()
...
func TestBuildSubAgentPromptFuncStripsGlobalSectionsAndKeepsSharedPrompt(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, os.WriteFile(filepath.Join(promptsDir, "SAFETY.md"),
		[]byte("Shared runMainSandboxWorkerModeShortCircuitsOtherStartup4 safety."), 0o600))
	require.NoError(t, os.WriteFile(filepath.Join(promptsDir, "CONVERSATION_RULES.md"),
		[]byte("Shared runMainSandboxWorkerModeShortCircuitsOtherStartup4 conversation rules."), 0o600))
	...
	assert.Contains(t, got, "Shared runMainSandboxWorkerModeShortCircuitsOtherStartup4 safety.")
	assert.Contains(t, got, "Shared runMainSandboxWorkerModeShortCircuitsOtherStartup4 conversation rules.")
	assert.NotContains(t, got, "GLOBAL IDENTITY MUST NOT LEAK")
	assert.NotContains(t, got, "GLOBAL TOOL RULES MUST NOT LEAK")
}

func TestInitP2PReturnsNilWhenNodeCreationFailsOnInvalidListenAddr(t *testing.T) {
	t.Parallel()
