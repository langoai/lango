...
func TestBuildMetaToolsWithRuntimes_ComposesAvailableRuntimeTools(t *testing.T) {
	t.Parallel()
...
func TestBuildMetaToolsWithRuntimes_ReceiptBackedHandlersRejectMissingStore(t *testing.T) {
	t.Parallel()
...
func TestRetryPostAdjudicationExecution_DependencyDisabledBeforeParams(t *testing.T) {
	t.Parallel()
...
func TestListSkills_FiltersBySessionModeAndReturnsSummary(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	registry := newMetaToolSkillRegistry(t)
	createBuildMetaToolsWithRuntimesComposesAvailableRuntimeToolsInstructionSkill(t, registry, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-allowed", "Allowed skill")
	createBuildMetaToolsWithRuntimesComposesAvailableRuntimeToolsInstructionSkill(t, registry, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-hidden", "Hidden skill")

	cfg := config.DefaultConfig()
	cfg.Modes = map[string]config.SessionMode{
		"runMainSandboxWorkerModeShortCircuitsOtherStartup4": {
			Name:   "runMainSandboxWorkerModeShortCircuitsOtherStartup4",
			Skills: []string{"runMainSandboxWorkerModeShortCircuitsOtherStartup4-allowed"},
		},
	}
	...
	got, err := tool.Handler(session.WithModeName(ctx, "runMainSandboxWorkerModeShortCircuitsOtherStartup4"), map[string]interface{}{
		"summary": true,
	})
	...
	assert.Equal(t, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-allowed", skills[0]["name"])
	assert.Equal(t, "Allowed skill", skills[0]["description"])
	assert.Equal(t, "Use runMainSandboxWorkerModeShortCircuitsOtherStartup4-allowed for deterministic meta-tool tests.", skills[0]["when_to_use"])
}

func TestViewSkill_ReturnsContextFallbackAndRejectsPathEscape(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	fallbackRegistry := newMetaToolSkillRegistry(t)
	createBuildMetaToolsWithRuntimesComposesAvailableRuntimeToolsInstructionSkill(t, fallbackRegistry, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-fallback", "Fallback skill")
	...
	got, err := fallbackTool.Handler(ctx, map[string]interface{}{"name": "runMainSandboxWorkerModeShortCircuitsOtherStartup4-fallback"})
	require.NoError(t, err)
	payload, ok := got.(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-fallback", payload["name"])
	assert.Equal(t, "Context for runMainSandboxWorkerModeShortCircuitsOtherStartup4-fallback.", payload["content"])
	...
	createBuildMetaToolsWithRuntimesComposesAvailableRuntimeToolsInstructionSkill(t, registry, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-files", "File skill")
	require.NoError(t, registry.Store().SaveResource(ctx, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-files", "docs/note.txt", []byte("note body")))
	...
	fileResult, err := fileTool.Handler(ctx, map[string]interface{}{
		"name": "runMainSandboxWorkerModeShortCircuitsOtherStartup4-files",
		"path": filepath.Join("docs", "note.txt"),
	})
	require.NoError(t, err)
	filePayload, ok := fileResult.(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-files", filePayload["name"])
	...
	linkPath := filepath.Join(skillsDir, "runMainSandboxWorkerModeShortCircuitsOtherStartup4-files", "docs", "outside-link.txt")
...
func TestLearningCleanup_DeletesSingleLearningByID(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	store := newAppKnowledgeStore(t)
	require.NoError(t, store.SaveLearning(ctx, "", knowledge.LearningEntry{
		Trigger:      "runMainSandboxWorkerModeShortCircuitsOtherStartup4 tool failure",
...
func newBuildMetaToolsWithRuntimesComposesAvailableRuntimeToolsSkillRegistry(t *testing.T, skillsDir string) *skill.Registry {
	t.Helper()
	require.NoError(t, os.MkdirAll(skillsDir, 0o700))
	logger := zap.NewNop().Sugar()
	store := skill.NewFileSkillStore(skillsDir, logger)
	return skill.NewRegistry(store, []*agent.Tool{{Name: "buildMetaToolsWithRuntimesComposesAvailableRuntimeTools_base_tool"}}, logger)
}

func createBuildMetaToolsWithRuntimesComposesAvailableRuntimeToolsInstructionSkill(t *testing.T, registry *skill.Registry, name, description string) {
	t.Helper()
