...
func TestHandleNegotiateProtocolProposeInvalidPriceDefaultsToZero(t *testing.T) {
	t.Parallel()
...
func TestHandleNegotiateProtocolCounterReturnsRoundAndReason(t *testing.T) {
	t.Parallel()
...
func TestHandleNegotiateProtocolAcceptAndRejectTerminalPhases(t *testing.T) {
	t.Parallel()
...
func TestHandleNegotiateProtocolErrorsForUnknownActionAndMissingSession(t *testing.T) {
	t.Parallel()
...
func TestSelectSettlerReturnsNoopWithoutPaymentComponents(t *testing.T) {
	t.Parallel()
...
func TestSelectSettlerFallsBackToCustodianWhenOnChainAddressMissing(t *testing.T) {
	t.Parallel()
...
func TestSelectSettlerUsesConfiguredOnChainSettlers(t *testing.T) {
	t.Parallel()
...
func TestCatalogSourceAdapterCoversNilAndModeFilteredSections(t *testing.T) {
	t.Parallel()
	...
	catalog.Register("alpha", []*agent.Tool{
		handleNegotiateProtocolProposeInvalidPriceDefaultsToZeroTool("alpha_one", agent.ExposureDefault),
		handleNegotiateProtocolProposeInvalidPriceDefaultsToZeroTool("alpha_two", agent.ExposureAlwaysVisible),
		handleNegotiateProtocolProposeInvalidPriceDefaultsToZeroTool("alpha_deferred", agent.ExposureDeferred),
	})
	catalog.Register("disabled", []*agent.Tool{handleNegotiateProtocolProposeInvalidPriceDefaultsToZeroTool("disabled_one", agent.ExposureDefault)})

	cfg := config.DefaultConfig()
	cfg.Modes = map[string]config.SessionMode{
		"escrowToolsNonHubLifecycleOmitsOnChainFields3": {
			Name:  "escrowToolsNonHubLifecycleOmitsOnChainFields3",
	...
	modeSection := adapter.BuildToolCatalogSection("escrowToolsNonHubLifecycleOmitsOnChainFields3")
	assert.Contains(t, modeSection, "## Tools Available in `escrowToolsNonHubLifecycleOmitsOnChainFields3` Mode")
...
func TestCatalogSourceAdapterTruncatesLongCategoryToolLists(t *testing.T) {
	t.Parallel()
	...
	for i := 0; i < 10; i++ {
		tools = append(tools, handleNegotiateProtocolProposeInvalidPriceDefaultsToZeroTool("bulk_"+string(rune('a'+i)), agent.ExposureDefault))
	}
...
func TestAutomationPromptSectionIncludesOnlyEnabledCapabilities(t *testing.T) {
	t.Parallel()
...
func handleNegotiateProtocolProposeInvalidPriceDefaultsToZeroTool(name string, exposure agent.ExposurePolicy) *agent.Tool {
	return &agent.Tool{
		Name:        name,
