...
func TestWave52P2PReputationToolReturnsKnownAndNewPeerPayloads(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	repStore := reputation.NewStore(testutil.TestEntClient(t), testLog())
	knownPeer := "did:lango:wave52-known"
	...
	got, err = tool.Handler(ctx, map[string]interface{}{"peer_did": "did:lango:wave52-new"})
	require.NoError(t, err)
	newPeer := wave28P2PPayload(t, got)
	assert.Equal(t, "did:lango:wave52-new", newPeer["peerDID"])
...
func TestWave52EconomyWiringBudgetAlertAndInvalidBudgetConfig(t *testing.T) {
	t.Parallel()
	...
	_, err := components.budgetEngine.Allocate("wave52-task", nil)
	require.NoError(t, err)
	require.NoError(t, components.budgetEngine.Record("wave52-task", budgetSpend(500_000)))

	require.Len(t, alerts, 1)
	assert.Equal(t, "wave52-task", alerts[0].TaskID)
...
func TestWave52SelectSettlerUnknownOnChainModeFallsBackToCustodian(t *testing.T) {
	t.Parallel()
...
func TestWave52KnowledgeFTS5NilAndMissingSourceTableBranches(t *testing.T) {
	t.Parallel()
