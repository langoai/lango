...
func TestBuildMetaTools_IncludesReleaseEscrowSettlement(t *testing.T) {
	tools := buildMetaToolsWithRuntimes(nil, nil, nil, config.SkillConfig{}, nil, receipts.NewStore(), nil, nil, nil, &fakeEscrowReleaseRuntime{})
...
func TestReleaseEscrowSettlement_FundedApprovedPathReturnsCanonicalReceipt(t *testing.T) {
	...
	tool := findTool(buildMetaToolsWithRuntimes(nil, nil, nil, config.SkillConfig{}, nil, store, nil, nil, nil, runtime), "release_escrow_settlement")
	require.NotNil(t, tool)
...
func TestReleaseEscrowSettlement_RejectsWhenEscrowOrSettlementStateIsWrong(t *testing.T) {
	...
	for _, tt := range cases {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			tool := findTool(buildMetaToolsWithRuntimes(nil, nil, nil, config.SkillConfig{}, nil, store, nil, nil, nil, &fakeEscrowReleaseRuntime{}), "release_escrow_settlement")
			require.NotNil(t, tool)
...
func TestReleaseEscrowSettlement_PropagatesRuntimeFailure(t *testing.T) {
	...
	tool := findTool(buildMetaToolsWithRuntimes(nil, nil, nil, config.SkillConfig{}, nil, store, nil, nil, nil, &fakeEscrowReleaseRuntime{err: errors.New("release failed")}), "release_escrow_settlement")
	require.NotNil(t, tool)
