...
func TestWave44PayGateAdapterMapsPostPaySettlementID(t *testing.T) {
	t.Parallel()
	...
	got, err := adapter.Check("did:lango:trusted", "wave44_paid_tool", map[string]interface{}{})
...
func TestWave44WirePostAgentRegistersP2PRoutesWhenHandlerIsNil(t *testing.T) {
	t.Parallel()
	...
	p2pc := &p2pComponents{
		pricingCfg: config.P2PPricingConfig{
			Enabled:    true,
			PerQuery:   "0.31",
			ToolPrices: map[string]string{"wave44_special": "1.23"},
		},
	}
	application := &App{
		Config:  cfg,
...
func TestWave44WirePostAgentInvalidMaxSafetyLevelDefaultsToModerate(t *testing.T) {
	t.Parallel()
	...
	cfg.P2P.AllowedTools = []string{"wave44_allowlisted"}
	handler := p2pproto.NewHandler(p2pproto.HandlerConfig{})
	catalog := toolcatalog.New()
	catalog.RegisterCategory(toolcatalog.Category{Name: "wave44", Description: "Wave 44", Enabled: true})
	catalog.Register("wave44", []*agent.Tool{
		{Name: "wave44_moderate", Description: "moderate", SafetyLevel: agent.SafetyLevelModerate},
		{Name: "wave44_allowlisted", Description: "allowlisted", SafetyLevel: agent.SafetyLevelDangerous},
	})
	application := &App{
		Config:      cfg,
		Gateway:     initGateway(cfg, nil, &stubSessionStore{}, nil),
	...
	wirePostAgent(
		...
		[]*agent.Tool{{Name: "wave44_moderate", Handler: func(context.Context, map[string]interface{}) (interface{}, error) {
	...
	assert.Equal(t, "wave44_allowlisted", allowedTools.Index(0).String())
	assert.False(t, handlerValue.FieldByName("safetyChecker").IsNil())
}

func TestWave44WirePostAgentDangerousToolRequiresOwnerApprovalAndDeniesUnavailableProvider(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	session, err := sessions.Create("did:lango:wave44-peer", false)
	require.NoError(t, err)
	handler := p2pproto.NewHandler(p2pproto.HandlerConfig{Sessions: sessions})
	catalog := toolcatalog.New()
	catalog.RegisterCategory(toolcatalog.Category{Name: "wave44", Description: "Wave 44", Enabled: true})
	catalog.Register("wave44", []*agent.Tool{{
		Name:        "wave44_dangerous",
	...
	wirePostAgent(
		...
		staticResolver{
			...
			appinit.ProvidesPayment: &paymentComponents{limiter: &wave44Limiter{}},
		},
		[]*agent.Tool{{
			Name:        "wave44_dangerous",
			Description: "dangerous",
	...
	request := p2pproto.Request{
		...
		RequestID:    "wave44-dangerous-tool",
		Payload: map[string]interface{}{
			"toolName": "wave44_dangerous",
	...
	assert.False(t, grants.IsGranted("p2p:did:lango:wave44-peer", "wave44_dangerous"))
	assert.True(t, stream.closed)
}

type wave44Limiter struct{}

func (l *wave44Limiter) Check(context.Context, *big.Int) error  { return nil }
func (l *wave44Limiter) Record(context.Context, *big.Int) error { return nil }
func (l *wave44Limiter) DailySpent(context.Context) (*big.Int, error) {
	return big.NewInt(0), nil
}
func (l *wave44Limiter) DailyRemaining(context.Context) (*big.Int, error) {
	return big.NewInt(0), nil
}
func (l *wave44Limiter) IsAutoApprovable(context.Context, *big.Int) (bool, error) {
	return false, nil
}
