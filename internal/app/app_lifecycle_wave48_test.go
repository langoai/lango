...
func TestWave48AppStartPropagatesRegistryErrorAndRollsBackStartedComponents(t *testing.T) {
	t.Parallel()
...
func TestWave48AppStopJoinsComponentStopAndWaitErrors(t *testing.T) {
	t.Parallel()
...
func TestWave48BuildCatalogFromEntriesPreservesEmptyAndDisabledCategories(t *testing.T) {
	t.Parallel()
	...
	catalog := buildCatalogFromEntries([]appinit.CatalogEntry{
		...
		},
		{
			...
			Tools: []*agent.Tool{{
				Name:        "wave48_visible",
				Description: "Wave 48 visible tool",
	...
	assert.Equal(t, []string{"wave48_visible"}, catalog.ToolNamesForCategory("visible"))

	section := (&catalogSourceAdapter{catalog: catalog, cfg: config.DefaultConfig()}).
		BuildToolCatalogSection("")
	assert.Contains(t, section, "**visible** (Visible tools): wave48_visible")
