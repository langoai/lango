...
func TestWave17InitP2P_InitializesLocalComponentsWithDefaultDurations(t *testing.T) {
	key, err := ethcrypto.GenerateKey()
	require.NoError(t, err)
	cfg := wave17P2PConfig(t)
	...
	t.Cleanup(func() { wave17StopP2PComponents(t, components) })
...
func TestWave17InitP2P_UsesBundleIdentityWhenSigningKeyAndLangoDirAreConfigured(t *testing.T) {
	key, err := ethcrypto.GenerateKey()
	require.NoError(t, err)
...
func TestWave17InitZKP_ReturnsProverWhenEnabledEvenIfCircuitCompileFails(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.P2P.ZKHandshake = true
	cfg.P2P.ZKAttestation = false
	cfg.P2P.ZKP.ProofCacheDir = t.TempDir()
	cfg.P2P.ZKP.ProvingScheme = "unsupported-wave17"

	prover := initZKP(cfg)
	require.NotNil(t, prover)
	assert.Equal(t, zkp.ProofScheme("unsupported-wave17"), prover.Scheme())
}

func TestWave17ReplayDispatcherAdapter_DispatchesPromptWithSessionOrigin(t *testing.T) {
	dispatcher := &fakeAdjudicationBackgroundDispatcher{taskID: "task-wave17"}
...
func TestWave17EngineEscrowReleaseAndRefundAdapters_DelegateToEngine(t *testing.T) {
	ctx := context.Background()

	t.Run("release", func(t *testing.T) {
		engine := wave17EscrowEngine()
		entry := wave17CreateFundedEscrow(t, ctx, engine)
...
func TestWave17EngineEscrowReleaseAndRefundAdapters_RejectNilEngine(t *testing.T) {
	ctx := context.Background()
...
func TestWave17AutomationModuleInit_CronOnlyRegistersSchedulerAndDisabledCategories(t *testing.T) {
	t.Parallel()
...
func TestWave17NetworkModuleInit_DisabledPaymentAndP2PRegisterPlainDisabledEntries(t *testing.T) {
	t.Parallel()
...
func wave17P2PConfig(t *testing.T) *config.Config {
	t.Helper()
	...
	cfg := config.DefaultConfig()
	cfg.A2A.AgentName = "wave17-agent"
...
func wave17StopP2PComponents(t *testing.T, components *p2pComponents) {
	t.Helper()
...
func wave17EscrowEngine() *escrow.Engine {
	return escrow.NewEngine(escrow.NewMemoryStore(), escrow.NoopSettler{}, escrow.DefaultEngineConfig())
}

func wave17CreateFundedEscrow(t *testing.T, ctx context.Context, engine *escrow.Engine) *escrow.EscrowEntry {
	t.Helper()
	...
	entry, err := engine.Create(ctx, escrow.CreateRequest{
		...
		Reason:    "wave17 escrow adapter test",
