...
type missionAwareSubmitterPropagatesSubmitAndCancelErrorsSubmitter struct {
	taskID    string
	submitErr error
}

func (s *missionAwareSubmitterPropagatesSubmitAndCancelErrorsSubmitter) Submit(context.Context, string, background.Origin) (string, error) {
	if s.submitErr != nil {
		return "", s.submitErr
	}
	return s.taskID, nil
...
type missionAwareSubmitterPropagatesSubmitAndCancelErrorsCancelFailingSubmitter struct {
	missionAwareSubmitterPropagatesSubmitAndCancelErrorsSubmitter
	cancelledID string
	cancelErr   error
}

func (s *missionAwareSubmitterPropagatesSubmitAndCancelErrorsCancelFailingSubmitter) Cancel(taskID string) error {
	s.cancelledID = taskID
	return s.cancelErr
}

func TestMissionAwareSubmitterPropagatesSubmitAndCancelErrors(t *testing.T) {
	t.Parallel()
	...
	t.Run("submit error skips mission link", func(t *testing.T) {
		...
		submitter := &missionAwareSubmitter{
			base:   &missionAwareSubmitterPropagatesSubmitAndCancelErrorsSubmitter{submitErr: wantErr},
			linker: linker,
		}
	...
	t.Run("cancel error is included after link failure", func(t *testing.T) {
		...
		base := &missionAwareSubmitterPropagatesSubmitAndCancelErrorsCancelFailingSubmitter{
			missionAwareSubmitterPropagatesSubmitAndCancelErrorsSubmitter: missionAwareSubmitterPropagatesSubmitAndCancelErrorsSubmitter{taskID: "task-pureTransferCallClassification9"},
			cancelErr: cancelErr,
		}
		submitter := &missionAwareSubmitter{
			base:   base,
		...
		require.Error(t, err)
		assert.Empty(t, taskID)
		assert.Equal(t, "task-pureTransferCallClassification9", base.cancelledID)
		assert.ErrorContains(t, err, "attach spawned child execution to mission")
		assert.ErrorContains(t, err, `cancel submitted task "task-pureTransferCallClassification9"`)
		assert.ErrorContains(t, err, cancelErr.Error())
	})
}

func TestNetworkModuleInitMarksP2PDisabledWithoutPaymentRequirement(t *testing.T) {
	t.Parallel()
...
func TestCatalogSourceAdapterTruncatesVisibleToolsAndReportsDeferredCount(t *testing.T) {
	t.Parallel()
...
func TestMetaToolsUseSettlementRuntimeAsPartialFallback(t *testing.T) {
	t.Parallel()
...
func TestCreateDisputeReadyReceiptReportsValidationError(t *testing.T) {
	t.Parallel()
...
func TestPayGateAdapterMapsQuoteContractSellerAndExpiry(t *testing.T) {
	t.Parallel()
...
func TestInitZKPDisabledReturnsNilAndAttestationCreatesProver(t *testing.T) {
	t.Parallel()
	...
	cfg.P2P.ZKP.ProvingScheme = "unsupported-pureTransferCallClassification9-scheme"

	assert.NotNil(t, initZKP(cfg))
}

func TestPayGateAdapterMapsVerifiedAuthWithoutQuote(t *testing.T) {
	t.Parallel()
