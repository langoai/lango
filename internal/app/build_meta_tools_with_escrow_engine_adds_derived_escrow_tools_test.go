...
func TestBuildMetaToolsWithEscrowEngineAddsDerivedEscrowTools(t *testing.T) {
	t.Parallel()
...
func TestBuildMetaToolsWithSettlementRuntimeDerivesPartialRuntime(t *testing.T) {
	t.Parallel()

	runtime := &buildMetaToolsWithEscrowEngineAddsDerivedEscrowToolsSettlementRuntime{}
...
func TestPathInsideDirAcceptsRootAndRejectsSiblingEscapes(t *testing.T) {
	t.Parallel()
...
func TestNetworkModuleInitPaymentDisabledKeepsWorkspaceAndP2PDisabled(t *testing.T) {
	t.Parallel()

	cfg := buildMetaToolsWithEscrowEngineAddsDerivedEscrowToolsModuleConfig(t)
...
func TestInitSecurityUnsupportedProviderIsReported(t *testing.T) {
	t.Parallel()

	cfg := config.DefaultConfig()
	cfg.Security.Signer.Provider = "buildMetaToolsWithEscrowEngineAddsDerivedEscrowTools5-kms"

	provider, keys, secrets, err := initSecurity(cfg, &stubSessionStore{}, nil)

	require.Error(t, err)
	assert.ErrorContains(t, err, `unsupported security provider "buildMetaToolsWithEscrowEngineAddsDerivedEscrowTools5-kms"`)
	assert.Nil(t, provider)
	assert.Nil(t, keys)
	assert.Nil(t, secrets)
}

type buildMetaToolsWithEscrowEngineAddsDerivedEscrowToolsSettlementRuntime struct{}

func (r *buildMetaToolsWithEscrowEngineAddsDerivedEscrowToolsSettlementRuntime) ExecuteSettlement(
	context.Context,
	settlementexecution.DirectPaymentRequest,
) (settlementexecution.DirectPaymentResult, error) {
	return settlementexecution.DirectPaymentResult{Reference: "buildMetaToolsWithEscrowEngineAddsDerivedEscrowTools5-settlement"}, nil
}

func buildMetaToolsWithEscrowEngineAddsDerivedEscrowToolsModuleConfig(t *testing.T) *config.Config {
	t.Helper()
