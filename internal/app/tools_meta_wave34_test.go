...
func TestWave34BuildMetaTools_ConstructsDeterministicReceiptBackedSurface(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, wave34ToolNames(tools), wave34ToolNames(repeated))
...
func wave34ToolNames(tools []*agent.Tool) []string {
...
func TestWave34EvaluateExportability_RejectsUnavailableStoreBeforeParams(t *testing.T) {
	t.Parallel()
...
func TestWave34ExportabilityHelpers_DefaultPolicyAndSourceRefBranches(t *testing.T) {
	t.Parallel()
...
func TestWave34ExportabilityReceiptPayload_MapsLineageRows(t *testing.T) {
	t.Parallel()

	payload := exportabilityReceiptPayload("artifact/wave34", exportability.Receipt{
	...
	assert.Equal(t, "artifact/wave34", payload["artifact_label"])
...
func TestWave34ApproveUpfrontPayment_CapturesEvaluatorInputAndEscalatesWithoutEscrowFields(t *testing.T) {
	t.Parallel()

	store := receipts.NewStore()
	ctx := context.Background()
	submission, tx, err := store.CreateSubmissionReceipt(ctx, receipts.CreateSubmissionInput{
		TransactionID:       "tx-wave34-payment",
		ArtifactLabel:       "artifact/wave34-payment",
		PayloadHash:         "hash-wave34-payment",
		SourceLineageDigest: "lineage-wave34-payment",
	})
	require.NoError(t, err)
...
func TestWave34ParseEscrowExecutionInput_RejectsMalformedMilestoneShapes(t *testing.T) {
	t.Parallel()
