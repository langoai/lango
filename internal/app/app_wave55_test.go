...
func TestWave55NetworkModuleMetadataAndEnablementBranches(t *testing.T) {
	t.Parallel()
...
func TestWave55P2PPaymentToolEarlyValidationAvoidsChainRPC(t *testing.T) {
	t.Parallel()
	...
	got, err := tool.Handler(context.Background(), map[string]interface{}{
		...
		"transaction_receipt_id": "tx-wave55-missing-session",
		"amount":                 "0.50",
	})
	require.Error(t, err)
	assert.Nil(t, got)
	...
	got, err = tool.Handler(context.Background(), map[string]interface{}{
		"peer_did":               "not-a-did",
		"transaction_receipt_id": "tx-wave55-bad-did",
		"amount":                 "0.50",
	})
	require.Error(t, err)
	assert.Nil(t, got)
...
func TestWave55MetaPostAdjudicationAdaptersHandleNilDependencies(t *testing.T) {
	t.Parallel()
	...
	receipt, err := (replayDispatcherAdapter{}).Dispatch(context.Background(), postadjudicationreplay.BackgroundDispatchRequest{
		TransactionReceiptID: "tx-wave55",
		SubmissionReceiptID:  "sub-wave55",
		EscrowReference:      "escrow-wave55",
...
func TestWave55P2PWiringPureAdapterBranchesAvoidNodeStartup(t *testing.T) {
	t.Parallel()
	...
	dispatcher := &wave55BackgroundDispatcher{taskID: "task-wave55"}
	dispatchReceipt, err := (replayDispatcherAdapter{dispatcher: dispatcher}).Dispatch(context.Background(), postadjudicationreplay.BackgroundDispatchRequest{
		TransactionReceiptID: "tx-wave55-dispatch",
		SubmissionReceiptID:  "sub-wave55-dispatch",
		EscrowReference:      "escrow-wave55-dispatch",
		Outcome:              receipts.EscrowAdjudicationRefund,
		Prompt:               "dispatch refund",
	})
	require.NoError(t, err)
	assert.Equal(t, "queued", dispatchReceipt.Status)
	assert.Equal(t, "task-wave55", dispatchReceipt.DispatchReference)
	assert.Equal(t, "dispatch refund", dispatcher.prompt)
}

func TestWave55WiringHelpersResolveModeHintAndAutomationPrompt(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, "stay deterministic", resolver.LookupModeHint("wave55"))
	assert.Empty(t, resolver.LookupModeHint("missing"))
	assert.Empty(t, (*modeResolverAdapter)(nil).LookupModeHint("wave55"))
...
type wave55BackgroundDispatcher struct {
	taskID string
	prompt string
}

func (d *wave55BackgroundDispatcher) Submit(_ context.Context, prompt string, _ background.Origin) (string, error) {
	d.prompt = prompt
	return d.taskID, nil
}

func (d *wave55BackgroundDispatcher) List() []background.TaskSnapshot {
	return []background.TaskSnapshot{{ID: d.taskID, StatusText: "queued", RetryKey: "retry-wave55"}}
}
