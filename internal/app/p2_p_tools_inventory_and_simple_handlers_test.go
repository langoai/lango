...
func TestP2PToolsInventoryAndSimpleHandlers(t *testing.T) {
	t.Parallel()

	sessions, err := handshake.NewSessionStore(time.Hour)
	require.NoError(t, err)
	_, err = sessions.Create("did:lango:peer-p2PToolsInventoryAndSimpleHandlers1", true)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	peers := p2PToolsInventoryAndSimpleHandlersPayload(t, result)
	...
	result, err = findP2PTool(t, tools, "p2p_firewall_add").Handler(context.Background(), map[string]interface{}{
		"peer_did":   "did:lango:peer-p2PToolsInventoryAndSimpleHandlers1",
		"action":     "allow",
		"tools":      []interface{}{"search_knowledge", "payment_balance"},
		"rate_limit": float64(3),
	})
	require.NoError(t, err)
	assert.Equal(t, "added", p2PToolsInventoryAndSimpleHandlersPayload(t, result)["status"])

	result, err = findP2PTool(t, tools, "p2p_firewall_rules").Handler(context.Background(), map[string]interface{}{})
	require.NoError(t, err)
	rules := p2PToolsInventoryAndSimpleHandlersPayload(t, result)
	...
	require.NoError(t, err)
	assert.Equal(t, 1, p2PToolsInventoryAndSimpleHandlersPayload(t, result)["count"])

	result, err = findP2PTool(t, tools, "p2p_discover").Handler(context.Background(), map[string]interface{}{"capability": "search"})
	require.NoError(t, err)
	discovery := p2PToolsInventoryAndSimpleHandlersPayload(t, result)
	...
	require.NoError(t, err)
	assert.Equal(t, "disconnected", p2PToolsInventoryAndSimpleHandlersPayload(t, result)["status"])
	assert.Nil(t, sessions.Get("did:lango:peer-p2PToolsInventoryAndSimpleHandlers1"))
}

func TestAuthToMapSerializesAuthorizationFields(t *testing.T) {
	t.Parallel()
...
func TestP2PPaidInvokeFreeAndPaymentBranches(t *testing.T) {
	originalRemoteAgent := newP2PPaidInvokeRemoteAgent
	t.Cleanup(func() {
		newP2PPaidInvokeRemoteAgent = originalRemoteAgent
	})

	did := p2PToolsInventoryAndSimpleHandlersPeerDID(t)
	sessions, err := handshake.NewSessionStore(time.Hour)
	require.NoError(t, err)
	_, err = sessions.Create(did.ID, false)
	require.NoError(t, err)

	remote := &p2PToolsInventoryAndSimpleHandlersPaidInvokeRemoteAgent{
	...
	require.NoError(t, err)
	payload := p2PToolsInventoryAndSimpleHandlersPayload(t, result)
...
func TestP2PPaidInvokePaidResponseBranches(t *testing.T) {
	originalRemoteAgent := newP2PPaidInvokeRemoteAgent
	t.Cleanup(func() {
		newP2PPaidInvokeRemoteAgent = originalRemoteAgent
	})
...
type p2PToolsInventoryAndSimpleHandlersPaidInvokeRemoteAgent struct {
	quote        *protocol.PriceQuoteResult
...
func (a *p2PToolsInventoryAndSimpleHandlersPaidInvokeRemoteAgent) QueryPrice(context.Context, string) (*protocol.PriceQuoteResult, error) {
	return a.quote, nil
}

func (a *p2PToolsInventoryAndSimpleHandlersPaidInvokeRemoteAgent) InvokeTool(_ context.Context, _ string, params map[string]interface{}) (map[string]interface{}, error) {
	a.invokeParams = params
	return a.invokeResult, nil
}

func (a *p2PToolsInventoryAndSimpleHandlersPaidInvokeRemoteAgent) InvokeToolPaid(_ context.Context, _ string, _ map[string]interface{}, auth map[string]interface{}) (*protocol.Response, error) {
	a.paidInvoked = true
	a.lastAuth = auth
	return a.paidResponse, nil
}

func p2PToolsInventoryAndSimpleHandlersPeerDID(t *testing.T) *identity.DID {
	t.Helper()
...
func p2PToolsInventoryAndSimpleHandlersPayload(t *testing.T, result interface{}) map[string]interface{} {
	t.Helper()
