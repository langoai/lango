...
func TestWave27BuildMetaToolsWithEscrowEngineRegistersEngineBackedHelpers(t *testing.T) {
	t.Parallel()
...
func TestWave27RuntimeBackedMetaToolsRejectWhitespaceTransactionReceiptID(t *testing.T) {
	t.Parallel()
	...
	tools := buildMetaToolsWithRuntimes(
		...
		receipts.NewStore(),
		fakeWave21EscrowExecutionRuntime{},
...
func TestWave27AdjudicateEscrowDisputeBackgroundMissingDispatcherReturnsCanonicalReceiptAndError(t *testing.T) {
	t.Parallel()

	store := receipts.NewStore()
	ctx := context.Background()
	tx := wave27PrepareHeldEscrowTransaction(t, store, ctx, "deal-wave27-background-missing")
...
func TestWave27AdjudicateEscrowDisputeBackgroundSubmitFailureReturnsCanonicalReceipt(t *testing.T) {
	t.Parallel()

	store := receipts.NewStore()
	ctx := context.Background()
	tx := wave27PrepareHeldEscrowTransaction(t, store, ctx, "deal-wave27-background-submit-failure")
...
func TestWave27AdjudicateEscrowDisputeInlineMissingRuntimeReturnsErrorAfterAdjudication(t *testing.T) {
	t.Parallel()

	store := receipts.NewStore()
	ctx := context.Background()
	tx := wave27PrepareHeldEscrowTransaction(t, store, ctx, "deal-wave27-inline-missing-runtime")
	tool := findTool(buildMetaToolsWithRuntimes(nil, nil, nil, config.SkillConfig{}, nil, store, nil, nil, nil, nil, nil, nil), "adjudicate_escrow_dispute")
	require.NotNil(t, tool)
...
func TestWave27EngineEscrowReleaseAndRefundRuntimeSurfaceEngineErrors(t *testing.T) {
	t.Parallel()
...
func wave27PrepareHeldEscrowTransaction(t *testing.T, store *receipts.Store, ctx context.Context, transactionID string) receipts.TransactionReceipt {
	t.Helper()
