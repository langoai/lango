...
func TestWave34NetworkModuleDisabledFlagSkipsRuntimeServices(t *testing.T) {
	t.Parallel()
...
func TestWave34NetworkModuleInitPaymentRequiredCatalogWithoutExternalServices(t *testing.T) {
	t.Parallel()
	...
	result, err := module.Init(context.Background(), staticResolver{
		appinit.ProvidesSupervisor: wave34FoundationValues(),
	})
...
func TestWave34IntelligenceModuleInitDisabledCatalogWithoutLifecycleComponents(t *testing.T) {
	t.Parallel()
	...
	assert.Nil(t, wave34CatalogEntry(result.CatalogEntries, "ontology"))
}

func wave34ModuleConfig(t *testing.T) *config.Config {
	t.Helper()
...
func wave34FoundationValues() *foundationValues {
