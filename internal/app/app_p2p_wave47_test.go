...
func TestWave47P2PToolsSessionBranchesAvoidNetwork(t *testing.T) {
	t.Parallel()

	sessions, err := handshake.NewSessionStore(time.Hour)
	require.NoError(t, err)
	verified, err := sessions.Create("did:lango:wave47-verified", true)
	require.NoError(t, err)
	unverified, err := sessions.Create("did:lango:wave47-unverified", false)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	peerPayload := wave28P2PPayload(t, peers)
...
func TestWave47P2PToolsRejectMissingSessionsBeforePeerWork(t *testing.T) {
	t.Parallel()
...
func TestWave47P2PToolsFirewallMutationAndFormatting(t *testing.T) {
	t.Parallel()
	...
	added, err := findP2PTool(t, tools, "p2p_firewall_add").Handler(context.Background(), map[string]interface{}{
		"peer_did":   "did:lango:wave47-peer",
		"action":     "allow",
	...
	assert.Equal(t, map[string]interface{}{
		"status":  "added",
		"message": "Firewall rule added: allow did:lango:wave47-peer",
	}, added)

	listed, err := findP2PTool(t, tools, "p2p_firewall_rules").Handler(context.Background(), nil)
	require.NoError(t, err)
	payload := wave28P2PPayload(t, listed)
	...
	assert.Equal(t, map[string]interface{}{
		"status":  "removed",
		"count":   1,
		"message": "Removed 1 rules for did:lango:wave47-peer",
	}, removed)
	assert.Len(t, fw.Rules(), 1)
}

func TestWave47P2PToolsUnavailableDependenciesReturnDeterministicResponses(t *testing.T) {
	t.Parallel()
...
func TestWave47P2PWiringShortCircuitsBeforeListeners(t *testing.T) {
	t.Parallel()
