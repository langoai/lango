...
type fakeWave21EscrowExecutionRuntime struct {
	createErr error
	fundErr   error
}

func (f fakeWave21EscrowExecutionRuntime) Create(_ context.Context, _ escrow.CreateRequest) (*escrow.EscrowEntry, error) {
	if f.createErr != nil {
		return nil, f.createErr
	}
	return &escrow.EscrowEntry{ID: "escrow-wave21"}, nil
}

func (f fakeWave21EscrowExecutionRuntime) Fund(_ context.Context, id string) (*escrow.EscrowEntry, error) {
	if f.fundErr != nil {
...
func TestWave21BuildMetaToolsWithRuntimes_RuntimeToolComposition(t *testing.T) {
	t.Parallel()
...
func TestWave21ReplayDispatcherAdapter_ErrorBranches(t *testing.T) {
	t.Parallel()
...
func TestWave21RuntimeBackedToolHandlers_RejectNilReceiptStoreBeforeParams(t *testing.T) {
	t.Parallel()
...
func TestWave21CreateDisputeReadyReceipt_ReturnsLinkedReceiptIDs(t *testing.T) {
	t.Parallel()

	store := receipts.NewStore()
	got, err := createDisputeReadyReceipt(context.Background(), store, receipts.CreateSubmissionInput{
		TransactionID:       "tx-wave21-dispute-ready",
		ArtifactLabel:       "artifact/wave21",
		PayloadHash:         "hash-wave21",
		SourceLineageDigest: "lineage-wave21",
	})
...
func TestWave21ReceiptConstructors_MapBranchSpecificFields(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, &adjudicateEscrowDisputeBackgroundDispatchReceipt{
		...
		EscrowReference:      "escrow-adjudicate",
		Outcome:              "refund",
		DispatchReference:    "task-wave21",
	}, newAdjudicateEscrowDisputeBackgroundDispatchReceipt(adjudication, "task-wave21"))
