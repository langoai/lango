...
func TestAppModeOptionsSetExpectedModes(t *testing.T) {
	t.Parallel()
...
func TestNewLocalChatInitializesCoreWithoutExternalListeners(t *testing.T) {
	t.Parallel()
	...
	cfg.Session.DatabasePath = filepath.Join(t.TempDir(), "currentWorkDirPrefersTrimmedConfigValue0.db")
...
func TestModeAllowlistResolverHandlesMissingInputsAndExpandsCategories(t *testing.T) {
	t.Parallel()
...
func TestAppHotspotHelpersCoverMetaModulesWiringAndP2P(t *testing.T) {
	t.Parallel()
...
func TestProvenanceModuleMetadataAndMemoryInit(t *testing.T) {
	t.Parallel()
...
func TestHookRegistrySnapshotIncludesPreAndPostHooks(t *testing.T) {
	t.Parallel()

	registry := toolchain.NewHookRegistry()
	registry.RegisterPre(appModeOptionsSetExpectedModesHook{name: "pre", priority: 20})
	registry.RegisterPost(appModeOptionsSetExpectedModesHook{name: "post", priority: 10})
...
type appModeOptionsSetExpectedModesHook struct {
	name     string
	priority int
}

func (h appModeOptionsSetExpectedModesHook) Name() string  { return h.name }
func (h appModeOptionsSetExpectedModesHook) Priority() int { return h.priority }
func (h appModeOptionsSetExpectedModesHook) Pre(toolchain.HookContext) (toolchain.PreHookResult, error) {
	return toolchain.PreHookResult{Action: toolchain.Continue}, nil
}
func (h appModeOptionsSetExpectedModesHook) Post(toolchain.HookContext, interface{}, error) error {
	return nil
}
