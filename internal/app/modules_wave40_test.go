...
func TestWave40FoundationModuleInitContinuesAfterSanitizerPatternError(t *testing.T) {
	t.Parallel()
...
func TestWave40IntelligenceModuleInitAgentMemoryCatalogWithoutStorage(t *testing.T) {
	t.Parallel()
	...
	result, err := (&intelligenceModule{cfg: cfg, bus: eventbus.New()}).Init(
		...
		staticResolver{
			...
			appinit.ProvidesBaseTools: []*agent.Tool{{Name: "wave40_base"}},
		},
	)
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestWave40AutomationModuleInitAllOptionalSubsystemsDisabledCatalog(t *testing.T) {
	t.Parallel()
...
func TestWave40NetworkModuleInitFullyDisabledCatalogWithoutServices(t *testing.T) {
	t.Parallel()
	...
	assert.Nil(t, wave40CatalogEntry(result.CatalogEntries, "workspace"))
}

func TestWave40ExtensionModuleInitNoMCPServersAndIgnoredObservabilitySubfeatures(t *testing.T) {
	tmp := t.TempDir()
	origWD, err := os.Getwd()
	require.NoError(t, err)
...
func wave40ModuleConfig(t *testing.T) *config.Config {
	t.Helper()
...
func wave40CatalogEntry(entries []appinit.CatalogEntry, category string) *appinit.CatalogEntry {
	for i := range entries {
