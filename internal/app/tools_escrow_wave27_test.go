...
func TestWave27EscrowHubToolsAttachOnChainFieldsWithoutLiveChain(t *testing.T) {
	ctx := context.Background()
	caller := newWave27ContractCaller()
	...
	rig := newWave27EscrowToolRig(settler)
	...
	fund := wave27EscrowPayload(t, fundResult)
	...
	assertWave27BigIntArg(t, deposit.Args, 0, 27)
	...
	submitResult, err := rig.tool("escrow_submit_work").Handler(ctx, map[string]interface{}{
		"escrowId": escrowID,
		"workHash": "wave27-proof",
	})
	require.NoError(t, err)
	submit := wave27EscrowPayload(t, submitResult)
	assert.Equal(t, "wave27-proof", submit["workHash"])
	...
	assertWave27BigIntArg(t, submitReq.Args, 0, 27)
	assert.Equal(t, sha256.Sum256([]byte("wave27-proof")), submitReq.Args[1])

	statusResult, err := rig.tool("escrow_status").Handler(ctx, map[string]interface{}{"escrowId": escrowID})
	require.NoError(t, err)
	status := wave27EscrowPayload(t, statusResult)
	...
	require.NoError(t, err)
	dispute := wave27EscrowPayload(t, disputeResult)
	...
	assertWave27BigIntArg(t, disputeReq.Args, 0, 27)
	...
	require.NoError(t, err)
	resolve := wave27EscrowPayload(t, resolveResult)
	...
	assertWave27BigIntArg(t, resolveReq.Args, 0, 27)
	assert.Equal(t, true, resolveReq.Args[1])
	assertWave27BigIntArg(t, resolveReq.Args, 2, 12_500_000)
	assertWave27BigIntArg(t, resolveReq.Args, 3, 0)
}

func TestWave27EscrowHubReleaseAndRefundUseLocalMappings(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("release", func(t *testing.T) {
		...
		require.NoError(t, err)
		payload := wave27EscrowPayload(t, got)
		...
		assertWave27BigIntArg(t, releaseReq.Args, 0, 31)
	})
	...
	t.Run("refund", func(t *testing.T) {
		...
		assertWave27BigIntArg(t, refundReq.Args, 0, 32)
	})
}

func TestWave27EscrowVaultToolsAttachOnChainFieldsWithoutLiveChain(t *testing.T) {
	ctx := context.Background()
	caller := newWave27ContractCaller()
	...
	assert.Equal(t, sha256.Sum256([]byte("wave27-vault-proof")), submitReq.Args[0])
...
func TestWave27EscrowValidationBranchesRejectMissingRequiredInputs(t *testing.T) {
	t.Parallel()

	rig := newWave27EscrowToolRig(escrow.NoopSettler{})
	tests := []struct {
		name    string
		tool    string
...
type wave27EscrowToolRig struct {
	engine *escrow.Engine
	tools  map[string]*agent.Tool
}

func newWave27EscrowToolRig(settler escrow.SettlementExecutor) wave27EscrowToolRig {
	...
	return wave27EscrowToolRig{engine: engine, tools: tools}
}

func (r wave27EscrowToolRig) tool(name string) *agent.Tool {
	return r.tools[name]
}

func (r wave27EscrowToolRig) createEscrow(t *testing.T, ctx context.Context, buyerDID, sellerDID string) string {
	t.Helper()
	...
	require.NoError(t, err)
	payload := wave27EscrowPayload(t, result)
...
func (r wave27EscrowToolRig) completeAllMilestones(t *testing.T, ctx context.Context, escrowID string) {
	t.Helper()

	entry, err := r.engine.Get(escrowID)
	require.NoError(t, err)
	for _, milestone := range entry.Milestones {
		_, err = r.engine.CompleteMilestone(ctx, escrowID, milestone.ID, "wave27 accepted")
		require.NoError(t, err)
	}
}

func wave27EscrowPayload(t *testing.T, result interface{}) map[string]interface{} {
	t.Helper()
...
type wave27ContractCaller struct {
	mu     sync.Mutex
...
func newWave27ContractCaller() *wave27ContractCaller {
	return &wave27ContractCaller{}
}

func (c *wave27ContractCaller) Read(_ context.Context, req contract.ContractCallRequest) (*contract.ContractCallResult, error) {
	c.mu.Lock()
...
func (c *wave27ContractCaller) Write(_ context.Context, req contract.ContractCallRequest) (*contract.ContractCallResult, error) {
	c.mu.Lock()
	defer c.mu.Unlock()

	c.nextTx++
	c.writes = append(c.writes, req)
	result := &contract.ContractCallResult{TxHash: fmt.Sprintf("0xwave27%s%02d", req.Method, c.nextTx)}
...
func (c *wave27ContractCaller) sawWrite(method string) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *wave27ContractCaller) sawRead(method string) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *wave27ContractCaller) requireWrite(t *testing.T, method string) contract.ContractCallRequest {
	t.Helper()
...
func (c *wave27ContractCaller) requireRead(t *testing.T, method string) contract.ContractCallRequest {
	t.Helper()
...
func (c *wave27ContractCaller) requireWriteWithBigIntArg(t *testing.T, method string, index int, want int64) contract.ContractCallRequest {
	t.Helper()
...
func assertWave27BigIntArg(t *testing.T, args []interface{}, index int, want int64) {
	t.Helper()
