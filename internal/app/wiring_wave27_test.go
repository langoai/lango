...
func TestWave27InitSessionStorePrefersBootstrapStorageStore(t *testing.T) {
	t.Parallel()
...
func TestWave27InitSessionStoreFallsBackWhenBootstrapStorageCannotOpenStore(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Session.DatabasePath = filepath.Join(t.TempDir(), "fallback.db")
