...
func TestBuildMetaToolsWithRuntimesRegistersReceiptBackedToolsOnlyWithReceiptStore(t *testing.T) {
	t.Parallel()
...
func TestImportSkillDisabledShortCircuitsBeforeURLValidation(t *testing.T) {
	t.Parallel()
...
func TestInitAgentRegistryLoadsEmbeddedAndUserAgents(t *testing.T) {
	t.Parallel()

	agentsDir := t.TempDir()
	userAgentDir := filepath.Join(agentsDir, "prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9")
	require.NoError(t, os.MkdirAll(userAgentDir, 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(userAgentDir, "AGENT.md"), []byte(`---
name: prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9
description: "Deterministic receipt-backed test agent"
status: active
prefixes:
  - buildMetaToolsWithRuntimesRegistersReceiptBackedToolsOnlyWithReceiptStore_
---
...
`), 0o644))
	...
	def, ok := registry.Get("prepareTuiStartupInitializesLoggingAndRedirectsStdlibLog9")
	require.True(t, ok)
	assert.Equal(t, "Deterministic receipt-backed test agent", def.Description)
	assert.Equal(t, agentregistry.SourceUser, def.Source)
	assert.Contains(t, def.Prefixes, "buildMetaToolsWithRuntimesRegistersReceiptBackedToolsOnlyWithReceiptStore_")
...
func TestNetworkModuleEnabledFollowsNetworkFeatureFlags(t *testing.T) {
	t.Parallel()
...
func TestStopReturnsContextErrorWhenBackgroundWorkDoesNotDrain(t *testing.T) {
