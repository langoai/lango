...
func TestFoundationCatalogEntriesOmitWebCategoryWithoutWebTools(t *testing.T) {
	t.Parallel()
	...
	assert.Nil(t, foundationCatalogEntriesOmitWebCategoryWithoutWebToolsCatalogEntry(entries, "web"))
}

func TestAutomationAgentRunStoreMirrorPreconditions(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			configure: enableFoundationCatalogEntriesOmitWebCategoryWithoutWebToolsRunLedgerMirrorConfig,
		},
		{
...
func TestCatalogSourceModeWithNilConfigFallsBackToVisibleTools(t *testing.T) {
	t.Parallel()
...
func TestBuildSubAgentPromptFuncDefaultPathStripsGlobalSections(t *testing.T) {
	t.Parallel()
...
func enableFoundationCatalogEntriesOmitWebCategoryWithoutWebToolsRunLedgerMirrorConfig(cfg *config.Config) {
	cfg.RunLedger.Enabled = true
	cfg.RunLedger.WriteThrough = true
}

func foundationCatalogEntriesOmitWebCategoryWithoutWebToolsCatalogEntry(entries []appinit.CatalogEntry, category string) *appinit.CatalogEntry {
	for i := range entries {
