...
func TestPolicyBusAdapter_PublishForwardsNonPolicyDecisionEvents(t *testing.T) {
	...
	bus := eventbus.New()
	var got appWave16PolicyEvent
	bus.Subscribe(eventbus.EventPolicyDecision, func(evt eventbus.Event) {
		got = evt.(appWave16PolicyEvent)
	})

	adapter := &policyBusAdapter{bus: bus}
	adapter.Publish(appWave16PolicyEvent{value: "raw-event"})

	assert.Equal(t, appWave16PolicyEvent{value: "raw-event"}, got)
}
...
type appWave16PolicyEvent struct {
	value string
}

func (e appWave16PolicyEvent) EventName() string {
	return eventbus.EventPolicyDecision
}

type appWave16RecordingChannel struct {
	mu         sync.Mutex
	started    chan struct{}
	stopped    chan struct{}
	startCount int
...
func (c *appWave16RecordingChannel) Name() string {
	return "recording"
}

func (c *appWave16RecordingChannel) Start(context.Context) error {
	c.mu.Lock()
	c.startCount++
	if c.startCount == 1 {
		close(c.started)
...
func (c *appWave16RecordingChannel) Stop(context.Context) error {
	c.mu.Lock()
	c.stopCount++
	if c.stopCount == 1 {
		close(c.stopped)
