...
func TestWave38ViewSkillRejectsSymlinkEscape(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	skillsDir := filepath.Join(t.TempDir(), "skills")
	store := skill.NewFileSkillStore(skillsDir, zap.NewNop().Sugar())
	registry := skill.NewRegistry(store, []*agent.Tool{{Name: "wave38_base_tool"}}, zap.NewNop().Sugar())
	require.NoError(t, registry.CreateSkill(ctx, skill.SkillEntry{
		Name:        "wave38-skill",
		Description: "Wave 38 skill",
		Type:        skill.SkillTypeInstruction,
		Context:     "safe skill context",
	}))
	require.NoError(t, registry.ActivateSkill(ctx, "wave38-skill"))

	outside := filepath.Join(t.TempDir(), "outside.md")
	require.NoError(t, os.WriteFile(outside, []byte("outside"), 0o600))
	require.NoError(t, os.Symlink(outside, filepath.Join(skillsDir, "wave38-skill", "leak.md")))
...
func TestWave38ExtensionModuleInitObservabilityEnabledWithoutExternalServers(t *testing.T) {
	t.Parallel()
	...
	assert.Nil(t, wave34CatalogEntry(result.CatalogEntries, "observability"))
}

func TestWave38InitAgentRegistryIgnoresBrokenUserStoreAndKeepsEmbeddedAgents(t *testing.T) {
	t.Parallel()
...
func TestWave38WalletHandshakeSignerDelegatesDirectMethodsWithoutNetwork(t *testing.T) {
	t.Parallel()
