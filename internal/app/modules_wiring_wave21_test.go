...
func TestWave21BuildCatalogFromEntriesRegistersCategoriesAndTools(t *testing.T) {
	t.Parallel()
...
func TestWave21PopulateAppFieldsPublishesFeatureStatusesToGatewayHealth(t *testing.T) {
	t.Parallel()
	...
	statuses.Add(&types.FeatureStatus{
		...
		Enabled: true,
		Healthy: true,
		Reason:  "wave21 deterministic status",
	})
	...
	assert.Equal(t, "wave21 deterministic status", feature["reason"])
	assert.Same(t, statuses, application.FeatureStatuses)
}

func TestWave21WirePostAgentWiresP2PHandlerExecutorAndSafetyGate(t *testing.T) {
	t.Parallel()
	...
	catalog.RegisterCategory(toolcatalog.Category{
		Name:        "wave21",
		Description: "Wave 21 tools",
		Enabled:     true,
	})
	catalog.Register("wave21", []*agent.Tool{
		{Name: "wave21_safe", Description: "safe", SafetyLevel: agent.SafetyLevelSafe},
		{Name: "wave21_dangerous", Description: "dangerous", SafetyLevel: agent.SafetyLevelDangerous},
	})
...
func TestWave21ModeResolverAndCatalogSourceHandleNilAndMissingInputs(t *testing.T) {
	t.Parallel()
...
func TestWave21NetworkModuleInitRegistersDisabledEntriesWhenPaymentUnavailable(t *testing.T) {
	t.Parallel()
...
func TestWave21InitP2PSkipsDisabledAndMissingWalletBeforeNetworkSetup(t *testing.T) {
	t.Parallel()
