...
func TestServerRunDispatchesConfigAndRecallRequests(t *testing.T) {
	t.Parallel()

	srv := openServerSecurityAndConfigWrappersServer(t, true)
	cfg := serverSecurityAndConfigWrappersConfig(t, "runChatStartErrorStopsBeforeSessionAndTui6-model")
	cfg.Agent.Provider = "runChatStartErrorStopsBeforeSessionAndTui6-provider"

	responses := runServerRunDispatchesConfigAndRecallRequestsRequests(t, srv, []Request{
		{ID: 4601, Method: methodHealth, DeadlineMS: 50},
		{
			ID:     4602,
			Method: methodConfigSave,
			Payload: mustPayload(t, ConfigSaveRequest{
				Name:         "runChatStartErrorStopsBeforeSessionAndTui6",
		...
		},
		{
			...
			Payload: mustPayload(t, ConfigExistsRequest{Name: "runChatStartErrorStopsBeforeSessionAndTui6"}),
		},
		{
			ID:      4604,
			Method:  methodConfigSetActive,
			Payload: mustPayload(t, ConfigSetActiveRequest{Name: "runChatStartErrorStopsBeforeSessionAndTui6"}),
		},
		{ID: 4605, Method: methodConfigLoadActive},
	})

	require.Len(t, responses, 5)
	require.True(t, decodeServerRunDispatchesConfigAndRecallRequestsResult[HealthResult](t, responses[0]).Opened)
	require.Equal(t, map[string]bool{"saved": true}, decodeServerRunDispatchesConfigAndRecallRequestsResult[map[string]bool](t, responses[1]))
	require.True(t, decodeServerRunDispatchesConfigAndRecallRequestsResult[ConfigExistsResult](t, responses[2]).Exists)
	require.Equal(t, map[string]bool{"active": true}, decodeServerRunDispatchesConfigAndRecallRequestsResult[map[string]bool](t, responses[3]))

	active := decodeServerRunDispatchesConfigAndRecallRequestsResult[ConfigLoadActiveResult](t, responses[4])
	require.Equal(t, "runChatStartErrorStopsBeforeSessionAndTui6", active.Name)
	...
	require.Equal(t, "runChatStartErrorStopsBeforeSessionAndTui6-provider", loaded.Agent.Provider)
	require.Equal(t, "runChatStartErrorStopsBeforeSessionAndTui6-model", loaded.Agent.Model)

	base := time.Unix(4600, 0).UTC()
	sess := session.Session{
		Key:   "runChatStartErrorStopsBeforeSessionAndTui6-session",
		Model: "runChatStartErrorStopsBeforeSessionAndTui6-test-model",
		History: []session.Message{
			{Role: types.RoleUser, Content: "remember deterministic recall topic", Timestamp: base},
	...
	responses = runServerRunDispatchesConfigAndRecallRequestsRequests(t, srv, []Request{
		...
		},
		{
			...
			Payload: mustPayload(t, RecallSearchRequest{Query: "recall topic", Limit: 5}),
		},
		{
			ID:      4615,
	...
	search := decodeServerRunDispatchesConfigAndRecallRequestsResult[RecallSearchResult](t, responses[4])
	require.NotEmpty(t, search.Results)
	require.Equal(t, sess.Key, search.Results[0].RowID)
	require.NotZero(t, search.Results[0].Rank)

	summary := decodeServerRunDispatchesConfigAndRecallRequestsResult[RecallSummaryResult](t, responses[5])
	require.Contains(t, summary.Summary, "remember deterministic recall topic")
}

func TestServerPaymentAndWorkflowDefaultLimitBranches(t *testing.T) {
	t.Parallel()
	...
	for i := 0; i < 22; i++ {
		require.NoError(t, srv.client.WorkflowRun.Create().
			SetWorkflowName(fmt.Sprintf("runChatStartErrorStopsBeforeSessionAndTui6-flow-%02d", i)).
	...
	require.NoError(t, srv.client.PaymentTx.Create().
		...
		Exec(ctx))
	require.NoError(t, srv.client.PaymentTx.Create().
		SetTxHash("0xpayment-failed-ignored").
		SetFromAddress("0xfrom").
		SetToAddress("0xto").
		SetAmount("99.00").
		SetChainID(8453).
	...
	workflows := decodeServerRunDispatchesConfigAndRecallRequestsResult[WorkflowRunsResult](t, responses[0])
	require.Len(t, workflows.Runs, 20)
	require.Equal(t, "runChatStartErrorStopsBeforeSessionAndTui6-flow-21", workflows.Runs[0].WorkflowName)
	require.Equal(t, string(workflowrun.StatusCompleted), workflows.Runs[0].Status)
	require.Equal(t, 5, workflows.Runs[0].TotalSteps)

	history := decodeServerRunDispatchesConfigAndRecallRequestsResult[PaymentHistoryResult](t, responses[1])
	require.Len(t, history.Entries, 20)
	require.Equal(t, "0xpayment-21", history.Entries[0].TxHash)
	...
	usage := decodeServerRunDispatchesConfigAndRecallRequestsResult[PaymentUsageResult](t, responses[2])
	require.Equal(t, "2.20", usage.DailySpent)
}

func TestServerTargetedErrorBranches(t *testing.T) {
	t.Parallel()
	...
	noMasterKey := openServerSecurityAndConfigWrappersServer(t, false)
	resp := noMasterKey.handle(Request{ID: 4640, Method: methodConfigList})
	require.False(t, resp.OK)
	require.Contains(t, resp.Error, "master key not initialized")
}

func runServerRunDispatchesConfigAndRecallRequestsRequests(t *testing.T, srv *Server, requests []Request) []Response {
	t.Helper()
...
func decodeServerRunDispatchesConfigAndRecallRequestsResult[T any](t *testing.T, resp Response) T {
	t.Helper()
	require.True(t, resp.OK, "response %d failed: %s", resp.ID, resp.Error)
