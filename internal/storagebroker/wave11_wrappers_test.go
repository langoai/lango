...
func TestWave11ServerSecurityAndConfigWrappers(t *testing.T) {
	t.Parallel()
	...
	_, err = srv.dispatch(ctx, Request{
		...
		Payload: mustPayload(t, StoreSaltRequest{Salt: []byte("wave11-salt")}),
	})
	require.NoError(t, err)
	_, err = srv.dispatch(ctx, Request{
		Method:  methodStoreChecksum,
		Payload: mustPayload(t, StoreChecksumRequest{Checksum: []byte("wave11-checksum")}),
	})
	require.NoError(t, err)
	...
	require.Equal(t, []byte("wave11-salt"), state.Salt)
	require.Equal(t, []byte("wave11-checksum"), state.Checksum)
	...
	alpha := wave11Config(t, "wave11-alpha")
	beta := wave11Config(t, "wave11-beta")
	...
	require.Equal(t, "wave11-beta", activeCfg.Agent.Model)
	...
	require.Equal(t, "wave11-alpha", loadedCfg.Agent.Model)
...
func TestWave11ServerSessionRecallAndOperationalWrappers(t *testing.T) {
	t.Parallel()
	...
	sess := session.Session{
		...
		History: []session.Message{
			{Role: types.RoleUser, Content: "remember wave eleven", Timestamp: base},
	...
	_, err = srv.dispatch(ctx, Request{
		...
		Payload: mustPayload(t, SessionSetSaltRequest{Name: "wave11-agent", Salt: []byte("agent-salt")}),
	})
	require.NoError(t, err)

	saltAny, err := srv.dispatch(ctx, Request{
		Method:  methodSessionGetSalt,
		Payload: mustPayload(t, SessionGetSaltRequest{Name: "wave11-agent"}),
	})
	require.NoError(t, err)
	...
	_, err = srv.dispatch(ctx, Request{
		...
		Payload: mustPayload(t, SessionEndRequest{Key: "wave11-session"}),
	})
	require.NoError(t, err)
	...
	_, err = srv.dispatch(ctx, Request{
		...
		Payload: mustPayload(t, RecallIndexRequest{Key: "wave11-session"}),
	})
	require.NoError(t, err)

	searchAny, err := srv.dispatch(ctx, Request{
		Method:  methodRecallSearch,
		Payload: mustPayload(t, RecallSearchRequest{Query: "wave eleven", Limit: 3}),
	})
	require.NoError(t, err)
	...
	summaryAny, err := srv.dispatch(ctx, Request{
		...
		Payload: mustPayload(t, RecallSummaryRequest{Key: "wave11-session"}),
	})
	require.NoError(t, err)
	require.Contains(t, summaryAny.(RecallSummaryResult).Summary, "remember wave eleven")
	...
	repAny, err := srv.dispatch(ctx, Request{
		Method:  methodReputationGet,
		Payload: mustPayload(t, ReputationGetRequest{PeerDID: "did:example:wave11"}),
	})
	require.NoError(t, err)
...
func TestWave11ClientWrappersThroughFakeBroker(t *testing.T) {
	t.Parallel()
...
func TestWave11ClientCallErrorBranches(t *testing.T) {
	t.Parallel()
...
func openWave11Server(t *testing.T, withMasterKey bool) *Server {
	t.Helper()
...
func wave11Config(t *testing.T, model string) *config.Config {
	t.Helper()
