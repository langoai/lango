...
func newWave48EntStore(t *testing.T) (*EntStore, context.Context) {
	t.Helper()

	dsn := fmt.Sprintf("file:%s?_fk=1", filepath.Join(t.TempDir(), "runledger-wave48.db"))
...
func wave48AppendCreatedAndPlan(t *testing.T, ctx context.Context, store *EntStore, runID, sessionKey string) {
	t.Helper()
...
func TestWave48EntStoreGetRunSnapshotAppliesTailAndRewritesStepProjection(t *testing.T) {
	store, ctx := newWave48EntStore(t)
	wave48AppendCreatedAndPlan(t, ctx, store, "run-wave48-tail", "session-tail")

	initial, err := store.GetRunSnapshot(ctx, "run-wave48-tail")
	require.NoError(t, err)
	require.Equal(t, int64(2), initial.LastJournalSeq)
	require.Len(t, initial.Steps, 2)

	require.NoError(t, store.AppendJournalEvent(ctx, JournalEvent{
		RunID:   "run-wave48-tail",
	...
	updated, err := store.GetRunSnapshot(ctx, "run-wave48-tail")
	require.NoError(t, err)
	...
	rows, err := store.client.RunStep.Query().
		Where(entrunstep.RunIDEQ("run-wave48-tail")).
	...
	again, err := store.GetRunSnapshot(ctx, "run-wave48-tail")
	require.NoError(t, err)
...
func TestWave48EntStoreMaterializeAndSnapshotReportMalformedPayloads(t *testing.T) {
	store, ctx := newWave48EntStore(t)

	require.NoError(t, store.AppendJournalEvent(ctx, JournalEvent{
		RunID:   "run-wave48-bad-journal",
		Type:    EventRunCreated,
	...
	events, err := store.GetJournalEvents(ctx, "run-wave48-bad-journal")
	require.NoError(t, err)
	require.Len(t, events, 2)
	assert.Equal(t, json.RawMessage(`{`), events[1].Payload)

	snapshot, err := store.MaterializeRunSnapshot(ctx, "run-wave48-bad-journal")
	require.Error(t, err)
	assert.Nil(t, snapshot)
	assert.Contains(t, err.Error(), "apply event seq 2")
	assert.Contains(t, err.Error(), "unmarshal plan_attached")

	snapshot, err = store.GetRunSnapshot(ctx, "run-wave48-bad-journal")
	require.Error(t, err)
	assert.Nil(t, snapshot)
	assert.Contains(t, err.Error(), "unmarshal plan_attached")
}

func TestWave48EntStoreListsFilterLimitEmptyAndMaxSeq(t *testing.T) {
	store, ctx := newWave48EntStore(t)

	snapshots := []*RunSnapshot{
		{
			RunID:          "run-wave48-session-a-1",
	...
	assert.ElementsMatch(t, []string{"run-wave48-session-a-1", "run-wave48-session-a-2"}, []string{allSessionA[0].RunID, allSessionA[1].RunID})
	for _, summary := range allSessionA {
		assert.NotEqual(t, "run-wave48-session-b-1", summary.RunID)
	}

	limitedSessionA, err := store.ListRunSummariesBySession(ctx, "session-a", 1)
	require.NoError(t, err)
	require.Len(t, limitedSessionA, 1)
	assert.Contains(t, []string{"run-wave48-session-a-1", "run-wave48-session-a-2"}, limitedSessionA[0].RunID)
...
func TestWave48EntStoreMalformedPersistedSnapshotsFailListPaths(t *testing.T) {
	store, ctx := newWave48EntStore(t)

	_, err := store.client.RunSnapshot.Create().
		SetRunID("run-wave48-list-corrupt").
	...
	require.Error(t, err)
	assert.Nil(t, runs)
	assert.Contains(t, err.Error(), "listed snapshot run-wave48-list-corrupt")

	sessionRuns, err := store.ListRunSummariesBySession(ctx, "session-corrupt", 10)
	require.Error(t, err)
	assert.Nil(t, sessionRuns)
	assert.Contains(t, err.Error(), "session snapshot run-wave48-list-corrupt")
}

func TestWave48EntStorePruneNoopBranchesPreserveRows(t *testing.T) {
	store, ctx := newWave48EntStore(t)

	for _, runID := range []string{"run-wave48-prune-1", "run-wave48-prune-2"} {
		wave48AppendCreatedAndPlan(t, ctx, store, runID, "session-prune")
	...
	assert.ElementsMatch(t, []string{"run-wave48-prune-1", "run-wave48-prune-2"}, []string{runs[0].RunID, runs[1].RunID})

	journalEvents, err := store.GetJournalEvents(ctx, "run-wave48-prune-1")
	require.NoError(t, err)
