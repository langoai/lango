...
func newEntStoreAppendJournalDefaultsHookAndOrderingEntStore(t *testing.T) (*EntStore, context.Context) {
	t.Helper()

	dsn := fmt.Sprintf("file:%s?_fk=1", filepath.Join(t.TempDir(), "runledger-runChatBuilderErrorRunsStartupCleanupWithoutTui9.db"))
...
func TestEntStoreAppendJournalDefaultsHookAndOrdering(t *testing.T) {
	store, ctx := newEntStoreAppendJournalDefaultsHookAndOrderingEntStore(t)
	...
	require.NoError(t, store.AppendJournalEvent(ctx, JournalEvent{
		RunID:   "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-journal",
		Type:    EventNoteWritten,
	...
	events, err := store.GetJournalEvents(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-journal")
	require.NoError(t, err)
	...
	tail, err := store.GetJournalEventsSince(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-journal", 1)
	require.NoError(t, err)
	require.Len(t, tail, 1)
	assert.Equal(t, int64(2), tail[0].Seq)
	assert.Equal(t, EventProjectionSynced, tail[0].Type)

	emptyTail, err := store.GetJournalEventsSince(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-journal", 2)
	require.NoError(t, err)
...
func TestEntStoreRecordValidationResultPassAndFail(t *testing.T) {
	store, ctx := newEntStoreAppendJournalDefaultsHookAndOrderingEntStore(t)

	require.NoError(t, store.AppendJournalEvent(ctx, JournalEvent{
		RunID:   "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-validation",
		Type:    EventRunCreated,
		Payload: marshalPayload(RunCreatedPayload{SessionKey: "session-runChatBuilderErrorRunsStartupCleanupWithoutTui9", Goal: "validate steps"}),
	}))
	require.NoError(t, store.AppendJournalEvent(ctx, JournalEvent{
		RunID: "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-validation",
	...
	require.NoError(t, store.RecordValidationResult(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-validation", "step-pass", ValidationResult{
		Passed: true,
		Reason: "tests passed",
	}))
	require.NoError(t, store.RecordValidationResult(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-validation", "step-fail", ValidationResult{
		Passed:  false,
		Reason:  "build failed",
		Details: map[string]string{"target": "./internal/runledger"},
	}))

	events, err := store.GetJournalEventsSince(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-validation", 2)
	require.NoError(t, err)
	require.Len(t, events, 2)
	...
	snapshot, err := store.GetRunSnapshot(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-validation")
	require.NoError(t, err)
...
func TestEntStoreCachedSnapshotUpdateReadAndCacheHit(t *testing.T) {
	store, ctx := newEntStoreAppendJournalDefaultsHookAndOrderingEntStore(t)

	snapshot := &RunSnapshot{
		RunID:          "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-cache",
	...
	cached, seq, err := store.GetCachedSnapshot(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-cache")
	require.NoError(t, err)
	...
	reloaded, seq, err := reloadedStore.GetCachedSnapshot(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-cache")
	require.NoError(t, err)
	...
	_, err = store.client.RunSnapshot.Update().
		Where(entrunsnapshot.RunIDEQ("run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-cache")).
		SetGoal(dbChanged.Goal).
		SetSnapshotData(string(data)).
		Save(ctx)
	require.NoError(t, err)

	stillCached, seq, err := reloadedStore.GetCachedSnapshot(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-cache")
	require.NoError(t, err)
...
func TestEntStoreSnapshotErrorBranches(t *testing.T) {
	store, ctx := newEntStoreAppendJournalDefaultsHookAndOrderingEntStore(t)

	err := store.UpdateCachedSnapshot(ctx, &RunSnapshot{
		RunID:            "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-invalid-json",
	...
	_, err = store.client.RunSnapshot.Create().
		SetRunID("run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-corrupt").
	...
	corrupt, seq, err := store.GetCachedSnapshot(ctx, "run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-corrupt")
	require.Error(t, err)
	assert.Nil(t, corrupt)
	assert.Zero(t, seq)
	assert.Contains(t, err.Error(), "snapshot run-runChatBuilderErrorRunsStartupCleanupWithoutTui9-corrupt")
}
