...
func TestWave54DefaultValidatorsIncludesDeterministicSet(t *testing.T) {
	t.Parallel()
...
func TestWave54ArtifactExistsValidatorValidatesMissingTargetAndPath(t *testing.T) {
	t.Parallel()
...
func TestWave54FileChangedValidatorValidationAndGitError(t *testing.T) {
	t.Parallel()
...
func TestWave54FileChangedValidatorMatchesDiffByGlobOrSubstring(t *testing.T) {
	t.Parallel()

	requireWave54Command(t, "git")
	dir := t.TempDir()
	runGit(t, dir, "init")
	runGit(t, dir, "config", "user.email", "wave54@example.test")
	runGit(t, dir, "config", "user.name", "Wave54")
	...
	validator := &FileChangedValidator{}
	result, err := validator.Validate(wave54ValidatorContext(t), ValidatorSpec{
...
func TestWave54CommandPassValidatorCoversExpectedExitAndErrors(t *testing.T) {
	t.Parallel()

	requireWave54Command(t, "sh")
...
func TestWave54BuildAndTestValidatorsReturnStructuredFailures(t *testing.T) {
	t.Parallel()

	requireWave54Command(t, "go")
	dir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module wave54.example\n\ngo 1.24\n"), 0644))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "broken.go"), []byte("package wave54\n\nfunc Broken(\n"), 0644))

	buildResult, err := (&BuildPassValidator{}).Validate(wave54ValidatorContext(t), ValidatorSpec{Target: ".", WorkDir: dir}, nil)
	require.NoError(t, err)
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, "broken.go"), []byte("package wave54\n"), 0644))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "broken_test.go"), []byte("package wave54\n\nimport \"testing\"\n\nfunc TestBroken(t *testing.T) { t.Fatal(\"boom\") }\n"), 0644))

	testResult, err := (&TestPassValidator{}).Validate(wave54ValidatorContext(t), ValidatorSpec{Target: ".", WorkDir: dir}, nil)
	require.NoError(t, err)
...
func TestWave54BuildAndTestValidatorsUseDefaultTarget(t *testing.T) {
	t.Parallel()

	requireWave54Command(t, "go")
	dir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module wave54.example\n\ngo 1.24\n"), 0644))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "ok.go"), []byte("package wave54\n"), 0644))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "ok_test.go"), []byte("package wave54\n\nimport \"testing\"\n\nfunc TestOK(t *testing.T) {}\n"), 0644))

	buildResult, err := (&BuildPassValidator{}).Validate(wave54ValidatorContext(t), ValidatorSpec{WorkDir: dir}, nil)
	require.NoError(t, err)
	assert.True(t, buildResult.Passed)
	assert.Equal(t, "build succeeded", buildResult.Reason)

	testResult, err := (&TestPassValidator{}).Validate(wave54ValidatorContext(t), ValidatorSpec{WorkDir: dir}, nil)
	require.NoError(t, err)
	assert.True(t, testResult.Passed)
	assert.Equal(t, "all tests passed", testResult.Reason)
}

func TestWave54OrchestratorApprovalValidatorAlwaysRequiresApproval(t *testing.T) {
	t.Parallel()
...
func TestWave54ExitCodeStrAndTruncate(t *testing.T) {
	t.Parallel()

	requireWave54Command(t, "sh")
	ctx := wave54ValidatorContext(t)
...
func requireWave54Command(t *testing.T, name string) {
	t.Helper()
...
func wave54ValidatorContext(t *testing.T) context.Context {
	t.Helper()
