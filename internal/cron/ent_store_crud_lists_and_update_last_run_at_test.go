...
func TestEntStoreCRUDListsAndUpdateLastRunAt(t *testing.T) {
	ctx := context.Background()
	store := newEntStoreCrudListsAndUpdateLastRunAtEntStore(t)
	...
	job := Job{
		ID:           jobID,
		Name:         "p2PToolsInventoryAndSimpleHandlers1-crud",
		ScheduleType: "cron",
		Schedule:     "0 9 * * *",
	...
	assert.Equal(t, "p2PToolsInventoryAndSimpleHandlers1-crud", got.Name)
	...
	byName, err := store.GetByName(ctx, "p2PToolsInventoryAndSimpleHandlers1-crud")
	require.NoError(t, err)
	assert.Equal(t, got.ID, byName.ID)

	disabledJob := entStoreCrudListsAndUpdateLastRunAtJob("p2PToolsInventoryAndSimpleHandlers1-disabled")
	disabledJob.Enabled = false
	require.NoError(t, store.Create(ctx, disabledJob))

	allJobs, err := store.List(ctx)
	require.NoError(t, err)
	assert.ElementsMatch(t, []string{"p2PToolsInventoryAndSimpleHandlers1-crud", "p2PToolsInventoryAndSimpleHandlers1-disabled"}, entStoreCrudListsAndUpdateLastRunAtJobNames(allJobs))

	enabledJobs, err := store.ListEnabled(ctx)
	require.NoError(t, err)
	require.Len(t, enabledJobs, 1)
	assert.Equal(t, "p2PToolsInventoryAndSimpleHandlers1-crud", enabledJobs[0].Name)
...
func TestEntStoreUpsertCreatesAndUpdatesByName(t *testing.T) {
	ctx := context.Background()
	store := newEntStoreCrudListsAndUpdateLastRunAtEntStore(t)

	created, updated, err := store.Upsert(ctx, entStoreCrudListsAndUpdateLastRunAtJob("p2PToolsInventoryAndSimpleHandlers1-upsert"))
	require.NoError(t, err)
	assert.False(t, updated)
	require.NotNil(t, created)
	assert.NotEmpty(t, created.ID)
	assert.Equal(t, "p2PToolsInventoryAndSimpleHandlers1-upsert", created.Name)
	assert.False(t, created.CreatedAt.IsZero())

	createdAt := created.CreatedAt
	updatedJob := Job{
		Name:         "p2PToolsInventoryAndSimpleHandlers1-upsert",
		ScheduleType: "at",
...
func TestEntStoreSaveAndListHistory(t *testing.T) {
	ctx := context.Background()
	store := newEntStoreCrudListsAndUpdateLastRunAtEntStore(t)

	jobOne, updated, err := store.Upsert(ctx, entStoreCrudListsAndUpdateLastRunAtJob("p2PToolsInventoryAndSimpleHandlers1-history-one"))
	require.NoError(t, err)
	require.False(t, updated)
	jobTwo, updated, err := store.Upsert(ctx, entStoreCrudListsAndUpdateLastRunAtJob("p2PToolsInventoryAndSimpleHandlers1-history-two"))
	require.NoError(t, err)
	require.False(t, updated)
...
func TestEntStoreConversionHelpers(t *testing.T) {
	...
	entJob := &ent.CronJob{
		ID:           uuid.New(),
		Name:         "p2PToolsInventoryAndSimpleHandlers1-convert",
	...
	assert.Equal(t, "p2PToolsInventoryAndSimpleHandlers1-convert", job.Name)
	...
	assert.Equal(t, "p2PToolsInventoryAndSimpleHandlers1-convert", jobs[0].Name)

	completedAt := time.Date(2026, 5, 18, 6, 2, 0, 0, time.UTC)
	entHistory := &ent.CronJobHistory{
		ID:           uuid.New(),
		JobID:        entJob.ID,
		JobName:      "p2PToolsInventoryAndSimpleHandlers1-convert",
	...
	assert.Equal(t, "p2PToolsInventoryAndSimpleHandlers1-convert", history.JobName)
...
func TestEntStoreInvalidUUIDErrors(t *testing.T) {
	ctx := context.Background()
	store := newEntStoreCrudListsAndUpdateLastRunAtEntStore(t)

	err := store.Create(ctx, Job{
		ID:           "not-a-uuid",
		Name:         "p2PToolsInventoryAndSimpleHandlers1-invalid-create",
		ScheduleType: "cron",
		Schedule:     "0 9 * * *",
	...
	err = store.SaveHistory(ctx, HistoryEntry{
		ID:        "not-a-uuid",
		JobID:     uuid.NewString(),
		JobName:   "p2PToolsInventoryAndSimpleHandlers1-invalid-history",
		Status:    "running",
...
func newEntStoreCrudListsAndUpdateLastRunAtEntStore(t *testing.T) *EntStore {
	t.Helper()
...
func entStoreCrudListsAndUpdateLastRunAtJob(name string) Job {
	return Job{
		Name:         name,
		ScheduleType: "cron",
		Schedule:     "0 9 * * *",
...
func entStoreCrudListsAndUpdateLastRunAtJobNames(jobs []Job) []string {
