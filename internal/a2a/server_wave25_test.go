...
func TestWave25NewServerBuildsAgentCardFromConfigAndAgentTree(t *testing.T) {
	t.Parallel()

	operator := newWave25A2AAgent(t, "operator", "runs shell commands", nil)
	reviewer := newWave25A2AAgent(t, "reviewer", "reviews proposed changes", nil)
	root := newWave25A2AAgent(t, "root-agent", "coordinates sub-agents", []adkagent.Agent{
		operator,
		reviewer,
	})
...
func TestWave25NewServerFallsBackToAgentMetadata(t *testing.T) {
	t.Parallel()

	root := newWave25A2AAgent(t, "metadata-agent", "agent supplied description", nil)
...
func TestWave25SetP2PInfoAndPricingMutateRouteOutput(t *testing.T) {
	t.Parallel()

	root := newWave25A2AAgent(t, "priced-agent", "agent with p2p metadata", nil)
	server := NewServer(config.A2AConfig{
...
func TestWave25RegisterRoutesMountsAgentCardHandlerWithoutListener(t *testing.T) {
	t.Parallel()

	root := newWave25A2AAgent(t, "route-agent", "route description", nil)
	server := NewServer(config.A2AConfig{
		BaseURL: "https://route.example/a2a",
	}, root, zap.NewNop().Sugar())
	mux := &wave25RouteRecorder{routes: make(map[string]http.HandlerFunc)}
...
func TestWave25BuildSkillsExtractsRootOnlyAndSubAgentBranches(t *testing.T) {
	t.Parallel()

	rootOnly := newWave25A2AAgent(t, "solo", "solo description", nil)
	assert.Equal(t, []AgentSkill{
		{
			ID:          "solo",
			Name:        "solo",
	...
	child := newWave25A2AAgent(t, "child", "child description", nil)
	root := newWave25A2AAgent(t, "parent", "parent description", []adkagent.Agent{child})
	assert.Equal(t, []AgentSkill{
		{
			ID:          "parent",
			Name:        "parent",
...
func TestWave25AgentCardJSONOmitsEmptyP2PExtensions(t *testing.T) {
	t.Parallel()
...
func TestWave25ZKCredentialRoundTripsOnAgentCard(t *testing.T) {
	t.Parallel()
...
type wave25RouteRecorder struct {
	routes map[string]http.HandlerFunc
}

func (r *wave25RouteRecorder) Get(path string, handler http.HandlerFunc) {
