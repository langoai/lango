...
type fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient struct {
	mu sync.Mutex
...
func newFetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient() *fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient {
	return &fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient{
		headers: make(map[uint64]*types.Header),
	}
}

func (c *fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient) setHeader(num uint64, extra string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.headers[num] = fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesHeader(num, extra)
}

func (c *fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient) setLatest(num uint64) {
	c.setHeader(num, "latest")
}

func (c *fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient) HeaderByNumber(
	_ context.Context,
	number *big.Int,
) (*types.Header, error) {
	c.mu.Lock()
	...
	if number != nil {
		...
		if !ok {
			return fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesHeader(number.Uint64(), ""), nil
		}
		return header, nil
	}
	...
	if latestHeader == nil {
		return fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesHeader(0, ""), nil
	}
	return latestHeader, nil
}

func (c *fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient) FilterLogs(
	_ context.Context,
...
func fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesHeader(num uint64, extra string) *types.Header {
	return &types.Header{
...
type fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesErrorStore struct {
	err error
}

func (s fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesErrorStore) GetByOnChainDealID(string) (string, error) {
	return "", s.err
}

func TestFetchAndPublishFiltersRangePublishesABIEventAndAdvances(t *testing.T) {
	t.Parallel()

	client := newFetchAndPublishFiltersRangePublishesAbiEventAndAdvancesBlockchainClient()
	client.setLatest(15)
...
func TestFetchAndPublishReturnsLatestHeaderErrorWithoutAdvancing(t *testing.T) {
	t.Parallel()
...
func TestFetchAndPublishReturnsFilterErrorWithoutAdvancing(t *testing.T) {
	t.Parallel()
...
func TestFetchAndPublishNoopsWhenSafeBlockDoesNotAdvance(t *testing.T) {
	t.Parallel()
...
func TestHandleEventDisputeRaisedDecodesV2InitiatorFromData(t *testing.T) {
	t.Parallel()
...
func TestHandleEventSettlementFinalizedUsesV2DealIDTopic(t *testing.T) {
	t.Parallel()
...
func TestResolveEscrowIDReturnsEmptyWhenStoreErrors(t *testing.T) {
	t.Parallel()

	monitor := testMonitor(t, fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesErrorStore{err: errors.New("store unavailable")})

	assert.Equal(t, "", monitor.resolveEscrowID("99"))
}

func TestCheckBlockHashContinuityPublishesSilentReorgAndRollsBack(t *testing.T) {
	t.Parallel()
	...
	monitor.blockHashes[19] = fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesHeader(19, "old-parent").Hash()
...
func TestStartStopLifecycleIsIdempotent(t *testing.T) {
	...
	fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesWaitGroup(t, &started)
...
func fetchAndPublishFiltersRangePublishesAbiEventAndAdvancesWaitGroup(t *testing.T, wg *sync.WaitGroup) {
	t.Helper()
