...
func TestWave29BuildToolsIncludesEnabledEnginesInStableOrder(t *testing.T) {
	t.Parallel()

	be := newWave29BudgetEngine(t)
	re := newWave29RiskEngine(t, 0.9)
...
func TestWave29BudgetToolHandlersUseDefaultsAndReturnReports(t *testing.T) {
	t.Parallel()

	tools := buildBudgetTools(newWave29BudgetEngine(t))
...
func TestWave29RiskAndNegotiationToolHandlersReturnStructuredResults(t *testing.T) {
	t.Parallel()

	riskTool := findEconomyTool(buildRiskTools(newWave29RiskEngine(t, 0.95)), "economy_risk_assess")
...
func TestWave29EscrowCreateSkipsMalformedMilestonesAndStatusExpandsMilestones(t *testing.T) {
	t.Parallel()
...
func TestWave29PricingToolOmitsPriceFieldsForFreeQuotes(t *testing.T) {
	t.Parallel()

	priceTool := findEconomyTool(buildPricingTools(newWave29PricingEngine(t)), "economy_price_quote")
...
func newWave29BudgetEngine(t *testing.T) *budget.Engine {
	t.Helper()
...
func newWave29RiskEngine(t *testing.T, trustScore float64) *risk.Engine {
	t.Helper()
...
func newWave29PricingEngine(t *testing.T) *pricing.Engine {
	t.Helper()

	engine, err := pricing.New(config.DynamicPricingConfig{})
	require.NoError(t, err)
	engine.SetBasePrice("paid_tool", wave29USDC(t, "2.00"))
	return engine
}

func wave29USDC(t *testing.T, amount string) *big.Int {
	t.Helper()
