...
type wave45ConfigProfiles struct {
	ConfigProfileStore
}

type wave45SecurityState struct {
	SecurityStateStore
}

func TestFacadeWave45ConstructionKeepsExplicitStoresAndIgnoresNilFallbacks(t *testing.T) {
	t.Parallel()

	profiles := &wave45ConfigProfiles{}
	securityState := &wave45SecurityState{}
...
func TestFacadeWave45SessionFactoryWinsOverDBPathFallback(t *testing.T) {
	t.Parallel()
...
func TestFacadeWave45BrokerSessionStoreResolution(t *testing.T) {
	t.Parallel()
...
func TestFacadeWave45RawDBCloseDoesNotOverrideExistingCloser(t *testing.T) {
	t.Parallel()
...
func TestFacadeWave45ResolveEntBackedRequiresClientAndBuilder(t *testing.T) {
	client := enttest.Open(t, "sqlite3", "file:facade-wave45-resolve?mode=memory&_fk=1")
	t.Cleanup(func() {
...
func TestFacadeWave45EntBackedReadersPropagateClosedClientErrors(t *testing.T) {
	ctx := context.Background()
	client := enttest.Open(t, "sqlite3", "file:facade-wave45-closed?mode=memory&_fk=1")
	f := NewFacade(nil, nil, WithEntClient(client))
	require.NoError(t, client.Close())
