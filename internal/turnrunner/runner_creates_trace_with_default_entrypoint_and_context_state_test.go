...
type runnerCreatesTraceWithDefaultEntrypointAndContextStateExecutor struct {
	run func(
		ctx context.Context,
		sessionID string,
		input string,
...
func (e *runnerCreatesTraceWithDefaultEntrypointAndContextStateExecutor) RunStreamingDetailed(
	ctx context.Context,
...
func TestRunnerCreatesTraceWithDefaultEntrypointAndContextState(t *testing.T) {
	...
	executor := &runnerCreatesTraceWithDefaultEntrypointAndContextStateExecutor{
	...
	result, err := runner.Run(context.Background(), Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:session",
		Input:      "hello",
	})
	...
	assert.Equal(t, "defaultServeAwaitShutdownWaitsForContextCancellation3:session", captured.sessionID)
	assert.Equal(t, "hello", captured.input)
	assert.Equal(t, "defaultServeAwaitShutdownWaitsForContextCancellation3:session", captured.session)
	...
	assert.Equal(t, "defaultServeAwaitShutdownWaitsForContextCancellation3:session", trace.SessionKey)
...
func TestRunnerEmitsDelegationBudgetToolThinkingAndResultCallbacks(t *testing.T) {
	traceStore := newMemoryTraceStore()
	executor := &runnerCreatesTraceWithDefaultEntrypointAndContextStateExecutor{
	...
	result, err := runner.Run(context.Background(), Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:callbacks",
...
func TestRunnerAnnotatesTimeoutAndRecordsTerminalError(t *testing.T) {
	traceStore := newMemoryTraceStore()
	...
	result, err := runner.Run(context.Background(), Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:timeout",
		Input:      "wait",
	})
	...
	assert.Equal(t, []string{"defaultServeAwaitShutdownWaitsForContextCancellation3:timeout"}, sessionStore.annotated)
...
func TestRunnerClassifiesPlainExecutorErrorWithTruncatedDetails(t *testing.T) {
	traceStore := newMemoryTraceStore()
	...
	result, err := runner.Run(context.Background(), Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:error",
		Input:      "explode",
	})
...
func TestRunnerUsesGenericFallbackForEmptySuccessfulTurn(t *testing.T) {
	traceStore := newMemoryTraceStore()
	executor := &fixtureExecutor{report: adk.RunReport{}}
	runner := New(Config{HardCeiling: time.Second, TraceStore: traceStore}, executor, nil, nil)

	result, err := runner.Run(context.Background(), Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:empty-success",
		Input:      "quiet",
	})
...
func TestRunnerParentCancellationDuringRetryBackoffSkipsSleep(t *testing.T) {
	...
	result, err := runner.Run(ctx, Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:cancel-retry",
		Input:      "retry",
	})
...
func TestRunnerCallbacksRegisteredDuringFireWaitForNextTurn(t *testing.T) {
	...
	_, err := runner.Run(context.Background(), Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:first",
		Input:      "one",
	})
	require.NoError(t, err)
	assert.Equal(t, []string{"first:defaultServeAwaitShutdownWaitsForContextCancellation3:first"}, calls)

	_, err = runner.Run(context.Background(), Request{
		SessionKey: "defaultServeAwaitShutdownWaitsForContextCancellation3:second",
		Input:      "two",
	})
	require.NoError(t, err)
	assert.Equal(t, []string{
		"first:defaultServeAwaitShutdownWaitsForContextCancellation3:first",
		"first:defaultServeAwaitShutdownWaitsForContextCancellation3:second",
		"second:defaultServeAwaitShutdownWaitsForContextCancellation3:second",
	}, calls)
}
