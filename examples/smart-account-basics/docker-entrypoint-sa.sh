...
TIMEOUT=60
ELAPSED=0
while [ ! -f /shared/usdc-address.txt ] || [ ! -f /shared/entrypoint-address.txt ] || [ ! -f /shared/factory-address.txt ]; do
  sleep 1
  ELAPSED=$((ELAPSED + 1))
...
ENTRYPOINT_ADDRESS=$(cat /shared/entrypoint-address.txt)
FACTORY_ADDRESS=$(cat /shared/factory-address.txt)
echo "[$AGENT_NAME] USDC: $USDC_ADDRESS, EntryPoint: $ENTRYPOINT_ADDRESS, Factory: $FACTORY_ADDRESS"
...
if [ -f "$CONFIG_SECRET" ] && [ ! -f "$LANGO_DIR/lango.db" ]; then
  ...
  sed -i "s/PLACEHOLDER_ENTRYPOINT_ADDRESS/$ENTRYPOINT_ADDRESS/g" /tmp/lango-import.json
  sed -i "s/PLACEHOLDER_FACTORY_ADDRESS/$FACTORY_ADDRESS/g" /tmp/lango-import.json
  lango config import /tmp/lango-import.json --profile "$PROFILE_NAME"
  rm -f /tmp/lango-import.json
  echo "[$AGENT_NAME] Config imported."
fi

# ── Inject wallet private key ──
...
# Re-create keyfile for serve bootstrap
if [ -f "$PASSPHRASE_SECRET" ]; then
  cp "$PASSPHRASE_SECRET" "$LANGO_DIR/keyfile"
  chmod 600 "$LANGO_DIR/keyfile"
fi
