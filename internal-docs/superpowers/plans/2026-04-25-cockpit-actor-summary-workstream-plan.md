# Cockpit Actor Summary Workstream Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extend the cockpit dead-letter summary strip with top 5 latest manual replay actors.

**Architecture:** Keep the current dead-letters page and the landed multi-line summary strip intact, then extend that strip additively with a compact latest-actor line. The workstream reuses the same backlog rows already loaded by the cockpit page, performs latest-actor aggregation in page-local code, and preserves the existing reload-aligned recomputation behavior.
...
- Modify: `internal/cli/cockpit/pages/deadletters.go`
  - Extend page-local summary aggregation and summary-strip rendering with top actors.
- Modify: `internal/cli/cockpit/pages/deadletters_test.go`
  - Cover actor aggregation and strip rendering after reload paths.

### Worker B: Docs / OpenSpec / README

- Modify: `docs/architecture/dead-letter-browsing-status-observation.md`
  - Describe richer cockpit summary strip behavior with top actors.
...
- Modify: `openspec/specs/docs-only/spec.md`
  - Sync the public docs requirements for actor-rich cockpit summaries.
- Modify: `README.md` only if the cockpit dead-letter page description needs a visible note.
- Create: `openspec/changes/archive/2026-04-25-cockpit-actor-summary-workstream/**`
  - Proposal, design, tasks, and delta specs.

## Task Breakdown

### Task 1: Extend the Cockpit Summary Strip with Top Actors
...
- compact rendering of top actors
- recompute on:
  - initial load
  - filter apply
  - `Ctrl+R` reset
...
- aggregate from `LatestManualReplayActor`
...
- [ ] **Step 1: Audit the final actor-rich cockpit summary strip before writing docs**

Verify:

- placement on the dead-letters page
- compact multi-line strip shape
- top actor fields actually shown
...
- added top latest manual replay actors
...
to reflect the landed actor-rich cockpit summary slice.

- [ ] **Step 4: Archive the completed workstream**

Create:

- `openspec/changes/archive/2026-04-25-cockpit-actor-summary-workstream/proposal.md`
- `openspec/changes/archive/2026-04-25-cockpit-actor-summary-workstream/design.md`
- `openspec/changes/archive/2026-04-25-cockpit-actor-summary-workstream/tasks.md`
- `openspec/changes/archive/2026-04-25-cockpit-actor-summary-workstream/specs/docs-only/spec.md`
