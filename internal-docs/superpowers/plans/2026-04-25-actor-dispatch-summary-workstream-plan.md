...
- Modify: `internal/cli/status/status.go`
  - Extend summary result types and actor aggregation logic.
- Modify: `internal/cli/status/render.go`
  - Extend summary rendering with top-actors section.
- Modify: `internal/cli/status/status_test.go`
  - Cover actor aggregation and output.

### Worker B: Docs / OpenSpec / README

- Modify: `docs/cli/status.md`
  - Document the richer summary output with top actors.
...
- Modify: `docs/architecture/dead-letter-browsing-status-observation.md`
  - Describe top latest manual replay actors as landed work.
- Modify: `docs/architecture/p2p-knowledge-exchange-track.md`
  - Narrow the remaining actor/dispatch summary follow-on work.
- Modify: `openspec/specs/docs-only/spec.md`
  - Sync the public docs requirements for actor summaries.
- Create: `openspec/changes/archive/2026-04-25-actor-dispatch-summary-workstream/**`
  - Proposal, design, tasks, and delta specs.

## Task Breakdown

### Task 1: Extend the Dead-Letter Summary CLI Surface with Actors
...
- top 5 latest manual replay actor aggregation
- additive extension of `lango status dead-letter-summary`
- default `table` output with a top-actors section
- optional `json` output with `top_latest_manual_replay_actors`
- item shape:
  - `actor`
  - `count`
...
- [ ] **Step 3: Implement the actor summary extension**
...
- aggregate from `latest_manual_replay_actor`
- use top 5 actors only
...
- [ ] **Step 1: Audit the final actor summary command before writing docs**

Verify:

- command name stays unchanged
- top-actors section is actually present
...
- new `top 5 latest manual replay actors`
...
to reflect the landed actor-summary slice.

- [ ] **Step 4: Archive the completed workstream**

Create:

- `openspec/changes/archive/2026-04-25-actor-dispatch-summary-workstream/proposal.md`
- `openspec/changes/archive/2026-04-25-actor-dispatch-summary-workstream/design.md`
- `openspec/changes/archive/2026-04-25-actor-dispatch-summary-workstream/tasks.md`
- `openspec/changes/archive/2026-04-25-actor-dispatch-summary-workstream/specs/docs-only/spec.md`
