# Mission Lifecycle Slice 2 Implementation Plan
...
**Goal:** Land Slice 2 durable mission lifecycle so Mission Control no longer relies on runtime projection alone. Durable mission rows, mission history, execution links, and coarse decision state must exist in the real app, not only in tests.

**Architecture:** Keep `RunLedger` as execution truth and add a separate durable mission layer in Ent. `MissionService` is the single writer for mission latest state, mission history, and mission-execution links. Mission Control becomes durable-first, but still overlays unmatched runtime work and live activity/approval signals. Slice 2 is only complete if there are real write paths for:
...
- Do not make `TaskEntry` the authoritative mission checklist model in this slice.
- Do not auto-create durable missions for every background task or run.
- Do not persist a full durable approval queue; Slice 2 stores only coarse mission decision summary.
- Do not make standalone `lango chat` a mission-native surface yet. Slice 2 direct mission creation is scoped to Mission Control.

## File Map

### Worker A: OpenSpec / Docs / Public Truth

- Create: `openspec/changes/2026-05-03-mission-lifecycle-slice-two/proposal.md`
- Create: `openspec/changes/2026-05-03-mission-lifecycle-slice-two/design.md`
- Create: `openspec/changes/2026-05-03-mission-lifecycle-slice-two/tasks.md`
- Create: `openspec/changes/2026-05-03-mission-lifecycle-slice-two/specs/mission-control-tui/spec.md`
- Create: `openspec/changes/2026-05-03-mission-lifecycle-slice-two/specs/agent-control-plane-tools/spec.md`
- Modify: `README.md`
...
### Task 1: Create the Slice 2 OpenSpec Change
...
Write proposal/design/tasks that match the approved Slice 2 direction:
...
- `TaskEntry` remains lightweight in this slice
- Slice 2 does not promote task tracking into the durable mission checklist model
...
```bash
openspec validate 2026-05-03-mission-lifecycle-slice-two --strict
```

Expected:

```text
Change '2026-05-03-mission-lifecycle-slice-two' is valid
```
...
Slice 2 reads missions by session and recent activity. Add at least:
...
Keep the interface centered on Slice 2 reads and writes:
...
Standalone `lango chat` remains unchanged in this slice.
...
- [ ] **Step 3: Preserve Slice 1 runtime visibility**
...
- [ ] **Step 1: Audit landed Slice 2 behavior before writing docs**
...
- `openspec validate 2026-05-03-mission-lifecycle-slice-two --strict`
- `openspec verify 2026-05-03-mission-lifecycle-slice-two --strict`
- archive the change so delta specs merge into main specs

### Task 10: Final Verification and Slice Review
...
- [ ] **Step 3: Run final Slice 2 review**

Before claiming Slice 2 complete:

- request final spec-compliance review against the Slice 2 change
...
- archive the Slice 2 OpenSpec change
