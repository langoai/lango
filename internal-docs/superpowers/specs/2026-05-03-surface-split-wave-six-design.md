# Surface Split Wave 6 Design

## Strategic Goal

By the end of Wave 5, Lango has one strong mission-native control surface, but it still exposes that surface through two CLI commands that behave the same:
...
Wave 6 should finish the surface story:
...
This is not a feature-add wave first. It is a surface-contract wave.
...
Today the first and third answers collapse into the same TUI. Wave 6 removes that collapse.

## Fixed Decisions

The following decisions are fixed for Wave 6:
...
- Wave 6 will reuse existing Mission Control projection, lifecycle, loop, proposal, and collaboration assets rather than introducing a new mission domain
- the first Wave 6 slice may keep Mission Control available inside `lango cockpit`, but it must no longer be the bare-`lango` contract

## Why This Needs Its Own Wave

Wave 1 intentionally postponed surface split because the first problem was to build Mission Control itself. That was the right sequence.

After Waves 2 through 5, the product is now meaningfully different:
...
Wave 6 should introduce a new standalone root shell:
...
Wave 6 should not make plain chat silently create mission rows.
...
Wave 6 does not need a whole new diagnostics UI before the split becomes valuable.
...
Wave 6 should add a separate root shell instead of forking the Mission Control page itself.
...
- the reusable Mission Control page and projector may remain under the existing `internal/cli/cockpit/...` namespace in Wave 6
...
The existing `EventBus` utility does not provide unsubscribe support. Wave 6 should therefore keep Mission Control-related subscriptions aligned to program lifetime rather than pretending page-level unsubscribe exists.
...
- Wave 6 should not claim page-level subscribe/unsubscribe semantics that the current bus cannot support
...
Wave 6 must verify that no mission, approval, attribution, or continuity behavior incorrectly depends on the old bare-`lango` cockpit prefix.
...
Wave 6 should explicitly reuse:
...
Wave 6 does not require a generalized interactive-surface framework if that abstraction is not already clearly justified by the code.
...
Wave 6 should keep `--mode` behavior aligned across all interactive surfaces:
...
Wave 6 changes that default, so the first slice should include direct in-product discoverability:
...
Wave 6 should update at least these contracts:
...
The new main spec is recommended rather than optional. Wave 6 is introducing a first-class named root surface, so leaving that contract only as a Mission Control delta would keep product truth muddy.

## Documentation Impact

Wave 6 must update all public truth that currently says bare `lango` equals `lango cockpit`, including:
...
Wave 6 should add focused tests for:
...
1. Create the Wave 6 OpenSpec change
