# Mission Lifecycle Slice 2 Design

## Strategic Goal

Slice 1 made Mission Control visible as a projection over existing runtime facts. Slice 2 turns missions into durable first-class records without collapsing them back into `RunLedger` runs or background tasks.

The core shift is:

- Slice 1: missions are derived UI rows
- Slice 2: missions are durable work units with their own identity and lifecycle
...
The following design choices are fixed for Slice 2:
...
At the same time, a full mission event journal would duplicate too much of `RunLedger`. Slice 2 therefore stores mission latest state and mission state history in Ent, while linking execution records through lightweight references.

## Core Model

Slice 2 adds three new durable concepts:
...
Slice 2 intentionally does not persist durable `proposed` missions. `proposed` remains a transient suggestion overlay until acceptance creates a durable mission row in `prepared` or `active`.
...
Slice 2 does not need a second “mission journal” abstraction in code. A simple append-only history table is enough.
...
`task_os_execution` is the canonical execution identity for agent-driven background work because `AgentRun.ID` and background task ID are intentionally unified today. Slice 2 should not record the same real execution twice as both `background_task` and `agent_run`.

## State Semantics

Slice 2 mission states are user-facing states, not direct execution mirrors.
...
Slice 2 intentionally does not introduce `failed` as a top-level mission state. Execution may fail, but the mission usually remains `blocked` until the user or runtime decides what to do next.

## Creation Rules

Slice 2 durable mission creation happens only through two channels:

1. direct user-started work
2. explicit acceptance of a proposed mission

Slice 2 does not automatically create durable missions for every background task or run. Transient `proposed` overlays may still exist before a durable mission row is created.
...
The following do not create a durable mission in Slice 2 by default:
...
Slice 2 should allow one mission to outlive one execution.
...
Slice 2 should not embed mission lifecycle into `RunLedger` snapshots directly. Instead:
...
Slice 2 may attach a background task to a mission by storing:

- `execution_kind = task_os_execution`
- `execution_ref = task_id`

No requirement exists to make background tasks durable themselves in this slice.

### Agent Run

Agent runs remain execution/runtime records. Agent runtime conditions can influence mission latest state, but the canonical execution identity stays singular. Slice 2 should not create a second `MissionExecutionLink` row only because agent metadata exists for the same unified task execution ID.

## Relation to TaskEntry

`TaskEntry` stays separate in Slice 2.
...
Slice 2 stores mission truth in Ent first, not in ontology or graph storage.
...
However, Slice 2 naming must leave room for later promotion:
...
Slice 2 should introduce a mission service responsible for:
...
Mission Control should stop projecting durable mission state only from raw runtime surfaces once Slice 2 is landed.
...
Mission Control still overlays live approval and activity buffers for freshness, but mission identity and lifecycle come from durable mission storage first. Unmatched runtime overlays must remain visible until linked, cancelled, or explicitly ignored so Slice 2 does not regress Slice 1 visibility.
...
Slice 2 should not invent a distributed transaction between mission store and runledger store.

## Waiting Decision Semantics

Slice 2 allows at most one durable latest decision marker per mission row:
...
- multiple simultaneous pending approvals for one mission remain out of scope in this slice
...
Slice 2 deliberately does not store `primary_execution_kind` / `primary_execution_ref` on the `Mission` row. If a later performance slice needs those values on the row, they should be treated as derived cache only, never as a second source of truth.
...
At the end of Slice 2, the system should support:
