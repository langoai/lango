# Proactive Agent Behavior Wave 3 Design

## Strategic Goal

Wave 2 made missions durable and made Mission Control read durable mission rows first. Wave 3 adds proactive behavior on top of that foundation: agents should be able to notice useful work before the user asks explicitly, prepare low-risk work automatically, and surface that work as actionable proposed missions.

The key constraint is control. Wave 3 should make Lango feel more agentic without creating a system that silently performs risky work or mutates durable mission truth without a clear user-facing reason.

## Product Intent

Wave 3 should answer a simple user question:
...
The following decisions are fixed for Wave 3:
...
Wave 2 deliberately excluded durable `proposed` mission rows. That was the right tradeoff and should remain true in Wave 3.
...
Wave 3 therefore needs a dedicated transient proposal model rather than overloading durable missions.

## Proposal Model

Wave 3 introduces a transient `Proposal` concept.
...
This does not need to be durable in the same way as `Mission` in the first Wave 3 slice. A session-scoped or short-lived persisted registry is enough, as long as Mission Control can render it honestly and expiration/dismissal are deterministic.

## Proposal Producers

Wave 3 should start from producers that already exist in the repo or are close enough to current behavior to be credible.
...
Wave 3 should not treat all "possible future producers" as ready on day one. Each producer needs an explicit contract.
...
In other words, librarian is an intended Wave 3 producer, but not an implicit one.
...
These already exist and already surface as transient proposed missions. Wave 3 should promote them from passive suggestions into proposal records that can own preparation work.

### Proactive Librarian

The librarian already detects gaps, creates inquiries, and can auto-save high-confidence knowledge. Wave 3 should let those gaps create proposals such as:
...
When runtime work repeatedly fails or blocks with a stable reason, Wave 3 may generate a proposal such as:
...
Wave 3 must define what agents may do automatically before acceptance.
...
Wave 3 should introduce a transient proposal execution relationship:
...
Wave 3 must stay compatible with the Wave 2 rule that `MissionExecutionLink` is written at execution creation sites for durable missions.
...
- Wave 3 does not support arbitrary relinking of an execution ref that is already durably linked to another mission

This keeps Wave 2 durable link ownership rules intact.

## Mission Control Surface

Wave 3 should extend the existing proposed-mission surface rather than replacing it.
...
Wave 3 must preserve the live-decision discipline established earlier.
...
Wave 3 should remain compatible with the longer-term ontology direction without forcing ontology to become the runtime control plane immediately.
...
- Wave 3 should not require ontology writes for every proposal before the proposal model is stable

## Non-Goals

Wave 3 does not aim to:
