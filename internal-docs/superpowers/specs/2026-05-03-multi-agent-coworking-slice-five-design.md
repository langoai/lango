# Multi-Agent Coworking Slice 5 Design

## Strategic Goal

By the end of Slice 4, Lango can show:
...
Slice 5 should make collaboration inside those work units visible. The user should be able to tell not only what work exists, but also how multiple agents are cooperating, blocking each other, handing off, reviewing, or consuming budget inside one mission.

## Product Intent

Slice 5 should answer:
...
The following decisions are fixed for Slice 5:
...
## Why Coworking Needs Its Own Slice
...
That is the gap Slice 5 addresses.

## First-Slice Collaboration Sources

Slice 5 should start only from sources that already exist in code today.
...
Slice 2 already links mission to execution identities. Slice 5 should use those links to tie delegation and teammate runtime back into the mission surface.
...
The codebase contains P2P team coordination, but the first Slice 5 slice should not try to solve full external team UX at the same time.
...
Slice 5 introduces a transient collaboration projection per mission.
...
Slice 5 should start with lightweight review semantics, not a full PR-style review system.
...
Slice 5 should extend existing mission rows and detail rendering rather than building a separate collaboration dashboard first.
...
## Relationship To Slice 4 Loops

Loops and agenda stay above collaboration state.

Slice 5 should not turn loop rows into giant team dashboards. Instead:
...
The repo already has external team coordination primitives. Slice 5 should acknowledge them but not overcommit.
