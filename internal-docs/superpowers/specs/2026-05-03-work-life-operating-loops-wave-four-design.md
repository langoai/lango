# Work And Life Operating Loops Wave 4 Design

## Strategic Goal

Wave 3 introduced proactive proposals. Wave 4 should turn Mission Control from a mission-and-proposal surface into a broader operating surface for recurring work, open loops, follow-ups, and scheduled activity.
...
The following decisions are fixed for Wave 4:

- Wave 4 builds on durable missions and transient proposals; it does not replace them
...
By the end of Wave 3, Lango can:
...
Wave 4 should make those loops explicit.

## Initial Loop Sources

Wave 4 should start only from sources that already exist in code today.

### Source 1: Durable Missions

Durable missions remain the primary owned work units.

Wave 4 loop view should be able to group:
...
The first Wave 4 slice must not pretend the following are real loop sources unless adapters exist:
...
Wave 4 introduces a transient `LoopView` model.
...
Wave 4 should not create a second durable mission-like table that competes with `Mission`.
...
Wave 4 should extend Mission Control with a loop-aware overview rather than replacing the mission board entirely.
...
Wave 4 needs deterministic prioritization.
...
Wave 4 should introduce an agenda concept carefully.
...
Wave 4 should stop short of multi-agent coworking. However, loop surfaces should leave room for later visibility such as:

- which agent last touched the loop
- whether review or handoff is pending
- whether a loop is blocked on another agent

That belongs to Wave 5.
