...
- **Knowledge Store** - Persistent versioned storage for facts, patterns, and external references. Each save appends a new version (append-only), preserving full edit history. Use `get_knowledge_history` to browse previous versions. All reads and searches return the latest version by default
