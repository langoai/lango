...
The `lango onboard` command SHALL fail before bootstrap or TUI startup when the command input stream is not an interactive terminal.

#### Scenario: Non-interactive onboard fails with scripted guidance
- **WHEN** `lango onboard` is invoked while the command input stream is not an interactive terminal
