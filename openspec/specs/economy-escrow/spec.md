...
### Requirement: Integration with settlement.Service
When an escrow is released, the system SHALL invoke `settlement.Service` to execute the on-chain transfer from the escrow holding to the seller's wallet address.

#### Scenario: Released escrow triggers settlement
- **WHEN** escrow transitions to "released"
- **THEN** settlement.Service is called to transfer TotalAmount to seller
