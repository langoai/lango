# TUI Reactive Fields Spec

## Purpose

Define the reactive field dependency system for TUI forms — `OnChange` callbacks, async loading state, and `FieldOptionsLoadedMsg` pattern for dynamically updating dependent fields (e.g., provider → model).
