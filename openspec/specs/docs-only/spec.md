...
The `docs/architecture/p2p-knowledge-exchange-track.md` file SHALL describe dead-letter browsing / status observation as landed work with transaction-global dominant family, compact per-submission breakdown, and a thin raw background-task bridge on the detail view, and list the remaining work as higher-level cockpit or CLI surfaces.
...
- **AND** the remaining work SHALL be described as higher-level cockpit or CLI surfaces
