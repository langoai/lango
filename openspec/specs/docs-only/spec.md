...
The `docs/architecture/p2p-knowledge-exchange-track.md` file SHALL describe dead-letter browsing / status observation as landed work with cross-submission lifecycle grouping, and list the remaining work as transaction-global dominant family, raw background-task bridges, and higher-level cockpit or CLI surfaces.
...
- **AND** the remaining work SHALL be described as transaction-global dominant family, raw background-task bridges, and higher-level cockpit or CLI surfaces
