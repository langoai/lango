...
The `docs/architecture/p2p-knowledge-exchange-track.md` file SHALL describe dead-letter browsing / status observation as landed work with transaction-global dominant family, compact per-submission breakdown, a thin raw background-task bridge on the detail view, and a cockpit dead-letter read surface, and list the remaining work as replay/write controls, richer cockpit filtering, and higher-level CLI surfaces.
...
- **AND** the remaining work SHALL be described as replay/write controls, richer cockpit filtering, and higher-level CLI surfaces
