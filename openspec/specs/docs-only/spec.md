...
The `docs/architecture/p2p-knowledge-exchange-track.md` file SHALL describe dead-letter browsing / status observation as landed work with transaction-global dominant family, compact per-submission breakdown, a thin raw background-task bridge on the detail view, a cockpit dead-letter read surface, a thin cockpit filter bar, cockpit subtype filtering, cockpit latest-family filtering, cockpit actor/time filtering, a cockpit `Retry` action, and confirm/refresh recovery UX, and list the remaining work as richer cockpit filters beyond latest family, richer loading/failure recovery feedback, and higher-level CLI surfaces.
...
- **AND** the remaining work SHALL be described as richer cockpit filters beyond latest family, richer loading/failure recovery feedback, and higher-level CLI surfaces
