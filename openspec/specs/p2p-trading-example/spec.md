...
The `contracts/test/mocks/MockUSDC.sol` contract SHALL implement a minimal ERC-20 with `mint()`, `transfer()`, `transferFrom()`, `approve()`, `balanceOf()`, and `allowance()` functions with 6 decimals.
...
The `examples/p2p-trading/docker-entrypoint-p2p.sh` entrypoint SHALL wait for the USDC contract address from the setup sidecar, substitute it into the config, import the config, and inject the wallet private key via `--value-hex` flag.
...
The `examples/p2p-trading/scripts/test-p2p-trading.sh` script SHALL verify health, P2P status, peer discovery, DID identity, USDC balances, and a payment transfer via REST API and on-chain queries.
