...
The system SHALL provide an option to pass `plugin.Config` (ADK v1.0.0 `plugin/plugin.go:26`) through to `runner.Config.PluginConfig` when creating an ADK runner in `internal/adk/agent.go`.
