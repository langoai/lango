...
By the end of Wave 4, Mission Control can show owned work and operator loops, but it still treats collaboration largely as transcript noise. Delegation, teammate waiting state, review-needed execution state, budget pressure, and recovery activity exist in runtime signals, yet the user cannot reliably see those signals as mission-linked coworking state.

At the same time, Wave 5 must avoid inventing a second durable work model or overclaiming external team support. The first slice should make local coworking legible while keeping attribution strict and projection-only.
...
Wave 5 adds a collaboration projection attached to a mission rather than a new durable model.
...
Strict mission-linked attribution is the central rule of Wave 5.
...
Wave 5 extends mission rows and mission detail context without replacing the mission board.
...
External P2P team state already exists in the repo, but Wave 5 keeps it secondary.
