...
At the same time, `RunLedger` already serves as durable execution truth. Slice 2 must avoid duplicating execution storage or pretending that every execution row is itself the user-facing mission record.
...
Durable statuses in Slice 2 are:
...
Slice 2 intentionally stops at latest-row plus history. It does not introduce a second general-purpose event journal abstraction.
...
This preserves Slice 1 honesty. Runtime work does not disappear merely because it has not yet been linked into the durable mission graph.

## Write Paths

Slice 2 is only complete if all four of these durable write paths exist in the real app:
...
When mission-bound work creates a background execution or a `RunLedger` execution, the app writes the `MissionExecutionLink` at that execution creation site. Slice 2 does not rely on later inference from unrelated task tracking to reconstruct mission ownership.
...
Slice 2 stores `waiting_decision` as a coarse durable mission state. It records that the mission is paused on user direction, plus a latest decision kind/summary if available.
...
Slice 2 does not store a durable approval queue. Live approval rendering can still use runtime/session-owned structures while the durable mission row keeps only coarse mission state.

Slice 2 also does not promise durable multi-pending approval semantics for one mission. Concurrent or queued live approval rendering stays outside durable mission state in this slice.

Non-decision blockers remain `blocked`.

## Task Tracking Boundary

`TaskEntry` remains lightweight in Slice 2.
...
- Slice 2 should not rely on later inference from unrelated task-tracking records to reconstruct mission ownership
