...
#### Scenario: Retry confirmation label describes confirm and cancel paths
- **WHEN** the selected dead-letter row is in retry-confirm state
- **THEN** the retry action label SHALL mention both the confirm path (`r` or `Enter`) and the cancel path (`Esc`)
