...
`lango economy pricing status` is a config-backed inspection command with two simple states: enabled and disabled. No seam is required; tests can execute the command directly with synthetic config.
...
- No change to discount or minimum-price semantics
