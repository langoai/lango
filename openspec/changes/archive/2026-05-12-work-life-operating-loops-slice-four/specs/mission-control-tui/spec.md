...
- **WHEN** one of the approved first-slice predicates holds, such as an accepted proposal with no active linked execution after `10m`, a mission in `done` state updated within `24h` and still needing review, a pending inquiry older than `24h`, a cron job whose most recent execution failed within `24h`, or a retryable dead-letter entry
