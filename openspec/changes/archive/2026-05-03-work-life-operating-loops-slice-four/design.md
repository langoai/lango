...
Slice 3 made Mission Control proactive by introducing transient proposals, but the surface is still centered on individual missions and point-in-time decisions. The approved Slice 4 direction adds a higher-order operating view: recurring work, unresolved inquiries, dead-letter backlog, scheduled automation, and deterministic follow-up signals should all be visible as operator loops.
...
Slice 4 stays projection-first. The first slice should derive `LoopView` rows from existing durable and transient sources rather than adding another persistent table. This keeps the change narrow and avoids competing truths with the existing `mission` layer.
...
Agenda ordering will follow the approved Slice 4 order:
1. waiting-user
2. blocked
3. active
4. scheduled
