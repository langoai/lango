...
#### Scenario: Retry confirmation accepts Enter and r
- **WHEN** the selected dead-letter row is in retry-confirm state
- **THEN** pressing `r` or `Enter` SHALL submit the retry request for the selected row
- **AND** pressing `Esc` SHALL cancel the pending retry request without submitting it
