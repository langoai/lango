...
#### Scenario: Retry confirmation help exposes both confirm keys and cancel key
- **WHEN** the selected dead-letter row is in retry-confirm state
- **THEN** the page help SHALL include `r` and `Enter` for confirming the pending retry request
- **AND** it SHALL include `Esc` for canceling the pending retry request
