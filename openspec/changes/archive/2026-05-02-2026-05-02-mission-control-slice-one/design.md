...
Slice 1 adds a Mission Control page that becomes the default first screen for `lango`. The page renders:
...
- accepting a proposed mission may route to existing prompt or approval flows, but does not create durable mission state or direct learning persistence in Slice 1

### Decision

- the latest pending approval request is the only required live decision in Slice 1
- Slice 1 promises exactly one visible pending approval at a time; if a second approval arrives while one is already pending, this change does not promise queueing, ordering, or concurrent rendering behavior
...
- at minimum, Slice 1 may surface compact status such as active-agent summary, pending-decision count, and degraded-reader notice when that data is already available
...
- Slice 1 should minimize shortcut churn; existing global page shortcuts remain stable, while Mission Control becomes the default route and sidebar/page-selection target
...
The change is valid only if the OpenSpec delta stays inside Slice 1:

- Mission Control is the default surface, but not a new mission engine
- live approvals use the shared pending channel path
- Slice 1 promises only one visible pending approval at a time
