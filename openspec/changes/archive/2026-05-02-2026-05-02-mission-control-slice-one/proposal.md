...
Slice 1 exists to change the first-screen shape without inventing a new mission domain engine. The runtime already has enough honest signals to project a useful Mission Control view, but some UI-facing producers and ownership boundaries need to be tightened first.
...
- Slice 1 promises exactly one visible pending approval at a time; additional concurrent pending approvals are out of scope for this change
...
Slice 1 is explicitly limited to projection and producer hardening:
