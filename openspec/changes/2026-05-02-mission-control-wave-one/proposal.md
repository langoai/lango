...
The current default `lango` surface is still chat-first even though the product direction has shifted toward a mission-native operator experience. Users need to see ongoing work, pending approvals, and prepared suggestions immediately instead of discovering them only after navigating through cockpit pages or waiting for chat interruptions.
...
- pending approvals become live decisions through a shared pending-response owner
